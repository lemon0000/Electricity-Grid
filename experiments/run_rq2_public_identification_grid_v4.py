"""Joint-coupling identification for the E0-aware RQ2 v6 successor."""
...
_STAGE = "identification_grid_v4"
_PAIRWISE_STAGE = "pairwise_replay_v4"
_BASE_METRICS = (
    "correct_failure",
    "b6_failure",
    "correct_shortfall",
...
_PAIR_FIELDS = (
    ...
    "column_id",
    "outcome_resolved",
    "correct_flexibility_capacity",
    "b6_flexibility_capacity",
    "flexibility_underprovisioning",
    "outcome_status",
    "correct_failure",
    "b6_failure",
...
_CELL_STATUS_FIELDS = (
    ...
    "full_training_support_passed",
    "full_training_support_pair_count",
    "full_training_support_failure_count",
)
_POLICY_FIELDS = (
    "cell_id",
    "varied_dimension",
...
def _marginal(
    ...
    states = tuple(row["grid_evaluation_state"] for row in rows)
    return identifiers, probabilities, scores, states
...
def _expected_pairwise_checkpoint_keys(
    ...
    *,
    ...
    row_ids, _, _, _ = _marginal(
        package / "power_system_holdout_marginal.csv.gz"
    )
    column_ids, _, _, _ = _marginal(
    ...
    for status in status_rows:
        ...
        support_passed = _boolean(
            status["full_training_support_passed"],
            "full_training_support_passed",
        )
        eligible = all(feasible) and support_passed
        if (
...
def _matrices(
    ...
    for metric in _BASE_METRICS:
        if metric in {"correct_failure", "b6_failure"}:
            base[metric] = tuple(
                tuple(
                    float(_boolean(lookup[(row, column)][metric], metric))
...
def _preflight(config_path: Path) -> tuple[dict[str, Any], dict[str, object]]:
    ...
    if config["ambiguity_set"] != {
        ...
        "exogenous_grid_infeasibility": {
            "preserve_unconditional_probability_mass": True,
            "conditional_contract_risk_support": (
                "finite_grid_need_rows_only"
            ),
            "include_in_R3_common_insufficiency": False,
            "finite_grid_need_imputation": "forbidden",
        },
    }:
        raise ValueError("transport ambiguity-set contract drifted")
    sampling = config["sampling_uncertainty"]
    if (
        sampling.get("method") != "independent_marginal_block_bootstrap"
        or not isinstance(sampling.get("enabled"), bool)
        or int(sampling.get("bootstrap_replicates", 0)) <= 0
        or int(sampling.get("seed", -1)) < 0
        or not 0.0 < float(sampling.get("confidence_level", 0.0)) < 1.0
        or sampling.get("population_identification_claimed") is not False
    ):
        raise ValueError("sampling-uncertainty contract drifted")
    ...
    if ready:
        ...
        expected_summary = {
            ...
            "exogenous_grid_infeasibility_in_R3": False,
            "capacity_estimand": (
                "normalized_minimum_flexibility_underprovisioning_"
                "not_interconnection_capacity_X"
            ),
            "solver_name": config["input"]["expected_solver_name"],
            "empirical_joint_distribution_claimed": False,
    ...
    report = {
        "schema": "rq2_public_identification_grid_preflight_v4",
...
def run(
    ...
    *,
    ...
    row_ids, row_probability, row_scores, row_states = _marginal(
        package / "power_system_holdout_marginal.csv.gz"
    )
    column_ids, column_probability, column_scores, column_states = _marginal(
        package / "workload_holdout_marginal.csv.gz"
    )
    if any(column_states):
        raise ValueError("workload marginal cannot carry grid states")
    if set(row_states) - {
        FINITE_GRID_NEED,
        EXOGENOUS_GRID_INFEASIBILITY,
    }:
        raise ValueError("power marginal contains unresolved grid states")
    evaluable_indices = tuple(
        index
        for index, state in enumerate(row_states)
        if state == FINITE_GRID_NEED
    )
    exogenous_probability_mass = sum(
        row_probability[index]
        for index, state in enumerate(row_states)
        if state == EXOGENOUS_GRID_INFEASIBILITY
    )
    conditioning_mass = 1.0 - exogenous_probability_mass
    if conditioning_mass <= 0.0:
        raise ValueError("no grid-evaluable holdout mass remains")
    conditional_row_ids = tuple(row_ids[index] for index in evaluable_indices)
    conditional_row_probability = tuple(
        row_probability[index] / conditioning_mass
        for index in evaluable_indices
    )
    conditional_row_scores = tuple(
        row_scores[index] for index in evaluable_indices
    )
    status_fields, status_rows = _read_gzip_rows(
        package / "cell_status.csv.gz"
    )
    ...
    for status in status_rows:
        ...
        for variant in ("correct", "b6"):
            ...
            if feasible:
                capacity = _number(
                    policy["minimum_flexibility_capacity"],
                    "policy.minimum_flexibility_capacity",
                )
                if capacity < 0.0:
                    raise ValueError("policy capacity must be nonnegative")
                lower = _number(policy["lower_bound"], "policy.lower_bound")
                upper = _number(policy["upper_bound"], "policy.upper_bound")
                absolute_gap = _number(
                    policy["absolute_gap"],
                    "policy.absolute_gap",
                )
                relative_gap = _number(
                    policy["relative_gap"],
                    "policy.relative_gap",
                )
                ...
                if (
                    lower > upper
                    or abs((upper - lower) - absolute_gap) > 1.0e-9
                    or absolute_gap > gap_tolerance
                    or relative_gap < 0.0
                    or abs(capacity - upper) > gap_tolerance
                ):
                    raise ValueError("policy optimization certificate drifted")
                policy_capacity[(status["cell_id"], variant)] = capacity
            elif policy["minimum_flexibility_capacity"] != "":
                raise ValueError("nonfeasible policy cannot report capacity")
            if (
                int(policy["model_variables"]) <= 0
                or int(policy["model_constraints"]) <= 0
                or policy["solver_name"]
                != config["input"]["expected_solver_name"]
            ):
                raise ValueError("policy solver/model audit drifted")
            try:
                options = json.loads(policy["solver_options"])
            except json.JSONDecodeError as error:
                raise ValueError("policy solver options are invalid") from error
            if not isinstance(options, dict) or not options:
                raise ValueError("policy solver options are invalid")
        ...
        if (
            ...
            or status_pairwise_eligible
            is not (all(policy_feasible) and support_passed)
        ):
    ...
    state_by_row = dict(zip(row_ids, row_states, strict=True))
    for status in status_rows:
        cell_id = status["cell_id"]
        ...
        for row in cell_pairs:
            ...
            expected_status = state_by_row[row["row_id"]]
            if outcome_status != expected_status:
                raise ValueError("pairwise outcome and power state disagree")
            for variant in ("correct", "b6"):
                capacity = _number(
                    row[f"{variant}_flexibility_capacity"],
                    f"{variant}_flexibility_capacity",
                )
                if abs(capacity - policy_capacity[(cell_id, variant)]) > 1.0e-12:
                    raise ValueError("pairwise policy capacity drifted")
            if outcome_status == EXOGENOUS_GRID_INFEASIBILITY:
                if any(row[metric] != "" for metric in _BASE_METRICS):
                    raise ValueError("E0 pair cannot contain finite metrics")
                continue
            if outcome_status != FINITE_GRID_NEED:
                raise ValueError("pairwise outcome status is unsupported")
            for variant in ("correct", "b6"):
                failure = _boolean(
            ...
            underprovisioning = _number(
                row["flexibility_underprovisioning"],
                "flexibility_underprovisioning",
            )
    ...
    independent = tuple(
        ...
        for row_mass in conditional_row_probability
    )
    comonotone = quantile_coupling(
        conditional_row_probability,
        column_probability,
        conditional_row_scores,
        column_scores,
    )
    ...
    for status in status_rows:
        ...
        if not eligible or not complete:
            ...
            if not training_resolved:
                ...
                reason = "representative_policy_failed_full_training_support"
            else:
        ...
        finite_pairs = [
            row
            for row in pairs_by_cell[cell_id]
            if row["outcome_status"] == FINITE_GRID_NEED
        ]
        matrices = _matrices(
            finite_pairs,
            conditional_row_ids,
        ...
        for metric in _BOUND_METRICS:
            ...
            if config["sampling_uncertainty"]["enabled"]:
                bootstrap = bootstrap_transport_interval(
                    conditional_row_probability,
                    column_probability,
                    matrix,
                    replicates=int(
                        config["sampling_uncertainty"]["bootstrap_replicates"]
                    ),
                    seed=int(config["sampling_uncertainty"]["seed"]),
                    confidence_level=float(
                        config["sampling_uncertainty"]["confidence_level"]
                    ),
                )
        ...
        joint = joint_region_compatibility(
            conditional_row_probability,
            column_probability,
            {
                metric: matrices[metric]
                for metric in (
                    "delta_failure_probability",
                    "delta_expected_shortfall",
                    "flexibility_underprovisioning",
        ...
        if classification.identified and not set(
            classification.compatible_regions
        ).issubset(joint.compatible_regions):
            raise AssertionError(
                "metric-wise identified region lacks a common coupling witness"
            )
        compatible_regions = (
            classification.compatible_regions
            if classification.identified
            else joint.compatible_regions
        )
        joint_regions.append(
            {
                "cell_id": cell_id,
                "compatible_regions": joint.compatible_regions,
                "witness_couplings": joint.witness_couplings,
                "solver_status": joint.solver_status,
            }
        )
        identifications.append(
            {
                ...
                "compatibility_rule": "single_common_transport_coupling",
            }
        )
    ...
    try:
        if _sha256(config_path) != config_sha:
            raise ValueError("identification config drifted during execution")
        ...
        (staging / "joint_region_compatibility.json").write_text(
            json.dumps(joint_regions, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
        (staging / "bootstrap_transport_intervals.json").write_text(
        ...
        summary = {
            ...
            "joint_region_compatibility_reported": (
                len(joint_regions) == len(bounds_by_cell)
            ),
            "holdout_exogenous_grid_infeasibility_probability_mass": (
                exogenous_probability_mass
            ),
            "contract_risk_conditioning_probability_mass": conditioning_mass,
            "exogenous_grid_infeasibility_in_R3": False,
            "capacity_estimand": (
                "normalized_minimum_flexibility_underprovisioning_"
                "not_interconnection_capacity_X"
            ...
            "bootstrap_replicates": (
                int(config["sampling_uncertainty"]["bootstrap_replicates"])
                if config["sampling_uncertainty"]["enabled"]
                else 0
            ),
            "bootstrap_seed": int(config["sampling_uncertainty"]["seed"]),
            "bootstrap_confidence_level": float(
                config["sampling_uncertainty"]["confidence_level"]
            ),
            "bootstrap_is_population_identification": False,
            "joint_parameter_robustness_claimed": False,
            "empirical_joint_distribution_claimed": False,
            "empirical_probability_claimed": False,
            "security_certified": False,
        ...
        names = (
            ...
            "bootstrap_transport_intervals.json",
            "cell_bounds.csv.gz",
            "cell_identification.json",
            "config.yaml",
            "joint_region_compatibility.json",
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/rq2_public_identification_grid_v4.yaml"),
    )
