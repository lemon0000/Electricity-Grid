...
_STAGE = "identification_grid_v2"
_PAIRWISE_STAGE = "pairwise_replay_v2"
_BASE_METRICS = (
    "correct_failure",
    "b6_failure",
    "correct_shortfall",
...
def _preflight(config_path: Path) -> tuple[dict[str, Any], dict[str, object]]:
    ...
    if ready:
        ...
        verified_pairwise_provenance = verify_stage_provenance(
            json.loads(
                pairwise_provenance_path.read_text(encoding="utf-8")
            ),
            stage=_PAIRWISE_STAGE,
        ...
        checkpoint_inventory = json.loads(
            (package / "checkpoint_inventory.json").read_text(
                encoding="utf-8"
            )
        )
        if (
            checkpoint_inventory
            != verified_pairwise_provenance["checkpoint_inventory"]
            or summary.get("checkpoint_inventory_sha256")
            != canonical_sha256(checkpoint_inventory)
        ):
            raise ValueError("pairwise checkpoint inventory provenance drifted")
    report = {
        "schema": "rq2_public_identification_grid_preflight_v2",
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/rq2_public_identification_grid_v2.yaml"),
    )
