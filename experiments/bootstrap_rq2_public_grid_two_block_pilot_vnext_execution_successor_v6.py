"""Stdlib-first bootstrap for review-closed Vnext execution successor v6."""
...
class BootstrapRejected(RuntimeError):
    """The pre-project-import V6 bootstrap failed closed."""
...
def _verify_bundle() -> tuple[dict[str, object], str]:
    try:
        ...
        raise BootstrapRejected("V6 bundle JSON malformed") from exc
    ...
    if (
        ...
        raise BootstrapRejected("V6 bundle identity/inventory drifted")
    for relative, expected in inner["files"].items():
        if _sha(_stable(ROOT / relative)) != expected:
            raise BootstrapRejected(f"V6 bundle member hash drifted: {relative}")
...
def _preimport_review(config: dict[str, object], outer_sha256: str) -> None:
    if not REVIEW.exists() or REVIEW.is_symlink():
        raise BootstrapRejected("fixed V6 execution-review PASS receipt is absent")
    try:
        value = json.loads(_stable(REVIEW))
    except json.JSONDecodeError as exc:
        raise BootstrapRejected("V6 execution-review receipt JSON malformed") from exc
    authority = config["fixed_execution_review"]
    predecessor = config["predecessor_v5"]
    ...
    if (
        ...
        raise BootstrapRejected("V6 execution-review receipt binding/effect drifted")
...
def main(argv: list[str] | None = None) -> int:
    ...
    from experiments import (
        run_rq2_public_grid_two_block_pilot_vnext_execution_successor_v6 as controller,
    )
