"""E0-aware checkpointed Cartesian replay for the RQ2 v6 successor."""
...
_STAGE = "pairwise_replay_v4"
_GRID_STAGE = "grid_need_dispatch_v4"
_POLICY_FIELDS = (
    "cell_id",
    "varied_dimension",
    "variant",
...
def _capacity_row(
    ...
    return {
        ...
        "absolute_gap": (
            "" if result.absolute_gap is None else result.absolute_gap
        ),
        "relative_gap": (
            "" if result.relative_gap is None else result.relative_gap
        ),
        "gap_tolerance": (
            "" if result.gap_tolerance is None else result.gap_tolerance
        ),
        "model_variables": result.model_variables,
        "model_constraints": result.model_constraints,
        "solver_name": result.solver_name,
        "solver_options": json.dumps(result.solver_options, sort_keys=True),
    }
...
def _pair_payload(
    ...
    *,
    ...
    if power_state == EXOGENOUS_GRID_INFEASIBILITY:
        row = {
            field: "" for field in _PAIR_FIELDS
        }
        row.update(
            {
                "cell_id": cell.cell_id,
                ...
                "correct_flexibility_capacity": correct_capacity,
                "b6_flexibility_capacity": b6_capacity,
    ...
    if power_state != FINITE_GRID_NEED:
        raise ValueError(f"unsupported power block state: {power_state}")
    ...
    row = {
        ...
        "outcome_status": (
            FINITE_GRID_NEED if resolved else "unresolved_pairwise_execution"
        ),
        "correct_flexibility_capacity": correct_capacity,
        "b6_flexibility_capacity": b6_capacity,
        "flexibility_underprovisioning": correct_capacity - b6_capacity,
    ...
    return [
        {
            ...
            "grid_evaluation_state": (
                ""
                if state_by_block is None
                else state_by_block[block.block_id]
            ),
        }
        for block in blocks
    ]
...
def _preflight(config_path: Path) -> tuple[dict[str, Any], dict[str, object]]:
    ...
    workload_summary = load_json_strict(workload_package / "summary.json")
    ...
    power_state_by_block: dict[str, str] = {}
    ...
    if power_ready:
        ...
        loaded_training = load_power_blocks_with_state(
            power_package,
            "training",
        )
        loaded_holdout = load_power_blocks_with_state(
            power_package,
            "holdout",
        )
        power_training = loaded_training.blocks
        power_holdout = loaded_holdout.blocks
        power_state_by_block = {
            **loaded_training.state_by_block,
            **loaded_holdout.state_by_block,
        }
        power_summary = load_json_strict(power_package / "summary.json")
        ...
        power_provenance = load_json_strict(power_provenance_path)
        checkpoint_inventory = load_json_strict(
            power_package / "checkpoint_inventory.json"
        )
        verify_checkpoint_inventory_bundle(
            power_provenance,
    ...
    solver_specification = solver_spec(config["solver"])
    ...
    report = {
        "schema": "rq2_public_pairwise_replay_preflight_v4",
    ...
    return config, {
        ...
        "power_state_by_block": power_state_by_block,
...
def run(
    ...
    *,
    ...
    cells = context["cells"]
    power_state_by_block = context["power_state_by_block"]
    training_marginal = condition_on_grid_evaluable(
        power_training,
        {
            block.block_id: power_state_by_block[block.block_id]
            for block in power_training
        },
    )
    holdout_marginal = condition_on_grid_evaluable(
        power_holdout,
        {
            block.block_id: power_state_by_block[block.block_id]
            for block in power_holdout
        },
    )
    selected_power = select_weighted_quantile_representatives(
        training_marginal.evaluable_blocks,
    ...
    for cell in cells:
        ...
        checkpoint = _load_checkpoint(
            ...
            schema="rq2_public_policy_checkpoint_v4",
        )
        if checkpoint is None:
            ...
            plan = plan_minimum_flexibility_pair_with_spec(
                model_inputs,
                solver_specification=context["solver_specification"],
            )
            support_audits = ()
            if plan.correct.feasible and plan.b6.feasible:
                support_audits = audit_training_support(
                    training_marginal.evaluable_blocks,
                    workload_training,
                    cell,
                    correct_capacity=float(plan.correct.minimum_capacity),
                    b6_capacity=float(plan.b6.minimum_capacity),
                    fixed_policy=config["fixed_policy"],
                )
            checkpoint = {
                "schema": "rq2_public_policy_checkpoint_v4",
                **identity,
                ...
                "b6": asdict(plan.b6),
                "full_training_support": [
                    asdict(audit) for audit in support_audits
                ],
            }
    ...
    for policy in policy_checkpoints:
        ...
        b6 = policy["b6"]
        support_passed = bool(policy["full_training_support"]) and all(
            not item["failed_pair_ids"]
            for item in policy["full_training_support"]
        )
        if (
            not correct["feasible"]
            or not b6["feasible"]
            or not support_passed
        ):
            continue
            ...
            for workload in workload_holdout:
                ...
                pair_identity = {
                    **identity,
                    ...
                    "row_id": power.block_id,
                    "column_id": workload.block_id,
                    "correct_flexibility_capacity": correct[
                        "minimum_capacity"
                    ],
                    "b6_flexibility_capacity": b6["minimum_capacity"],
                    "policy_checkpoint_sha256": policy_checkpoint_sha256,
                }
                checkpoint = _load_checkpoint(
                    path,
                    pair_identity,
                    schema="rq2_public_pair_checkpoint_v4",
                )
                if checkpoint is None:
                    payload = _pair_payload(
                        ...
                        power_state=power_state_by_block[power.block_id],
                        config=config,
                    )
                    checkpoint = {
                        "schema": "rq2_public_pair_checkpoint_v4",
                ...
                if maximum_pairs is not None and processed >= maximum_pairs:
                    return {
                        "schema": "rq2_public_pairwise_replay_progress_v4",
    ...
    eligible_cells = {
        item["cell_id"]
        for item in policy_checkpoints
        if item["correct"]["feasible"]
        and item["b6"]["feasible"]
        and item["full_training_support"]
        and all(
            not audit["failed_pair_ids"]
            for audit in item["full_training_support"]
        )
    }
    ...
    for policy in policy_checkpoints:
        ...
        support_audits = policy["full_training_support"]
        support_pair_count = (
            int(support_audits[0]["pair_count"])
            if support_audits
            else 0
        )
        support_failure_count = sum(
            len(audit["failed_pair_ids"]) for audit in support_audits
        )
        status_rows.append(
            {
                ...
                "full_training_support_passed": (
                    bool(support_audits) and support_failure_count == 0
                ),
                "full_training_support_pair_count": support_pair_count,
                "full_training_support_failure_count": support_failure_count,
            }
        )
    unresolved_pairs = [
    ...
    expected_checkpoint_keys = {
        f"policies/{cell.cell_id}.json" for cell in cells
    }
    expected_checkpoint_keys.update(
        f"pairs/{cell_id}/{power.block_id}__{workload.block_id}.json"
    ...
    if observed_checkpoint_keys != expected_checkpoint_keys:
        raise ValueError("pairwise checkpoint file set drifted")
    ...
    try:
        if _sha256(config_path) != config_sha:
            raise ValueError("pairwise replay config drifted during execution")
        shutil.copyfile(config_path, staging / "config.yaml")
        policy_rows = [
            _capacity_row(
                cell_by_id[item["cell_id"]],
                variant,
                MinimumFlexibilityCapacitySuccessor(**item[variant]),
            )
            for item in policy_checkpoints
        ...
        _write_gzip_csv(
            ...
            _marginal_rows(
                power_holdout,
                role="power",
                state_by_block=power_state_by_block,
            ),
        )
        _write_gzip_csv(
        ...
        summary = {
            ...
            "training_support_invalid_cell_count": sum(
                not row["full_training_support_passed"]
                and row["correct_training_feasible"]
                and row["b6_training_feasible"]
                for row in status_rows
            ),
            "training_infeasible_cell_count": len(cells)
            - sum(
                row["correct_training_feasible"]
                and row["b6_training_feasible"]
                for row in status_rows
            )
            - len(unresolved_training),
            ...
            "finite_contract_pairwise_outcome_count": sum(
                row["outcome_status"] == FINITE_GRID_NEED
                for row in pair_rows
            ),
            "exogenous_grid_infeasibility_pair_count": sum(
                row["outcome_status"] == EXOGENOUS_GRID_INFEASIBILITY
                for row in pair_rows
            ),
            "holdout_exogenous_grid_infeasibility_probability_mass": (
                holdout_marginal.exogenous_probability_mass
            ),
            "training_exogenous_grid_infeasibility_probability_mass": (
                training_marginal.exogenous_probability_mass
            ),
            "contract_risk_conditioning_probability_mass": (
                holdout_marginal.conditioning_probability_mass
            ),
            "training_conditioning_probability_mass": (
                training_marginal.conditioning_probability_mass
            ),
            ...
            "solver_name": context["solver_specification"].name,
            "solver_contract": context["solver_specification"].__dict__,
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/rq2_public_pairwise_replay_v4.yaml"),
    )
