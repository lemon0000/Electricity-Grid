"""Reproducible HiGHS pilot for the terminated AC-aware candidate solve."""
...
_CONFIG_PATH = Path("configs/rts_gmlc_zero_dc_ac_aware_solver_benchmark.yaml")
...
def _read_config(path: Path) -> dict[str, Any]:
    ...
    if (
        benchmark.get("id")
        != "rts_gmlc_google_day0_zero_dc_ac_aware_solver_benchmark_v1"
        or benchmark.get("schema")
        != "rts_gmlc_zero_dc_ac_aware_solver_benchmark_v1"
        ...
        != "repository_local_solver_pilot_after_v2_operational_termination"
    ...
    if config["output"] != {
        "directory": "results/tables/rts_gmlc_google_day0_zero_dc_ac_aware_solver_benchmark_v1",
        "log_directory": "results/logs/rts_gmlc_zero_dc_ac_aware_solver_benchmark_v1",
...
def _solve_proxy_once(
    ...
    *,
    ...
    if not solver.available(exception_flag=False):
        raise RuntimeError("HiGHS is unavailable for the solver pilot")
    options = {
        "threads": threads,
        "random_seed": int(solver_config["random_seed"]),
        "time_limit": float(solver_config["time_limit_seconds_per_stage"]),
        "mip_rel_gap": float(solver_config["target_mip_relative_gap"]),
        "mip_abs_gap": float(solver_config["mip_absolute_gap"]),
        ...
        "mip_report_level": int(solver_config["mip_report_level"]),
...
def run_benchmark(
    ...
    def writer(staging: Path) -> None:
        ...
        for threads, repetition in specs:
            ...
            relative_log = Path(f"highs_t{threads}_r{repetition}.log")
            _append_jsonl(
                progress_path,
                {
                    "event": "run_started",
