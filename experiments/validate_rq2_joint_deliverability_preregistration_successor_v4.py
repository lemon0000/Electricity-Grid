"""Validate the complete RQ2 joint-deliverability scientific successor v4."""
...
CONFIG_RELATIVE = "configs/rq2_joint_deliverability_preregistration_successor_v4.yaml"
SPEC_RELATIVE = "docs/model_spec/rq2_joint_deliverability_estimands_v3.md"
PLAN_RELATIVE = "docs/plan/RQ2_联合服务可交付前沿确认性方案_v4.md"
VALIDATOR_RELATIVE = (
    "experiments/validate_rq2_joint_deliverability_preregistration_successor_v4.py"
)
TEST_RELATIVE = "tests/test_rq2_joint_deliverability_preregistration_successor_v4.py"
INNER_RELATIVE = (
    "configs/rq2_joint_deliverability_preregistration_successor_v4.SHA256SUMS.json"
)
OUTER_RELATIVE = (
    "configs/rq2_joint_deliverability_preregistration_successor_v4."
...
V3_OUTER = (
    ROOT / "configs/rq2_joint_deliverability_preregistration_successor_v3."
    "OUTER.SHA256SUMS.json"
)
V3_OUTER_SHA256 = "edbbe5be9ae559a258b8928a6678e4adc9abeda4ce9c841238d67589d372bc99"
V3_ESCALATE = (
    ROOT / "configs/rq2_joint_deliverability_preregistration_review_escalate_v3.yaml"
)
V3_ESCALATE_SHA256 = "0e0acf0ddf49d2eca3c68529b3cf111eb90f0cdda6d7c6ce6bbf6421d2d533d3"

EXPECTED_SEMANTIC_SHA256 = (
    "a50b6fb8d4ff06dfdb557053014faeb138b00285d5981767eccff81ff6c2efb6"
)
BOOTSTRAP_PROBE_SHA256 = (
    "f2a5ed36b6bb1c263b16c5888efe6a36b6072d3caf0c98d7ad50fdfca296c9d9"
)
TRANSPORT_PROBE_SHA256 = (
    "510ef5cd1578927cf134893e8a17bf5172bc0f875cdb8c5cfb4e381b1181ce76"
)
EXPECTED_SPEC_SEMANTIC_SHA256 = (
    "f133accce45bc9b6c2e128025fe57d8e341e0a934ee48fb0c9ac42ebc67d63ad"
)
EXPECTED_PLAN_SEMANTIC_SHA256 = (
    "f6dfa805e642bcc09424df1a1bf8272eef3bbf757e0534be65e5f19a9064a9f7"
)
EXPECTED_TOP_LEVEL = {
    "schema",
    "version",
...
def _validate_authority(design: dict[str, Any]) -> None:
    ...
    for path, digest, label in (
        (V3_OUTER, V3_OUTER_SHA256, "v3 outer"),
        (V3_ESCALATE, V3_ESCALATE_SHA256, "v3 ESCALATE receipt"),
    ):
        _require_regular_file(path, label)
        _equal(_sha256(path), digest, f"{label} hash")
    receipt = _load_yaml(V3_ESCALATE)
    _equal(receipt.get("verdict"), "ESCALATE", "v3 review verdict")
    _equal(len(_sequence(receipt.get("findings"), "v3 findings")), 4, "v3 Major count")
...
def transport_probe_payload(design: dict[str, Any]) -> dict[str, object]:
    ...
    payload = {
        "schema": "rq2_joint_deliverability_transport_probe_v4",
...
def _validate_scientific_invariants(design: dict[str, Any]) -> None:
    ...
    _equal(
        transport["multimetric_sign_statement_requires_one_common_pi_witness"],
        True,
        "common-pi gate",
    )
    common_pi = _mapping(transport["common_pi_problem"], "common-pi problem")
    _equal(
        common_pi,
        {
            "predicate_order": "registered_metric_order_then_predicate_order",
            "predicate_forms": {
                "robust_positive": (
                    "negative_metric_dot_pi_less_than_or_equal_to_negative_1e-6"
                ),
                "robust_negative": (
                    "metric_dot_pi_less_than_or_equal_to_negative_1e-6"
                ),
                "nonpositive": "metric_dot_pi_less_than_or_equal_to_1e-6",
            },
            "phase_one_variable": "z_nonnegative",
            "phase_one_constraint": (
                "each_predicate_left_side_minus_z_less_than_or_equal_to_rhs"
            ),
            "phase_one_objective": "minimize_z",
            "compatible": (
                "valid primal feasible witness with phase_one_optimum less than "
                "or equal to 1e-8 and all certificate residuals at most 1e-8"
            ),
            "certified_incompatible": (
                "valid dual feasible lower bound on phase_one_optimum greater than 1e-8"
            ),
            "otherwise": "unresolved",
            "certificate_requirements": [
                "primal_feasible_solution",
                "dual_feasible_solution",
                "primal_dual_gap_not_greater_than_1e-8",
                "marginal_residual_not_greater_than_1e-8",
                "predicate_residual_not_greater_than_1e-8",
                "dual_lower_bound_serialized",
            ],
        },
        "common-pi problem",
    )
    statements = _mapping(
        transport["common_pi_registered_statements"],
        "common-pi registered statements",
    )
    _equal(
        set(statements),
        {"b6_operational_penalty", "b6_operational_relief"},
        "common-pi statement inventory",
    )
    _equal(
        statements,
        {
            "b6_operational_penalty": {
                "logic": "any_single_branch_compatible",
                "branches": [
                    {
                        "branch_id": "b6_joint_service_failure_positive",
                        "predicates": [
                            {
                                "metric": ("B6_minus_correct_joint_service_failure"),
                                "form": "robust_positive",
                            }
                        ],
                    },
                    {
                        "branch_id": "b6_total_service_shortfall_positive",
                        "predicates": [
                            {
                                "metric": ("B6_minus_correct_total_service_shortfall"),
                                "form": "robust_positive",
                            }
                        ],
                    },
                    {
                        "branch_id": "b6_cfe_shortfall_positive",
                        "predicates": [
                            {
                                "metric": "B6_minus_correct_cfe_shortfall",
                                "form": "robust_positive",
                            }
                        ],
                    },
            ...
            "b6_operational_relief": {
                ...
                "branches": [
                    {
                        "branch_id": "b6_joint_service_failure_negative",
                        "predicates": [
                            {
                                "metric": ("B6_minus_correct_joint_service_failure"),
                                "form": "robust_negative",
                            }
                        ],
                    },
                    {
                        "branch_id": "b6_total_service_shortfall_negative",
                        "predicates": [
                            {
                                "metric": ("B6_minus_correct_total_service_shortfall"),
                                "form": "robust_negative",
                    ...
                    {
                        "branch_id": "b6_cfe_shortfall_negative",
                        "predicates": [
                            {
                                "metric": "B6_minus_correct_cfe_shortfall",
                                "form": "robust_negative",
...
def validate_design(
    ...
    *,
    ...
    _equal(
        design["schema"],
        "rq2_joint_deliverability_preregistration_successor_v4",
        "schema",
    )
    _equal(design["version"], 4, "version")
...
def _validate_manifest_shapes(
    ...
    *,
    ...
    _equal(
        inner["schema"],
        "rq2_joint_deliverability_preregistration_successor_manifest_v4",
        "inner schema",
    )
    ...
    _equal(
        outer["schema"],
        "rq2_joint_deliverability_preregistration_successor_outer_v4",
        "outer schema",
    )
    _equal(outer["version"], 4, "outer version")
