"""Narrow repair-005 successor for fail-closed cost decision bisection."""
...
CHECKPOINT_SCHEMA = "rts_gmlc_v4_repair_005_cost_bisection_candidate_checkpoint_v1"
PREFIX_EVIDENCE_SCHEMA = "rts_gmlc_v4_repair_005_prefix_import_evidence_v1"
HYBRID_EVIDENCE_SCHEMA = "rts_gmlc_v4_repair_005_hybrid_cost_evidence_v1"
COMBINED_COST_SCHEMA = "rts_gmlc_v4_repair_005_combined_cost_certificate_v1"
FRONTIER_SCHEMA = "rts_gmlc_v4_repair_005_candidate_frontier_v1"
FLOAT_SERIALIZATION = "python_json_roundtrip_full_precision_v1"
_SUCCESSOR_MODULE = (
    "experiments.run_rts_gmlc_zero_dc_ac_aware_commitment_v4_repair_005_formal"
)
_CONFIG_KEYS = {
    "schema",
    "preregistration",
...
def _read_config(config_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(config, dict) or set(config) != _CONFIG_KEYS:
        raise ValueError("repair-005 config schema drifted")
    ...
    if (
        config.get("schema") != "rts_gmlc_v4_repair_005_formal_successor_config_v1"
        or preregistration.get("id")
        != "rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4_repair_005"
        or preregistration.get("schema")
        != "rts_gmlc_zero_dc_ac_aware_commitment_preregistration_v4_repair_005"
        ...
        != list(invalidation.EXPECTED_CHECKPOINT_MANIFEST_SHA256S)
        or predecessor.get("candidate_json_sha256s")
        != list(invalidation.EXPECTED_CANDIDATE_JSON_SHA256S)
        ...
        raise ValueError("repair-005 frozen contract drifted")
...
def _verify_parent_invalidation(
    ...
    root = invalidation_root or (
        Path(predecessor["root"]) / str(predecessor["invalidation_relative_path"])
    )
    _verify_manifest(root)
    if _sha256(root / "SHA256SUMS") != predecessor["invalidation_manifest_sha256"]:
        raise RuntimeError("repair-005 parent invalidation manifest drifted")
    payload = v4._load_json(root, "invalidation.json")
    if (
        payload.get("status") != predecessor["required_invalidation_status"]
        ...
        or payload.get("candidate_checkpoint_manifest_sha256s")
        != predecessor["candidate_checkpoint_manifest_sha256s"]
        or payload.get("candidate_json_sha256s")
        != predecessor["candidate_json_sha256s"]
        or payload.get("failure_is_infeasibility_evidence") is not False
        or payload.get("direct_cost_certificate_valid") is not True
        or payload.get("direct_cost_full_state_audit_passed") is not True
        or payload.get("direct_cost_residual_audit_passed") is not True
        or payload.get("candidate_frontier_artifact_published") is not False
        or payload.get("joint_ac_solver_call_count") != 0
        or payload.get("repair_004_resume_allowed") is not False
        or payload.get("successor_must_restart_from_candidate_ordinal") != 5
        or payload.get("scientific_protocol_changed") is not False
    ):
        raise RuntimeError("repair-005 parent invalidation content drifted")


def _source_context(config: Mapping[str, Any]) -> v4._FrontierContext:
    return repair004._build_context(
        Path(config["predecessor_repair_004"]["config_path"])
    )
...
def _verified_source_prefix(
    ...
    for ordinal in range(1, 5):
        item = repair004._load_candidate_checkpoint(source_context, root, ordinal)
        if item is None:
            raise RuntimeError("repair-005 source prefix checkpoint is missing")
        ...
        if (
            ...
            raise RuntimeError("repair-005 source prefix hash drifted")
...
def _verify_frozen_inputs(
    ...
    for key, path_value in config["implementation"].items():
        ...
        if re.fullmatch(r"[0-9a-f]{64}", expected) is None:
            raise RuntimeError(f"repair-005 implementation hash is not frozen: {key}")
    ...
    if len(_verified_source_prefix(config)) != 4:
        raise RuntimeError("repair-005 source prefix count drifted")
...
def _build_context(
    ...
    contract = {
        "schema": "rts_gmlc_v4_repair_005_formal_inputs_v1",
        ...
        "predecessor_repair_004": successor_config["predecessor_repair_004"],
    ...
    *,
    ...
    if target.exists():
        ...
        if (
            ...
            raise RuntimeError("repair-005 published preregistration drifted")
        return observed
    if output_root.exists() and any(output_root.iterdir()):
        raise RuntimeError("Cannot prepare repair-005 beside existing artifacts")
...
def _prefix_candidate(
    ...
    predecessor = context.input_contract["predecessor_repair_004"]
    ...
    imported = replace(
        candidate,
        source="repair_005_verified_repair_004_prefix",
...
def _hybrid_candidate(
    ...
    *,
    ...
    predecessor_evidence_sha = common_input_signature_sha256(
        {
            ...
            "repair_004_proxy_successor_contract_sha256": (
                context.input_contract["predecessor_repair_004"][
        ...
        return run_exact_cg_stage(
            ...
            time_limits=repair004.ExactCgTimeLimits(
                master_seconds=float(
                    context.input_contract["predecessor_repair_004"].get(
    ...
    parent_successor = repair004._read_config(
        Path(context.input_contract["predecessor_repair_004"]["config_path"])
    )["formal_successor"]
    ...
    proxy_evidence: dict[str, object] = {
        "schema": repair004.HYBRID_EVIDENCE_SCHEMA,
...
def _validate_candidate_physics(
    ...
    if ordinal <= 4:
        ...
        if (
            candidate.source != "repair_005_verified_repair_004_prefix"
    ...
    repair004._validate_candidate_physics(candidate, source_context, ordinal)
...
def _validate_proxy_checkpoint_evidence(
    ...
    *,
    ...
    if (
        proxy.get("schema") != repair004.HYBRID_EVIDENCE_SCHEMA
...
def _validate_round_artifacts(
    ...
    repair004._validate_round_artifacts(
        checkpoint_root,
        context,
        ordinal,
        candidate,
...
def _run_joint_ac_attempt(
    ...
    original = repair004._joint_worker_command
    repair004._joint_worker_command = _joint_worker_command
    try:
        return repair004._run_joint_ac_attempt(
            context,
            output_root,
            registration,
            candidates,
        ...
        repair004._joint_worker_command = original
