"""Stdlib-first bootstrap for RQ2 joint-deliverability activation v2."""
...
CONFIG_RELATIVE = "configs/rq2_joint_deliverability_activation_successor_v2.json"
INNER_RELATIVE = (
    "configs/rq2_joint_deliverability_activation_successor_v2.SHA256SUMS.json"
)
OUTER_RELATIVE = (
    "configs/rq2_joint_deliverability_activation_successor_v2.OUTER.SHA256SUMS.json"
)
CONFIG = ROOT / CONFIG_RELATIVE
INNER = ROOT / INNER_RELATIVE
OUTER = ROOT / OUTER_RELATIVE
BOOTSTRAP_RELATIVE = "experiments/bootstrap_rq2_joint_deliverability_activation_v2.py"
CONTROLLER_RELATIVE = "experiments/run_rq2_joint_deliverability_activation_v2.py"
EXECUTION_CORE_RELATIVE = "src/rq2_joint_deliverability_execution_v3/core.py"
ACTIVATION_REVIEW_RELATIVE = (
    "configs/rq2_joint_deliverability_activation_review_pass_v2.json"
)
ACTIVATION_REVIEW_SCHEMA = "rq2_joint_deliverability_activation_review_pass_v2"
ACTIVATION_REVIEW_SCOPE = "rq2_joint_deliverability_activation_successor_v2_exact_outer"
...
if envelope.get("schema") != "rq2_joint_deliverability_activation_parent_envelope_v2":
...
def _absolute_parts(path: Path) -> tuple[Path, tuple[str, ...]]:
    ...
    if not absolute.is_absolute() or not absolute.anchor or len(parts) < 2:
        raise ActivationRejected(f"anchored file path is invalid: {path}")
    if any(part in {"", ".", ".."} for part in parts[1:]):
        raise ActivationRejected(f"anchored file path is non-canonical: {path}")
...
def _posix_open_anchored(path: Path, *, final_directory: bool = False) -> int:
    ...
    try:
        ...
        raise ActivationRejected(f"filesystem anchor is unavailable: {path}") from error
    try:
        for index, part in enumerate(parts):
            final = index == len(parts) - 1
            flags = directory_flags if (not final or final_directory) else file_flags
            child = os.open(part, flags, dir_fd=descriptor)
            descriptors.append(child)
            descriptor = child
        metadata = os.fstat(descriptor)
        expected = stat.S_ISDIR if final_directory else stat.S_ISREG
        if not expected(metadata.st_mode):
            kind = "directory" if final_directory else "ordinary file"
            raise ActivationRejected(f"{kind} required: {path}")
        result = os.dup(descriptor)
    except OSError as error:
        if error.errno in {errno.ELOOP, errno.ENOTDIR}:
            rejection = ActivationRejected(
                f"path alias or non-directory component rejected: {path}"
            )
        else:
...
def _windows_open_anchored(path: Path, *, final_directory: bool = False) -> int:
    import ctypes
    import msvcrt
    from ctypes import wintypes

    absolute, parts = _absolute_parts(path)
    if absolute.anchor.startswith("\\\\"):
        raise ActivationRejected("UNC anchors are not registered")
    ...
    def reject_reparse(handle: int, label: str) -> None:
        ...
        if not kernel32.GetFileInformationByHandleEx(
            ...
            raise OSError(ctypes.get_last_error(), f"attribute query failed: {label}")
        if info.FileAttributes & file_attribute_reparse_point:
            raise ActivationRejected(f"path alias or reparse point rejected: {path}")

    def close_handle_with_recovery(owned: int) -> OSError | None:
        if kernel32.CloseHandle(owned):
            return None
        first_error = OSError(ctypes.get_last_error(), "CloseHandle failed")
        info = FileAttributeTagInfo()
        if not kernel32.GetFileInformationByHandleEx(
            owned,
            file_attribute_tag_info,
            ...
            if ctypes.get_last_error() == 6:
                return first_error
            return OSError(
                ctypes.get_last_error(), "handle close state is indeterminate"
            )
        if not kernel32.CloseHandle(owned):
            return OSError(ctypes.get_last_error(), "CloseHandle retry failed")
        return first_error
    ...
    def open_relative(parent: int, name: str, *, directory: bool) -> int:
        ...
        access |= file_list_directory | file_traverse if directory else file_read_data
        ...
        if status_code < 0:
            winerror = int(ntdll.RtlNtStatusToDosError(status_code))
            if winerror in {2, 3}:
                raise FileNotFoundError(
                    winerror,
                    f"NtCreateFile did not find component: {name}",
                )
            raise OSError(
                winerror,
                f"NtCreateFile failed with NTSTATUS 0x{status_code & 0xFFFFFFFF:08x}",
            )
    ...
    if root_handle == invalid_handle:
        raise ActivationRejected(f"filesystem anchor is unavailable: {path}")
    handles = [int(root_handle)]
    handle = handles[-1]
    descriptor = -1
    try:
        reject_reparse(handle, absolute.anchor)
        for index, part in enumerate(parts):
            final = index == len(parts) - 1
            child = open_relative(
                handle,
            ...
            handle = child
            reject_reparse(handle, part)
        transferred_handle = handles.pop()
        try:
            ...
            handles.append(transferred_handle)
            raise
        metadata = os.fstat(descriptor)
        expected = stat.S_ISDIR if final_directory else stat.S_ISREG
        if not expected(metadata.st_mode):
        ...
        while handles:
            owned = handles.pop()
            close_error = close_handle_with_recovery(owned)
            if close_error is not None:
                close_failures.append(close_error)
        if close_failures:
            raise ActivationRejected("Windows ancestor handle cleanup failed") from (
                close_failures[-1]
            )
        result = descriptor
        descriptor = -1
        ...
        if isinstance(error, ActivationRejected):
            raise
        raise ActivationRejected(f"handle-anchored open failed: {path}") from error
    finally:
        if descriptor >= 0:
            _close_descriptor_with_recovery(descriptor)
        while handles:
            owned = handles.pop()
            close_handle_with_recovery(owned)


def _open_anchored(path: Path, *, final_directory: bool = False) -> int:
    if os.name == "nt":
        return _windows_open_anchored(path, final_directory=final_directory)
    if not _OPEN_SUPPORTS_DIR_FD:
        raise ActivationRejected("descriptor-relative open is unsupported")
    return _posix_open_anchored(path, final_directory=final_directory)
...
def _path_presence_once(path: Path) -> bool:
    if os.name == "nt":
        try:
            descriptor = _windows_open_anchored(path)
            ...
            message = str(error)
            if (
                "reparse point rejected" in message
                or "ordinary file required" in message
            ):
                return True
            raise
        else:
...
def _path_is_present_or_aliased(path: Path) -> bool:
    first = _path_presence_once(path)
    second = _path_presence_once(path)
    if first != second:
        raise ActivationRejected(f"path presence changed during verification: {path}")
    return first
...
def _stable(path: Path) -> bytes:
    ...
    try:
        ...
        if descriptor >= 0:
            os.close(descriptor)
    replay_descriptor = _open_anchored(path)
    try:
        replay_before = os.fstat(replay_descriptor)
        with os.fdopen(replay_descriptor, "rb", closefd=True) as replay_handle:
            replay_descriptor = -1
            replay = replay_handle.read()
            replay_after = os.fstat(replay_handle.fileno())
    except OSError as error:
        raise ActivationRejected(f"artifact replay failed: {path}") from error
    finally:
    ...
    if (
        ...
        or _metadata_identity(after_second) != _metadata_identity(replay_before)
        or _metadata_identity(replay_before) != _metadata_identity(replay_after)
    ):
    ...
    *,
    ...
    if require_sealed:
        ...
        chain = _verify_outer_chain(
            OUTER,
            expected_outer_schema=("rq2_joint_deliverability_activation_outer_v2"),
            expected_inner_schema=("rq2_joint_deliverability_activation_inner_v2"),
            expected_version=2,
...
def _expected_predecessor_rework_receipt() -> dict[str, object]:
    return {
        "schema": "rq2_joint_deliverability_activation_review_rework_v1",
        "version": 1,
        "reviewed_on": "2026-09-06",
        "review_scope": (
            "rq2_joint_deliverability_activation_successor_v1_exact_outer"
        ),
        "reviewer_role": "independent_sol_reviewer",
        "reviewer_model": "gpt-5.6-sol",
        "verdict": "REWORK",
        "reviewed_subject": {
            "outer_path": PREDECESSOR_OUTER_RELATIVE,
            "outer_sha256": PREDECESSOR_OUTER_SHA256,
            "inner_sha256": (
                "c6c057d97b8accb268705f501d513fd3d3d2fa9b1238e9ea37d64a161d774585"
            ),
        ...
        "review_conclusion": {
            ...
            "major_findings": [
                {
                    "id": "activation-v1-ancestor-directory-swap",
                    "summary": (
                        "The lstat walk and pathname open are not one "
                        "descriptor-anchored traversal, so an ancestor directory "
                        "can be replaced between them."
                    ),
                    "evidence": (
                        "A single-point fault injection replaced the ancestor "
                        "after _strict_file() and before _stable() opened the leaf; "
                        "the replacement bytes were accepted."
                    ),
                    "required_remediation": (
                        "Create a versioned successor using descriptor-anchored "
                        "component traversal and add an ancestor-directory swap "
                        "fault test."
                    ),
                }
            ],
        ...
        "broad_regression": {
            "raw_result": "714 passed, 4 skipped, 1 failed",
            "raw_failure": (
                "tests/test_rq2_joint_deliverability_execution_v1.py::"
                "test_static_candidate_and_validate_only_are_closed"
            ),
            "failure_is_historical_execution_v1_state_assertion": True,
            "failure_blocks_activation_v1_review": False,
            "current_state_result": "714 passed, 4 skipped, 1 deselected",
        },
        "effect": {
            "activation_v1_bytes_immutable": True,
            "versioned_v2_rework_permitted": True,
            "independent_activation_R3_review_passed": False,
            "activation_review_gate_closed": False,
            "formal_execution_authorized": False,
            "formal_result_exists": False,
            "paper_claim": False,
            "security_certified": False,
...
def _verify_predecessor_activation(
    config: Mapping[str, object],
) -> dict[str, str]:
    predecessor = _mapping(
        config.get("predecessor_activation"),
    ...
    expected = {
        ...
        "sealed_member_count": 22,
        "review_rework_path": PREDECESSOR_REWORK_RELATIVE,
        "review_rework_sha256": PREDECESSOR_REWORK_SHA256,
        "finding_id": "activation-v1-ancestor-directory-swap",
    }
    if predecessor != expected:
        raise ActivationRejected("activation predecessor contract drifted")
    chain = _verify_outer_chain(
        ROOT / PREDECESSOR_OUTER_RELATIVE,
        expected_outer_sha256=PREDECESSOR_OUTER_SHA256,
    ...
    if (
        chain["inner_sha256"] != expected["inner_sha256"]
        or chain["member_count"] != expected["sealed_member_count"]
    ):
        raise ActivationRejected("activation predecessor sealed identity drifted")
    rework_raw = _stable(ROOT / PREDECESSOR_REWORK_RELATIVE)
    if _sha256(rework_raw) != PREDECESSOR_REWORK_SHA256:
        raise ActivationRejected("activation predecessor REWORK SHA-256 drifted")
    if _json_bytes(rework_raw, PREDECESSOR_REWORK_RELATIVE) != (
        _expected_predecessor_rework_receipt()
    ):
        raise ActivationRejected("activation predecessor REWORK contract drifted")
    return {
        "activation_v1_outer_sha256": PREDECESSOR_OUTER_SHA256,
        "activation_v1_rework_sha256": PREDECESSOR_REWORK_SHA256,
    }
...
def _python_closure_inventory(
    ...
    if (
        ...
        or closure.get("artifact_open_policy")
        != "descriptor_or_handle_anchored_component_traversal_with_replay"
    ...
    return _canonical_json_bytes(
        {
            ...
            "predecessor_activation": dict(predecessor),
...
def _decode_parent_envelope(
    ...
    if set(envelope) != {
        ...
        "rq2_joint_deliverability_activation_parent_envelope_v2"
    ):
    ...
    predecessor_raw = _mapping(
        envelope.get("predecessor_activation"),
        "parent activation predecessor",
    )
    if set(predecessor_raw) != {
        "activation_v1_outer_sha256",
        "activation_v1_rework_sha256",
    } or not all(_is_digest(value) for value in predecessor_raw.values()):
        raise ActivationRejected("parent activation predecessor drifted")
    predecessor = {key: str(value) for key, value in predecessor_raw.items()}
...
def _internal_probe(expected_pycache_prefix: str | None) -> dict[str, object]:
    ...
    if _verify_predecessor_activation(config) != predecessor:
        raise ActivationRejected("child activation predecessor differs from parent")
    ...
    try:
        from experiments import (
            run_rq2_joint_deliverability_activation_v2 as controller,
        )
...
def _run_fresh_probe(
    *,
    ...
    if (
        payload.get("schema") != "rq2_joint_deliverability_activation_fresh_probe_v2"
        ...
        != "rq2_joint_deliverability_activation_import_validation_v2"
...
def validate_only() -> dict[str, object]:
    ...
    predecessor = _verify_predecessor_activation(config)
    ...
    predecessor_after = _verify_predecessor_activation(config_after)
    ...
    if (
        ...
        or predecessor_after != predecessor
    ...
    return {
        "schema": "rq2_joint_deliverability_activation_validation_v2",
...
def execute() -> dict[str, object]:
    raise ActivationRejected(
        "activation v2 is review-only: sealed execution v3 keeps grid, runtime, "
