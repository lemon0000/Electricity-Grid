...
def validate_only() -> dict[str, Any]:
    ...
    return {
        "schema": "rq2_public_grid_highs_formal_activation_validation_v4",
        ...
        "user_formal_run_authority_present": static[
            "user_formal_run_authority_present"
        ],
...
def _require_execution_gates() -> tuple[dict[str, Any], dict[str, Any]]:
    # Presence is checked first so this design-authorization turn cannot be
    # mistaken for the distinct future formal-run authorization.
...
def _supervise_controller_after_acceptance(
    ...
    *,
    ...
    """Remain the writable observer until exact controller termination."""
    settings = contract.load_config()["startup_handshake"]
    ...
    if process.returncode != 0:
        raise contract.FormalActivationRejected(
            f"controller exited after acceptance: {process.returncode}"
        )
    if not terminal_success_path.is_file() or terminal_success_path.is_symlink():
        raise contract.FormalActivationRejected(
            "controller exited zero without terminal success evidence"
        )
    terminal = contract._mapping(
        json.loads(contract._read_stable(terminal_success_path)),
        "controller terminal success",
    )
    unresolved = terminal_success_path.parent / str(
        settings["post_release_unresolved_filename"]
    )
    if (
        terminal.get("schema")
        != "rq2_public_grid_highs_formal_controller_terminal_success_v4"
        or terminal.get("version") != 4
        or terminal.get("controller_identity") != dict(controller_identity)
...
def _spawn_controller(
    ...
    *,
    ...
    try:
        ...
        if (
            release_persisted
            and release_acceptance is None
            and release_value is not None
            and controller_identity is not None
            and paths["science_release_accepted"].is_file()
        ):
            try:
                closure = contract.verify_execution_closure()
                release_acceptance = contract.validate_science_release_acceptance(
                    json.loads(
                        contract._read_stable(paths["science_release_accepted"])
                    ),
            ...
            except Exception:  # noqa: BLE001 - evidence remains explicitly unproven
        ...
        if release_persisted:
            _record_post_release_unresolved(
                attempt_root,
        ...
        raise
    spawn = {
        "schema": "rq2_public_grid_highs_formal_activation_spawn_receipt_v4",
        "version": 4,
    ...
    try:
        ...
        terminal = _supervise_controller_after_acceptance(
            ...
            release_path=paths["science_release"],
            release_acceptance=release_acceptance,
            terminal_success_path=paths["controller_terminal_success"],
        )
    except BaseException as exc:
        termination = (
        ...
        raise
    return {**spawn, "controller_terminal_success": terminal}
