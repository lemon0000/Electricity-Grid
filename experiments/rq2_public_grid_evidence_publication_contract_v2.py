"""Secret-free public contract for closed evidence/publication successor v2."""
...
def load_config() -> dict[str, Any]:
    try:
        ...
        raise ContractRejected("v2 config JSON malformed") from exc
    if not isinstance(value, dict) or value.get("status") != (
        "evidence_publication_successor_v2_review_closed"
    ):
        raise ContractRejected("v2 config identity drifted")
    if tuple(value.get("blocks", ())) != BLOCKS:
        raise ContractRejected("v2 block order drifted")
    gates = value.get("gates")
    if not isinstance(gates, dict) or any(gates.values()):
        raise ContractRejected("v2 gates are not all closed")
    expected_truth = [
        {
        ...
        {
            "condition": "exact_v2_result_and_exact_bound_success_and_terminal_clean_absent",
    ...
    if value.get("publication_truth_table") != expected_truth:
        raise ContractRejected("v2 publication truth table drifted")
    return value
...
def verify_full_live_closure() -> dict[str, str]:
    """Return the complete canonical predecessor-plus-self path/hash mapping."""
    config = load_config()
    mapping: dict[str, str] = {}
    predecessor = config["predecessor"]
    for label in ("outer", "inner", "rework"):
        _add(mapping, predecessor[f"{label}_path"], predecessor[f"{label}_sha256"])

    try:
        predecessor_inner = json.loads(
            read_stable(ROOT / predecessor["inner_path"])
        )
    except json.JSONDecodeError as exc:
        raise ContractRejected("v1 inner manifest malformed") from exc
    predecessor_files = predecessor_inner.get("files")
    if not isinstance(predecessor_files, dict) or len(predecessor_files) != 8:
        raise ContractRejected("v1 exact member inventory drifted")
    for relative, expected in predecessor_files.items():
        if not isinstance(relative, str) or not isinstance(expected, str):
            raise ContractRejected("v1 member binding malformed")
        _add(mapping, relative, expected)

    try:
        from experiments import (
            rq2_public_grid_evidence_publication_contract_v1 as prior,
        )

        prior_paths = prior.verify_full_live_closure()
    except Exception as exc:  # fail closed across the immutable predecessor
        raise ContractRejected("v1 recursive closure rejected") from exc
    for relative in prior_paths:
        raw = read_stable(ROOT / relative)
        observed = sha256_bytes(raw)
        previous = mapping.setdefault(relative, observed)
        if previous != observed:
            raise ContractRejected(f"v1 recursive binding drifted: {relative}")
    ...
    try:
        ...
        raise ContractRejected("v2 manifests unavailable") from exc
    ...
    if outer != {
        ...
        raise ContractRejected("v2 outer binding drifted")
    own_files = inner.get("files")
    ...
    if (
        ...
        raise ContractRejected("v2 inner inventory drifted")
    for relative, expected in own_files.items():
        if not isinstance(relative, str) or not isinstance(expected, str):
            raise ContractRejected("v2 member binding malformed")
        _add(mapping, relative, expected)
    mapping[inner_relative] = sha256_bytes(inner_raw)
    mapping[outer_relative] = sha256_bytes(outer_raw)
