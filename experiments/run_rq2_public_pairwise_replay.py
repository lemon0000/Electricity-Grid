...
def _preflight(config_path: Path) -> tuple[dict[str, Any], dict[str, object]]:
    ...
    report = {
        "schema": "rq2_public_pairwise_replay_preflight_v1",
...
def run(
    ...
    *,
    ...
    scenario_sha = _sha256(_ROOT / "src/scenarios/rq2_public_replay.py")
    identity = {
        "config_sha256": config_sha,
        "implementation_sha256": implementation_sha,
        "scenario_module_sha256": scenario_sha,
        "capacity_module_sha256": _sha256(
            _ROOT / "src/models/temporal_flexibility_capacity.py"
        ),
        "temporal_model_sha256": _sha256(
            _ROOT / "src/models/economic_temporal_stochastic.py"
        ),
        "flexibility_envelope_sha256": _sha256(
    ...
    for cell in cells:
        ...
        if checkpoint is None:
            ...
            checkpoint = {
                "schema": "rq2_public_policy_checkpoint_v1",
                **identity,
                "cell_id": cell.cell_id,
                "varied_dimension": cell.varied_dimension,
    ...
    for policy in policy_checkpoints:
        ...
        for power in power_holdout:
            for workload in workload_holdout:
                ...
                checkpoint = _load_checkpoint(path, pair_identity)
                if checkpoint is None:
                    ...
                    checkpoint = {
                        "schema": "rq2_public_pair_checkpoint_v1",
                        **pair_identity,
                        **payload,
                    }
                    _write_checkpoint(path, checkpoint)
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/rq2_public_pairwise_replay_v1.yaml"),
    )
