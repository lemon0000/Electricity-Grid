"""Validate or execute the reviewed RQ2 confirmatory pilot v4 successor."""
...
from experiments.run_rq2_public_solver_confirmatory_pilot_v3 import (
    _load_scientific_parent as _load_v3_scientific_parent,
)
...
CONFIG = ROOT / "configs/rq2_public_solver_confirmatory_pilot_v4.yaml"
BUNDLE = (
    ROOT / "configs/rq2_public_solver_confirmatory_pilot_bundle_v4.SHA256SUMS.json"
)
V3_CONFIG = ROOT / "configs/rq2_public_solver_confirmatory_pilot_v3.yaml"
SEMANTIC_CONFIG = (
    ROOT / "configs/rq2_public_solver_pilot_semantic_successor_v1.yaml"
)
MODULE = "experiments.run_rq2_public_solver_confirmatory_pilot_v4"
CONFIG_SCHEMA = "rq2_public_solver_confirmatory_pilot_config_v4"
CONTROLLER_SCHEMA = "rq2_public_solver_confirmatory_controller_receipt_v4"
PAYLOAD_SCHEMA = "rq2_public_solver_confirmatory_worker_payload_v4"
WORKER_REPORT_SCHEMA = "rq2_public_solver_confirmatory_worker_report_v4"
WORKER_RECEIPT_SCHEMA = "rq2_public_solver_confirmatory_worker_receipt_v4"
RESULT_SCHEMA = "rq2_public_solver_confirmatory_pilot_v4"
SEMANTIC_VALIDATION_SCHEMA = (
    "rq2_public_solver_confirmatory_semantic_validation_v4"
)
V3_OUTER_SHA256 = (
    "d7b8b7dd2cf0bc51f46602c1c7e28c1aad281c977ff8a15b0617be392e4a2f49"
)


def _bundle_files() -> dict[str, str]:
    bundle = _mapping(_load_json_strict(BUNDLE, "v4 bundle"), "v4 bundle")
    return _mapping(bundle.get("files"), "v4 bundle files")


def _load_config(config_path: Path = CONFIG) -> dict[str, Any]:
    if config_path.resolve() != CONFIG.resolve():
        raise ValueError("only the canonical v4 config is accepted")
    payload = _mapping(
        yaml.safe_load(config_path.read_text(encoding="utf-8")), "v4 config"
    )
    if payload.get("schema") != CONFIG_SCHEMA:
        raise ValueError("v4 config schema drifted")
    files = _bundle_files()
    for relative, path in (
        ("configs/rq2_public_solver_confirmatory_pilot_v4.yaml", config_path),
        ("experiments/run_rq2_public_solver_confirmatory_pilot_v4.py", Path(__file__)),
    ):
        if files.get(relative) != _sha256(path):
            raise ValueError(f"v4 runtime authority drifted: {relative}")
    return payload


def _load_scientific_parent(config: Mapping[str, Any]) -> dict[str, Any]:
    authority = _mapping(config.get("v3_predecessor_authority"), "v3 authority")
    registered = _mapping(authority.get("config"), "v3 config authority")
    if (
        registered.get("path")
        != "configs/rq2_public_solver_confirmatory_pilot_v3.yaml"
        or registered.get("sha256")
        != "3f7a1fd1e93ec46a608e8cd164abb685365fd04d4575a7f8890f0832791415ca"
        or not V3_CONFIG.is_file()
        or V3_CONFIG.is_symlink()
        or _sha256(V3_CONFIG) != registered["sha256"]
    ):
        raise ValueError("v3 scientific predecessor authority drifted")
    predecessor = _mapping(
        yaml.safe_load(V3_CONFIG.read_text(encoding="utf-8")), "v3 config"
    )
    if predecessor.get("schema") != "rq2_public_solver_confirmatory_pilot_config_v3":
        raise ValueError("v3 scientific predecessor schema drifted")
    return _load_v3_scientific_parent(predecessor)


def _require_execution_gate(config: Mapping[str, Any]) -> None:
    gates = _mapping(config.get("gates"), "v4 gates")
    if (
        gates.get("independent_v3_implementation_review_passed") is not True
        or gates.get("v4_execution_successor_present") is not True
        or gates.get("independent_v4_implementation_review_passed") is not True
        or gates.get("confirmatory_execution_ready") is not True
    ):
        raise RuntimeError("v4 confirmatory execution gate is closed")
    for key in (
        ...
        if gates.get(key) is not False:
            raise RuntimeError(f"v4 forbidden downstream gate opened: {key}")
...
def _run_identity(run_id: str) -> tuple[str, int]:
    try:
        ...
        raise ValueError(f"unregistered v4 run_id: {run_id}") from error
...
def _expected_payload_fields(
    ...
    return {
        ...
        "v3_outer_sha256": controller["v3_outer_sha256"],
...
def build_expected_worker_report(
    *,
    ...
    """Reconstruct the sole report authority from non-nested observations."""
    solver, repetition = _run_identity(run_id)
    if (
...
def _execute_worker(
    ...
    *,
    ...
    from experiments.run_rq2_public_solver_pilot_v1 import _preflight as _v1_preflight
    from experiments.run_rq2_public_solver_pilot_v1 import _run_block
    from experiments.validate_rq2_public_solver_confirmatory_pilot_v4 import validate
    ...
    if (
        ...
        or _execution_index(run_id) != expected_execution_index
    ):
        raise ValueError("worker controller identity or execution index drifted")
...
def _validate_worker_directory(
    ...
    *,
    ...
    issued = receipt.get("receipt_issued_ns")
    if (
        isinstance(issued, bool)
        or not isinstance(issued, int)
        or issued <= minimum_receipt_issued_ns
    ):
    ...
    if _mapping(receipt.get("worker_report"), "stored worker report") != expected_report:
    ...
    expected_receipt = {
        ...
        "worker_report_sha256": _sha256_bytes(_canonical_json_bytes(expected_report)),
        "controller_receipt_sequence": payload["execution_index"],
        "receipt_issued_ns": issued,
    ...
    return run, worker_pid, _sha256(receipt_path), issued
...
def _publish_worker_receipt(
    ...
    *,
    ...
    payload_path = worker_root / "payload.json"
    payload = _mapping(_load_json_strict(payload_path, "payload"), "payload")
    expected_report = build_expected_worker_report(
        run_id=run_id,
        payload=payload,
        controller=controller,
...
def _validate_controller(
    ...
    if (
        ...
        or receipt.get("schema") != CONTROLLER_SCHEMA
        ...
        or receipt.get("v3_outer_sha256") != V3_OUTER_SHA256
...
def _reconstruct_runs(
    ...
    *,
    ...
    for run_id in EXPECTED_RUNS:
        run, pid, receipt_sha, issued = _validate_worker_directory(
            workers / run_id,
            run_id,
            controller,
            load_json=load_json,
...
def _python_authority(scientific: Mapping[str, Any]) -> Path:
    authority = _mapping(scientific["execution"]["python_authority"], "Python authority")
...
def run(
    ...
    if target.exists():
        raise FileExistsError(f"refusing to overwrite v4 output: {target}")
    _require_execution_gate(config)
    require_execution_host(scientific["execution"])
    ...
    try:
        ...
        if target.exists():
            raise FileExistsError("v4 output appeared before atomic publication")
        ...
        try:
            ...
            raise RuntimeError(
                f"v4 staging cleanup could not be confirmed: {staging}"
            ) from cleanup_error
        raise
    return summary
