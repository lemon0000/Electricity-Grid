...
CONFIG = ROOT / "configs/rq2_public_grid_highs_formal_activation_successor_v3.json"
FORMAL_CONFIG = (
    ROOT
    / "configs/rts_gmlc_public_grid_need_dispatch_v4_highs_process_isolated_formal_v4.yaml"
)
INNER = (
    ROOT
    / "configs/rq2_public_grid_highs_formal_activation_successor_v3.SHA256SUMS.json"
)
OUTER = (
    ROOT
    / "configs/rq2_public_grid_highs_formal_activation_successor_v3.OUTER.SHA256SUMS.json"
)
CLOSURE = (
    ROOT
    / "configs/rq2_public_grid_highs_formal_activation_successor_v3.EXECUTION_CLOSURE.SHA256SUMS.json"
)
POST_RESULT_PASS = (
    ROOT
...
ACTIVATION_REVIEW_PASS = (
    ROOT / "configs/rq2_public_grid_highs_formal_activation_successor_review_pass_v3.json"
)
USER_FORMAL_RUN_AUTHORITY = (
    ROOT / "configs/rq2_public_grid_highs_formal_run_authority_v3.json"
)
PREDECESSOR_REWORK = (
    ROOT / "configs/rq2_public_grid_highs_formal_activation_successor_review_rework_v1.json"
)
PREDECESSOR_ESCALATE = (
    ROOT / "configs/rq2_public_grid_highs_formal_activation_successor_review_escalate_v2.json"
)
GIB = 1024**3
...
def build_startup_handshake(
    *,
    ...
    return {
        "schema": "rq2_public_grid_highs_formal_controller_startup_handshake_v3",
        "version": 3,
...
def validate_startup_handshake(
    ...
    *,
    ...
    if (
        ...
        != "rq2_public_grid_highs_formal_controller_startup_handshake_v3"
        or handshake.get("version") != 3
...
def build_startup_ack(
    *,
    ...
    return {
        "schema": "rq2_public_grid_highs_formal_bootstrap_startup_ack_v3",
        "version": 3,
        "wall_time_ns": time.time_ns(),
        "monotonic_ns": time.monotonic_ns(),
...
def validate_startup_ack(
    ...
    *,
    ...
    if (
        ...
        or ack.get("schema")
        != "rq2_public_grid_highs_formal_bootstrap_startup_ack_v3"
        or ack.get("version") != 3
...
def build_startup_ready(
    *,
    ...
    return {
        "schema": "rq2_public_grid_highs_formal_controller_startup_ready_v3",
        "version": 3,
        "wall_time_ns": time.time_ns(),
        "monotonic_ns": time.monotonic_ns(),
        "handshake_sha256": canonical_sha256(handshake),
...
def build_science_release(
    *,
    ...
    return {
        "schema": "rq2_public_grid_highs_formal_bootstrap_science_release_v3",
        "version": 3,
        "wall_time_ns": time.time_ns(),
        "monotonic_ns": time.monotonic_ns(),
...
def build_science_release_acceptance(
    *,
    ...
    return {
        "schema": "rq2_public_grid_highs_formal_controller_science_release_accepted_v3",
        "version": 3,
        "wall_time_ns": time.time_ns(),
        "monotonic_ns": time.monotonic_ns(),
...
def load_config() -> dict[str, Any]:
    ...
    if (
        config.get("schema")
        != "rq2_public_grid_highs_formal_activation_successor_v3"
...
def _verify_sealed_bundle() -> None:
    ...
    if (
        inner.get("schema")
        != "rq2_public_grid_highs_formal_activation_successor_v3_inner"
        or inner.get("version") != 3
        or outer
        != {
            "schema": (
                "rq2_public_grid_highs_formal_activation_successor_v3_outer"
            ),
            "version": 3,
            "inner": {
...
def _verify_fresh_one_shot_authority() -> None:
    ...
    if (
        consumed.exists()
        or value.get("schema")
        != "rq2_public_grid_highs_formal_activation_successor_v3_one_shot_authority"
...
def _local_import_candidates(tree: ast.AST) -> set[Path]:
    candidates: set[Path] = set()

    def add_package_initializers(base: Path) -> None:
        parts = base.parts
    ...
    for node in ast.walk(tree):
        ...
        for module, names in imports:
            ...
            add_package_initializers(base.parent)
            for candidate in (base.with_suffix(".py"), base / "__init__.py"):
                if (ROOT / candidate).is_file():
                    candidates.add(candidate)
            for name in names:
                for candidate in (
                    (base / name).with_suffix(".py"),
...
def discover_execution_import_closure() -> set[str]:
    pending = [
        Path("experiments/bootstrap_rq2_public_grid_highs_formal_activation_successor_v3.py"),
        Path("experiments/rq2_public_grid_highs_formal_activation_contract_v3.py"),
        Path("experiments/run_rts_gmlc_public_grid_need_dispatch_v4_process_isolated_formal_v4.py"),
    ]
    ...
    while pending:
        ...
        pending.extend(_local_import_candidates(tree) - observed)
...
def verify_execution_closure() -> dict[str, Any]:
    ...
    if (
        manifest.get("schema")
        != "rq2_public_grid_highs_formal_activation_successor_v3_execution_closure"
        or manifest.get("version") != 3
        or manifest.get("derivation")
        != "recursive_local_import_superset_with_package_initializers"
...
def _verify_predecessor_escalate(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    v2_outer = _repo_path(binding["v2_outer_path"], "v2 outer")
    if (
        binding.get("v2_outer_sha256")
        != "8c5db5e265141378b537e9e3096198c0f86221a324423a646d6645d107b56764"
        or sha256_file(v2_outer) != binding.get("v2_outer_sha256")
...
def _verify_formal_science_config(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        formal.get("schema")
        != "rts_gmlc_public_grid_need_dispatch_v4_highs_process_isolated_formal_v4"
...
def validate_static_authority(
    *, require_activation_review: bool = False
    ...
    return {
        "schema": "rq2_public_grid_highs_formal_activation_static_validation_v3",
...
def capture_preflight_evidence(
    ...
    *,
    ...
    payload = {
        "schema": "rq2_public_grid_highs_formal_activation_preflight_v3",
        "version": 3,
        "wall_time_ns": wall,
...
def publish_dynamic_authority(
    ...
    *,
    ...
    payload = {
        "schema": "rq2_public_grid_highs_formal_dynamic_activation_authority_v3",
        "version": 3,
        "preflight": {
    ...
    receipt_payload = {
        "schema": "rq2_public_grid_highs_formal_activation_attempt_receipt_v3",
        "version": 3,
...
def validate_dynamic_authority(path: Path) -> dict[str, Any]:
    ...
    if (
        value.get("schema")
        != "rq2_public_grid_highs_formal_dynamic_activation_authority_v3"
...
def consume_one_shot_authority(dynamic: Mapping[str, Any]) -> dict[str, Any]:
    ...
    tombstone = {
        "schema": "rq2_public_grid_highs_formal_activation_successor_v3_consumed_authority",
        "version": 3,
