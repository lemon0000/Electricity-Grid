"""Relaxed-gap successor for repair-006: raises maximum_accepted_relative_gap to 0.12% so direct cost certificate passes."""
...
from experiments import (
    record_rts_gmlc_zero_dc_ac_aware_commitment_v4_repair_006_failure as failure_record,
)
from experiments import (
    run_rts_gmlc_zero_dc_ac_aware_commitment_v4_repair_006_formal as repair006,
)
from experiments import (
...
DEFAULT_CONFIG_PATH = Path(
    "configs/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4_repair_007.yaml"
)
CHECKPOINT_SCHEMA = "rts_gmlc_v4_repair_007_cost_bisection_candidate_checkpoint_v1"
PREFIX_EVIDENCE_SCHEMA = "rts_gmlc_v4_repair_007_prefix_import_evidence_v1"
HYBRID_EVIDENCE_SCHEMA = "rts_gmlc_v4_repair_007_hybrid_cost_evidence_v1"
COMBINED_COST_SCHEMA = "rts_gmlc_v4_repair_007_combined_cost_certificate_v1"
FRONTIER_SCHEMA = "rts_gmlc_v4_repair_007_candidate_frontier_v1"
FLOAT_SERIALIZATION = "python_json_roundtrip_full_precision_v1"
_SUCCESSOR_MODULE = (
    "experiments.run_rts_gmlc_zero_dc_ac_aware_commitment_v4_repair_007_formal"
)
_CONFIG_KEYS = {
    "schema",
    "preregistration",
    "base_v4",
    "predecessor_repair_006",
    "formal_successor",
    "implementation",
    "output",
    "logging",
...
def _source_context(config: Mapping[str, Any]) -> v4._FrontierContext:
    return repair006._build_context(
        Path(config["predecessor_repair_006"]["config_path"])
    )
...
def _verified_source_prefix(
    ...
    for ordinal in range(1, 5):
        item = repair006._load_candidate_checkpoint(source_context, root, ordinal)
        if item is None:
            raise RuntimeError("repair-007 source prefix checkpoint is missing")
        ...
        if (
            ...
            raise RuntimeError("repair-007 source prefix hash drifted")
...
def _verify_frozen_inputs(
    ...
    if len(_verified_source_prefix(config)) != 4:
        raise RuntimeError("repair-007 source prefix count drifted")
...
def _build_context(
    ...
    contract = {
        "schema": "rts_gmlc_v4_repair_007_formal_inputs_v1",
        ...
        "predecessor_repair_006": successor_config["predecessor_repair_006"],
    ...
    *,
    ...
    if target.exists():
        ...
        if (
            ...
            raise RuntimeError("repair-007 published preregistration drifted")
        return observed
    if output_root.exists() and any(output_root.iterdir()):
        raise RuntimeError("Cannot prepare repair-007 beside existing artifacts")
...
def _prefix_candidate(
    ...
    predecessor = context.input_contract["predecessor_repair_006"]
    evidence: dict[str, object] = {
        ...
        "source_checkpoint_validated_by_repair_006_runner": True,
    ...
    imported = replace(
        candidate,
        source="repair_007_verified_repair_006_prefix",
...
def _hybrid_candidate(
    ...
    *,
    ...
    predecessor_evidence_sha = common_input_signature_sha256(
        {
            ...
            "repair_006_proxy_successor_contract_sha256": (
                context.input_contract["predecessor_repair_006"][
        ...
        return run_exact_cg_stage(
            ...
            time_limits=repair004.ExactCgTimeLimits(
                master_seconds=float(
                    context.input_contract["predecessor_repair_006"].get(
    ...
    parent_successor = repair006._read_config(
        Path(context.input_contract["predecessor_repair_006"]["config_path"])
    )["formal_successor"]
    ...
    proxy_evidence: dict[str, object] = {
        "schema": repair006.HYBRID_EVIDENCE_SCHEMA,
...
def _validate_candidate_physics(
    ...
    if ordinal <= 4:
        ...
        if (
            candidate.source != "repair_007_verified_repair_006_prefix"
    ...
    repair006._validate_candidate_physics(candidate, source_context, ordinal)
...
def _validate_proxy_checkpoint_evidence(
    ...
    *,
    ...
    if (
        proxy.get("schema") != repair006.HYBRID_EVIDENCE_SCHEMA
...
def _validate_checkpoint_document(
    ...
    if ordinal <= 4:
        ...
        if (
            ...
            != predecessor["candidate_json_sha256s"][ordinal - 1]
            or evidence.get("source_checkpoint_validated_by_repair_006_runner")
            is not True
...
def _validate_round_artifacts(
    ...
    repair006._validate_round_artifacts(
        checkpoint_root,
        context,
        ordinal,
        candidate,
...
def _run_joint_ac_attempt(
    ...
    original = repair006._joint_worker_command
    repair006._joint_worker_command = _joint_worker_command
    try:
        return repair006._run_joint_ac_attempt(
            context,
            output_root,
            registration,
            candidates,
        ...
        repair006._joint_worker_command = original
