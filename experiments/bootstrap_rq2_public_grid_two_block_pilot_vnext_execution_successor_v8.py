"""Stdlib-first bootstrap for review-closed Vnext execution successor v8."""
...
class BootstrapRejected(RuntimeError):
    """The pre-project-import V8 bootstrap failed closed."""
...
def _verify_bundle() -> tuple[dict[str, object], str]:
    try:
        ...
        raise BootstrapRejected("V8 bundle JSON malformed") from exc
    ...
    if (
        ...
        raise BootstrapRejected("V8 bundle identity/inventory drifted")
    for relative, expected in inner["files"].items():
        if _sha(_stable(ROOT / relative)) != expected:
            raise BootstrapRejected(f"V8 bundle member hash drifted: {relative}")
...
def _preimport_review(config: dict[str, object], outer_sha256: str) -> None:
    if not REVIEW.exists() or REVIEW.is_symlink():
        raise BootstrapRejected("fixed V8 execution-review PASS receipt is absent")
    try:
        value = json.loads(_stable(REVIEW))
    except json.JSONDecodeError as exc:
        raise BootstrapRejected("V8 execution-review receipt JSON malformed") from exc
    authority = config["fixed_execution_review"]
    predecessor = config["predecessor_v7"]
    ...
    if (
        ...
        != {
            "path": predecessor["review_receipt_path"],
            "sha256": predecessor["review_receipt_sha256"],
        }
        ...
        raise BootstrapRejected("V8 execution-review receipt binding/effect drifted")
