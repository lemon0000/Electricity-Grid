"""Stdlib-first bootstrap for the review-closed Vnext execution successor."""
...
INNER = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v1.SHA256SUMS.json"
OUTER = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v1.OUTER.SHA256SUMS.json"
...
class BootstrapRejected(RuntimeError):
    """The pre-project-import bootstrap failed closed."""
...
def _verify_bundle() -> tuple[dict[str, object], str]:
    ...
    if (
        outer.get("schema")
        != "rq2_public_grid_two_block_pilot_vnext_execution_successor_outer_v1"
        or outer.get("files") != {str(INNER.relative_to(ROOT)).replace("\\", "/"): _sha(inner_raw)}
        or inner.get("schema")
        != "rq2_public_grid_two_block_pilot_vnext_execution_successor_bundle_v1"
        or set(inner.get("files", {})) != set(config["bundle"]["members"])
        or len(inner["files"]) != config["bundle"]["exact_member_count"]
    ):
        raise BootstrapRejected("bundle manifest identity/inventory drifted")
    for relative, expected in inner["files"].items():
        if _sha(_stable(ROOT / relative)) != expected:
            raise BootstrapRejected(f"bundle member hash drifted: {relative}")
...
def _preimport_review(config: dict[str, object], outer_sha256: str) -> None:
    if not REVIEW.exists() or REVIEW.is_symlink():
        raise BootstrapRejected("fixed execution-review PASS receipt is absent")
    try:
        value = json.loads(_stable(REVIEW))
    except json.JSONDecodeError as exc:
        raise BootstrapRejected("execution-review receipt JSON malformed") from exc
    ...
    if (
        ...
        != {"path": str(OUTER.relative_to(ROOT)).replace("\\", "/"), "sha256": outer_sha256}
        ...
        raise BootstrapRejected("execution-review receipt binding/effect drifted")
