"""Internal worker for Vnext nonformal execution successor v8."""
...
def _parse(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    modes.add_argument(
        "--internal-review-deadline-no-exit-probe-worker", action="store_true"
    )
...
def _mode(args: argparse.Namespace) -> str:
    ...
    if args.internal_review_deadline_no_exit_probe_worker:
        return "review-deadline-no-exit-probe"
    return "science"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if mode == "review-resource-probe":
            ...
            return 0
        if mode == "review-deadline-no-exit-probe":
            _release_raw, release = contract.read_frame(
                read_descriptor, "deadline-no-exit probe envelope"
            )
            contract.require_eof(read_descriptor)
            expected = {
                "schema": (
            ...
            contract.require_exact(
                release, expected, label="deadline-no-exit probe envelope"
            )
            while True:
                time.sleep(1)
        return _run_science(
