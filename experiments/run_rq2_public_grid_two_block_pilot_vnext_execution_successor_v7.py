"""Review-closed V7 evidence rerun for the fixed nonformal two-block pilot."""
...
def _persist_resource_journal(
    ...
    mapping_match = journal["resource_sampler_binding"]["authority_mapping_match"]
    return {
        "schema": (
        ...
        "controller_acceptable": mapping_match,
    }
...
def _start_resource_monitor(
    ...
    *,
    after_same_pair_observation: Any = None,
    ...
    expected_identity = {
        "pid": process.pid,
        "create_time_ns": worker_identity["create_time_ns"],
    }
    binding_start_mono = monotonic_ns()
    binding_start_wall = wall_time_ns()
    pre_monitor_mapping = contract.verify_live_authorities()
    resources = contract.resource_primitives()
    binding_completed_mono = monotonic_ns()
    binding_completed_wall = wall_time_ns()
    sampler_prebinding = {
        "schema": (
            "rq2_public_grid_resource_sampler_prebinding_vnext_execution_v7"
        ),
        "version": 1,
        "binding_start_wall_time_ns": binding_start_wall,
        "binding_start_monotonic_ns": binding_start_mono,
        "binding_completed_wall_time_ns": binding_completed_wall,
        "binding_completed_monotonic_ns": binding_completed_mono,
        "expected_owned_identity": expected_identity,
        "pre_monitor_authority_mapping_sha256": (
            contract.closure_mapping_sha256(pre_monitor_mapping)
        ),
        "sampler_scope": (
    ...
    def sample_bound_pair(pid: int, create_time_ns: int) -> tuple[int, int]:
        if {"pid": pid, "create_time_ns": create_time_ns} != expected_identity:
            raise contract.ContractRejected("resource sampler owned identity drifted")
        observed = resources.sample_resource_pair(pid, create_time_ns)
        if after_same_pair_observation is not None:
            after_same_pair_observation()
        return (
            observed.child_private_commit_bytes,
            observed.system_commit_available_bytes,
        )
    ...
    future = executor.submit(
        ...
        state=state,
        sample=sample_bound_pair,
        monotonic_ns=monotonic_ns,
        wall_time_ns=wall_time_ns,
        sleep=sleep,
        terminate=terminate,
        sampler_prebinding=sampler_prebinding,
        verify_post_authority=contract.verify_live_authorities,
    ...
    *,
    ...
    try:
        ...
        if resource_future in done:
            ...
            if persisted["controller_acceptable"] is not True:
                raise HonestIncomplete(
                    "resource authority mapping changed across monitor"
                )
        ...
        if persisted["controller_acceptable"] is not True:
            raise HonestIncomplete("resource authority mapping changed across monitor")
...
def run_review_preloader_e2e() -> dict[str, Any]:
    ...
    try:
        ...
        expected_ack = {
            "schema": "rq2_public_grid_preloader_ack_vnext_execution_v7",
...
def run_review_resource_probe_e2e() -> dict[str, Any]:
    ...
    journal_directory = tempfile.TemporaryDirectory(prefix="rq2_v7_resource_probe_")
    ...
    try:
        ...
        release = {
            "schema": "rq2_public_grid_resource_probe_release_vnext_execution_v7",
            "hello_sha256": contract.sha256_bytes(hello_raw),
        ...
        if (
            ...
            or journal["status"] != "child_exited"
            or persisted["controller_acceptable"] is not True
        ):
            raise contract.ContractRejected("resource-probe child outcome rejected")
        expected_notice = {
            "schema": "rq2_public_grid_resource_probe_notice_vnext_execution_v7",
            "hello_sha256": contract.sha256_bytes(hello_raw),
...
def run_review_deadline_no_exit_probe_e2e(
    ...
    def after_same_pair_observation() -> None:
    ...
    try:
        ...
        state, executor, resource_future, deadline = _start_resource_monitor(
            ...
            worker_identity,
            persistence_path,
            after_same_pair_observation=after_same_pair_observation,
...
def run_two_block_nonformal(
    *,
    ...
    """Execute one new evidence run only after the fixed V7 PASS receipt."""
    ...
    if _review_lease_probe:
        ...
        test_lease = {
            "schema": "rq2_public_grid_vnext_execution_successor_v7_lamport_ots_private_lease",
            "version": 1,
            "state": "fresh",
            "key_id": auth["key_id"],
        ...
        if any(path.exists() or path.is_symlink() for path in paths.values()):
            raise contract.ContractRejected(
                "canonical V7 execution roots must be clean absent"
            )
        resources = contract.resource_primitives()
        ...
        if consumed_lease.exists() or consumed_lease.is_symlink():
            raise contract.ContractRejected("one-time V7 key was already consumed")
    ...
    def tombstone_key(state: str = "consumed") -> None:
        ...
        tombstone = {
            "schema": "rq2_public_grid_vnext_execution_successor_v7_lamport_ots_consumed_tombstone",
            "version": 1,
            "state": state,
            "key_id": auth["key_id"],
            "public_key_sha256": auth["public_key_sha256"],
    ...
    def acquire_private_lease() -> bytearray:
        ...
        try:
            ...
            if (
                ...
                or value.get("schema")
                != "rq2_public_grid_vnext_execution_successor_v7_lamport_ots_private_lease"
    ...
    if _review_lease_probe:
        ...
        try:
            ...
            digest = contract.sha256_bytes(b"rq2-v7-review-lease-probe")
    ...
    def dispatch(index: int) -> None:
        ...
        try:
            ...
            body = {
                "schema": "rq2_public_grid_accepted_evidence_vnext_execution_v7",
                "session_id": session_id,
                "execution_index": index,
                "block_id": block_id,
                "predecessor_digest": predecessor_digest,
