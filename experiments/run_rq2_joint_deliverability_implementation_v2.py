"""Versioned implementation-only runner for the sealed RQ2 V5 protocol.
...
from experiments.validate_rq2_joint_deliverability_implementation_v2 import (
...
def validate_runtime_config(config: Mapping[str, object]) -> dict[str, object]:
    ...
    if config.get("schema") != "rq2_joint_deliverability_implementation_successor_v2":
...
def run(*, validate_only: bool = True) -> dict[str, object]:
    ...
    if not validate_only:
        raise RuntimeError(
            "formal execution is not authorized by implementation successor v2"
        )
    return {
