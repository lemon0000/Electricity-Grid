"""Relaxed-gap successor for repair-007: raises maximum_accepted_relative_gap to 0.15% so direct cost certificate (gap=0.126%) passes directly."""
...
DEFAULT_CONFIG_PATH = Path(
    "configs/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4_repair_008.yaml"
)
CHECKPOINT_SCHEMA = "rts_gmlc_v4_repair_008_cost_bisection_candidate_checkpoint_v1"
PREFIX_EVIDENCE_SCHEMA = "rts_gmlc_v4_repair_008_prefix_import_evidence_v1"
HYBRID_EVIDENCE_SCHEMA = "rts_gmlc_v4_repair_008_hybrid_cost_evidence_v1"
COMBINED_COST_SCHEMA = "rts_gmlc_v4_repair_008_combined_cost_certificate_v1"
FRONTIER_SCHEMA = "rts_gmlc_v4_repair_008_candidate_frontier_v1"
FLOAT_SERIALIZATION = "python_json_roundtrip_full_precision_v1"
_SUCCESSOR_MODULE = (
    "experiments.run_rts_gmlc_zero_dc_ac_aware_commitment_v4_repair_008_formal"
)
_CONFIG_KEYS = {
    "schema",
    "preregistration",
...
def _read_config(config_path: Path) -> dict[str, Any]:
    ...
    if (
        config.get("schema") != "rts_gmlc_v4_repair_008_formal_successor_config_v1"
        or preregistration.get("id")
        != "rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4_repair_008"
        or preregistration.get("schema")
        != "rts_gmlc_zero_dc_ac_aware_commitment_preregistration_v4_repair_008"
        ...
        raise ValueError("repair-008 frozen contract drifted")
...
def _verified_source_prefix(
    ...
    for ordinal in range(1, 5):
        ...
        if item is None:
            raise RuntimeError("repair-008 source prefix checkpoint is missing")
        ...
        if (
            ...
            raise RuntimeError("repair-008 source prefix hash drifted")
...
def _verify_frozen_inputs(
    ...
    if len(_verified_source_prefix(config)) != 4:
        raise RuntimeError("repair-008 source prefix count drifted")
...
def _build_context(
    ...
    contract = {
        "schema": "rts_gmlc_v4_repair_008_formal_inputs_v1",
    ...
    *,
    ...
    if target.exists():
        ...
        if (
            ...
            raise RuntimeError("repair-008 published preregistration drifted")
        return observed
    if output_root.exists() and any(output_root.iterdir()):
        raise RuntimeError("Cannot prepare repair-008 beside existing artifacts")
...
def _prefix_candidate(
    ...
    evidence: dict[str, object] = {
        ...
        "source_checkpoint_validated_by_repair_007_runner": True,
    ...
    imported = replace(
        candidate,
        source="repair_008_verified_repair_007_prefix",
    ...
    predecessor_evidence_sha = common_input_signature_sha256(
        {
            ...
            "repair_007_proxy_successor_contract_sha256": (
...
def _validate_candidate_physics(
    ...
    if ordinal <= 4:
        ...
        if (
            candidate.source != "repair_008_verified_repair_007_prefix"
...
def _validate_checkpoint_document(
    ...
    if ordinal <= 4:
        ...
        if (
            ...
            != predecessor["candidate_json_sha256s"][ordinal - 1]
            or evidence.get("source_checkpoint_validated_by_repair_007_runner")
            is not True
