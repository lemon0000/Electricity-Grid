"""V6 lifecycle controller with the full block-zero V4 production path."""
...
from experiments import (
    rq2_public_grid_highs_formal_activation_contract_v6 as authority,
)
from experiments import (
...
REQUEST_SCHEMA = "rq2_grid_need_process_isolated_worker_request_formal_v7"
RELEASE_SCHEMA = "rq2_grid_need_process_isolated_worker_release_formal_v7"
...
def _verify_execution_boundary(label: str) -> dict[str, Any]:
    ...
    preseal = (
        os.environ.get("RQ2_V6_PRESEAL_CLOSURE") is not None
...
def _require_consumed_controller_authority(dynamic_authority: Path) -> dict[str, Any]:
    ...
    expected_schema = (
        ...
        else "rq2_public_grid_highs_formal_activation_v6_consumed_authority"
    )
    if (
...
def _dispatch_one(
    ...
    *,
    ...
    controller_receipt = {
        "schema": "rq2_grid_need_process_isolated_attempt_formal_v6",
        "block_id": block_id,
        "request_sha256": authority.sha256_file(request_path),
...
def validate_only(config_path: Path = CONFIG) -> dict[str, Any]:
    ...
    return {
        "schema": (
            "rq2_grid_need_process_isolated_formal_preflight_v7_sealed"
            if sealed
            else "rq2_grid_need_process_isolated_formal_preflight_v7_draft"
        ),
    ...
    *,
    ...
    try:
        ...
        if preseal_startup_probe:
            ...
            result = {
                "schema": "rq2_v6_preseal_startup_probe_result",
