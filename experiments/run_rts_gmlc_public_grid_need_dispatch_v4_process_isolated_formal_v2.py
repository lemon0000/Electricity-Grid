...
from experiments import (
    rq2_public_grid_highs_formal_activation_contract_v1 as authority,
)
from experiments import (
...
MODULE = (
    "experiments."
    "run_rts_gmlc_public_grid_need_dispatch_v4_process_isolated_formal_v2"
)
REQUEST_SCHEMA = "rq2_grid_need_process_isolated_worker_request_formal_v2"
RELEASE_SCHEMA = "rq2_grid_need_process_isolated_worker_release_formal_v2"
...
def _require_consumed_controller_authority(dynamic_authority: Path) -> dict[str, Any]:
    ...
    if (
        ...
        != "rq2_public_grid_highs_formal_activation_successor_v1_consumed_authority"
...
def _dispatch_one(
    ...
    *,
    ...
    controller_receipt = {
        "schema": "rq2_grid_need_process_isolated_attempt_formal_v2",
        "block_id": block_id,
        "request_sha256": authority.sha256_file(request_path),
...
def validate_only(config_path: Path = CONFIG) -> dict[str, Any]:
    ...
    return {
        "schema": "rq2_grid_need_process_isolated_formal_preflight_v2",
...
def run(config_path: Path, dynamic_authority: Path) -> dict[str, object]:
    if config_path.resolve() != CONFIG.resolve():
        raise authority.FormalActivationRejected("only canonical formal config is accepted")
...
def main() -> None:
    ...
    if args.worker_request is not None:
        if args.validate_only or args.activation_authority is not None:
            parser.error("worker mode accepts only --worker-request")
        raise SystemExit(_worker(args.worker_request))
    if args.validate_only:
        if args.activation_authority is not None:
            parser.error("validate-only does not accept activation authority")
        print(json.dumps(validate_only(args.config), indent=2, sort_keys=True))
        return
    if args.activation_authority is None:
        parser.error("formal controller requires --activation-authority")
    print(json.dumps(run(args.config, args.activation_authority), indent=2, sort_keys=True))
