"""Stdlib-first bootstrap for RQ2 joint-deliverability activation v3."""
...
CONFIG_RELATIVE = "configs/rq2_joint_deliverability_activation_successor_v3.json"
INNER_RELATIVE = (
    "configs/rq2_joint_deliverability_activation_successor_v3.SHA256SUMS.json"
)
OUTER_RELATIVE = (
    "configs/rq2_joint_deliverability_activation_successor_v3.OUTER.SHA256SUMS.json"
)
CONFIG = ROOT / CONFIG_RELATIVE
INNER = ROOT / INNER_RELATIVE
OUTER = ROOT / OUTER_RELATIVE
BOOTSTRAP_RELATIVE = "experiments/bootstrap_rq2_joint_deliverability_activation_v3.py"
CONTROLLER_RELATIVE = "experiments/run_rq2_joint_deliverability_activation_v3.py"
EXECUTION_CORE_RELATIVE = "src/rq2_joint_deliverability_execution_v3/core.py"
ACTIVATION_REVIEW_RELATIVE = (
    "configs/rq2_joint_deliverability_activation_review_pass_v3.json"
)
ACTIVATION_REVIEW_SCHEMA = "rq2_joint_deliverability_activation_review_pass_v3"
ACTIVATION_REVIEW_SCOPE = "rq2_joint_deliverability_activation_successor_v3_exact_outer"
...
PREDECESSOR_OUTER_SHA256 = (
    "34e35c91a5af25903582c21b6736254f2fe0cd2dea879b9a0c6858f67135b37d"
)
PREDECESSOR_ESCALATE_RELATIVE = (
    "configs/rq2_joint_deliverability_activation_review_escalate_v2.json"
)
PREDECESSOR_ESCALATE_SHA256 = (
    "5d94ca406f8e70309e000f85d1534e607241f87e1114bbf2d26e399c8d39c0da"
)
_STAGE0_SOURCE = """\
import base64
import hashlib
...
if envelope.get("schema") != "rq2_joint_deliverability_activation_parent_envelope_v3":
...
def _close_windows_handle_with_recovery(
    owned: int,
    *,
...
def _windows_open_from_root(
    ...
    *,
    ...
    try:
        reject_reparse(root_handle, str(path.anchor))
        parent = root_handle
        for index, part in enumerate(parts):
            ...
            child = open_relative(
                parent,
                part,
                not final or final_directory,
            )
            handles.append(child)
            parent = child
            reject_reparse(child, part)
        descriptor = transfer_to_descriptor(handles[-1])
        handles.pop()
    ...
    if primary_error is not None or cleanup_failures:
        ...
        if isinstance(primary_error, (OSError, ValueError)):
            raise ActivationRejected(f"handle-anchored open failed: {path}") from (
                primary_error
            )
...
def _windows_open_anchored(path: Path, *, final_directory: bool = False) -> int:
    ...
    def close_once(owned: int) -> int | None:
    ...
    return _windows_open_from_root(
        path,
        parts,
        ...
        close_handle=close_handle_with_recovery,
    )
...
def _stable(path: Path) -> bytes:
    ...
    if read_error is not None:
        if isinstance(read_error, OSError):
            raise ActivationRejected(
                f"artifact descriptor read failed: {path}"
            ) from read_error
        raise read_error
    ...
    try:
        ...
        replay = _read_all_descriptor(replay_descriptor)
...
def _load_config() -> tuple[dict[str, object], bytes]:
    ...
    if (
        config.get("schema") != "rq2_joint_deliverability_activation_successor_v3"
...
def _validate_predecessor_escalate_receipt(value: Mapping[str, object]) -> None:
    reviewed = _mapping(value.get("reviewed_subject"), "predecessor reviewed subject")
    conclusion = _mapping(
        value.get("review_conclusion"),
        "predecessor review conclusion",
    )
    effect = _mapping(value.get("effect"), "predecessor review effect")
    majors = conclusion.get("major_findings")
    if (
        set(value)
        != {
            "schema",
        ...
        or value.get("schema")
        != "rq2_joint_deliverability_activation_review_escalate_v2"
        or value.get("version") != 2
        or value.get("reviewed_on") != "2026-09-06"
        or value.get("review_scope")
        != "rq2_joint_deliverability_activation_successor_v2_exact_outer"
        or value.get("reviewer_role") != "independent_sol_reviewer"
        or value.get("reviewer_model") != "gpt-5.6-sol"
        or value.get("verdict") != "ESCALATE"
        or reviewed
        != {
            "outer_path": PREDECESSOR_OUTER_RELATIVE,
            "outer_sha256": PREDECESSOR_OUTER_SHA256,
            "inner_sha256": (
                "de024959d1557c87b43ee3eb56eb8cae10dfc879820dab7f315f2a7f08a82184"
            ),
            "sealed_member_count": 24,
        }
        or conclusion.get("blocker_findings") != []
        or not isinstance(majors, list)
        or len(majors) != 1
        or not isinstance(majors[0], Mapping)
        or majors[0].get("id") != "activation-v2-close-error-resource-ownership"
        or conclusion.get("minor_findings") != []
        or effect.get("activation_v2_bytes_immutable") is not True
        or effect.get("independent_activation_R3_review_passed") is not False
        or effect.get("activation_review_gate_closed") is not False
        ...
        raise ActivationRejected("activation predecessor ESCALATE contract drifted")
...
def _verify_predecessor_activation(
    ...
    expected = {
        ...
        "sealed_member_count": 24,
        "review_escalate_path": PREDECESSOR_ESCALATE_RELATIVE,
        "review_escalate_sha256": PREDECESSOR_ESCALATE_SHA256,
        "finding_id": "activation-v2-close-error-resource-ownership",
    }
    ...
    escalate_raw = _stable(ROOT / PREDECESSOR_ESCALATE_RELATIVE)
    if _sha256(escalate_raw) != PREDECESSOR_ESCALATE_SHA256:
        raise ActivationRejected("activation predecessor ESCALATE SHA-256 drifted")
    _validate_predecessor_escalate_receipt(
        _json_bytes(escalate_raw, PREDECESSOR_ESCALATE_RELATIVE)
    )
    return {
        "activation_v2_outer_sha256": PREDECESSOR_OUTER_SHA256,
        "activation_v2_escalate_sha256": PREDECESSOR_ESCALATE_SHA256,
    }
...
def _decode_parent_envelope(
    ...
    if set(envelope) != {
        ...
        "rq2_joint_deliverability_activation_parent_envelope_v3"
    ):
...
def _internal_probe(expected_pycache_prefix: str | None) -> dict[str, object]:
    ...
    try:
        from experiments import (
            run_rq2_joint_deliverability_activation_v3 as controller,
        )
...
def _run_fresh_probe(
    *,
    ...
    if (
        payload.get("schema") != "rq2_joint_deliverability_activation_fresh_probe_v3"
        ...
        != "rq2_joint_deliverability_activation_import_validation_v3"
...
def validate_only() -> dict[str, object]:
    ...
    return {
        "schema": "rq2_joint_deliverability_activation_validation_v3",
...
def execute() -> dict[str, object]:
    raise ActivationRejected(
        "activation v3 is review-only: sealed execution v3 keeps grid, runtime, "
