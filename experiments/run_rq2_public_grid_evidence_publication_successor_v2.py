"""Closed V2 controller with runtime-local evidence and publication authority."""
...
def run_review_fixture_e2e(
    ...
    if registered_test_case not in allowed_cases:
        raise contract.ContractRejected("unregistered V2 review test case")
    if root.exists() or root.is_symlink():
        raise contract.ContractRejected("V2 review root must be clean absent")
    ...
    def verify_record(record: dict[str, Any]) -> None:
        ...
        if record["closure_mapping"] != mapping or record[
            ...
            raise contract.ContractRejected("V2 record closure mapping drifted")
        ...
        if (
            ...
            raise contract.ContractRejected("V2 module/config identity drifted")
        ...
        if any(contract.sha256_bytes(raw) != digest for raw, digest in byte_bindings):
            raise contract.ContractRejected("V2 raw byte/hash binding drifted")
        hello_keys = {
            "schema",
            "worker_identity",
            "parent_identity",
        ...
        if (
            ...
            raise contract.ContractRejected("V2 transport/source exact keyset drifted")
        ...
        if record["command"] != list(expected_command) or hello[
            ...
            raise contract.ContractRejected("V2 exact argv drifted")
        ...
        if (
            ...
            raise contract.ContractRejected("V2 cwd/environment authority drifted")
        ...
        if (
            ...
            raise contract.ContractRejected("V2 pipe authority binding drifted")
        ...
        for label in pipe:
            ...
            if (
                ...
                raise contract.ContractRejected("V2 pipe role/type/direction drifted")
        ...
        if any(
            ...
            raise contract.ContractRejected("V2 frame/source identity drifted")
        if (
            ...
            raise contract.ContractRejected("V2 PID/PPID/create-time binding drifted")
        if (
            ...
            raise contract.ContractRejected("V2 frame closure binding drifted")
        if (
            ...
            raise contract.ContractRejected("V2 frame/source hash cross-binding drifted")
        if (
            ...
            raise contract.ContractRejected("V2 scientific byte binding drifted")
        ...
        if (
            ...
            raise contract.ContractRejected("V2 review/claim semantics drifted")
        ...
        if (
            ...
            raise contract.ContractRejected("V2 source path/bytes drifted")

    def verify_history(candidate: list[dict[str, Any]]) -> None:
        if len(candidate) not in (1, 2):
            raise contract.ContractRejected("V2 history inventory rejected")
        ...
        for index, record in enumerate(candidate, 1):
            ...
            if (
                ...
                raise contract.ContractRejected("V2 history replay/order/session rejected")
    ...
    def controller_receipt(candidate: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(candidate) != 2:
            raise contract.ContractRejected("V2 controller receipt requires two records")
        mapping = contract.verify_full_live_closure()
        body = {
            "schema": "rq2_public_grid_controller_receipt_vnext_v2",
    ...
    def verify_receipt(receipt: dict[str, Any], candidate: list[dict[str, Any]]) -> None:
        ...
        if receipt != expected or not hmac.compare_digest(
            ...
            raise contract.ContractRejected("V2 controller receipt HMAC rejected")
    ...
    def dispatch(index: int) -> None:
        ...
        if active or index in attempted or index != len(attempted) + 1:
            raise contract.ContractRejected("V2 concurrent/retry/reorder rejected")
        if index == 2 and len(records) != 1:
            raise contract.ContractRejected("V2 0009 requires accepted 0008")
        attempted.add(index)
        active = True
        from experiments import (
        ...
        try:
            ...
            expected_hello = {
                "schema": "rq2_public_grid_worker_hello_vnext_v2",
            ...
            if hello != expected_hello:
                raise contract.ContractRejected("V2 worker HELLO rejected")
            pipe_authority = {
                "worker_read": worker_read_observed,
                "worker_ack": worker_ack_observed,
            ...
            envelope = {
                "schema": "rq2_public_grid_worker_envelope_vnext_v2",
                "session_id": session_id,
            ...
            if remaining <= 0 or process.wait(timeout=remaining) != 0:
                raise contract.ContractRejected("V2 worker exit rejected")
            ...
            if not isinstance(scientific, dict):
                raise contract.ContractRejected("V2 worker scientific payload missing")
            scientific_raw = contract.exact_json_bytes(scientific)
            body = {
                "schema": "rq2_public_grid_accepted_evidence_vnext_v2",
                "session_id": session_id,
                "execution_index": index,
                "block_id": block_id,
                "predecessor_digest": (
    ...
    def exact_success(success_path: Path, receipt: dict[str, Any]) -> bool:
        try:
            ...
            if set(success) != {
                ...
                return False
            return (
                body["schema"] == "rq2_public_grid_success_commit_vnext_v2"
    ...
    def commit(candidate: list[dict[str, Any]], receipt: dict[str, Any]) -> publisher.ReviewOutcome:
        ...
        try:
            ...
            if publisher.classify_publication(
                ...
                raise contract.ContractRejected("V2 initial publication presence rejected")
            if any(
                path.exists() or path.is_symlink()
                for path in (paths.result, paths.success, paths.terminal)
            ):
                raise contract.ContractRejected("V2 publication path appearance rejected")
            for record in candidate:
                verify_record(record)
            ...
            publisher.atomic_write(
                ...
                contract.exact_json_bytes(
                    {
                        "schema": "rq2_public_grid_review_fixture_summary_vnext_v2",
                        "record_count": 2,
            ...
            if not exact_result(staging, candidate, receipt):
                raise contract.ContractRejected("V2 staging exact reread rejected")
            ...
            if not exact_result(paths.result, candidate, receipt):
                raise contract.ContractRejected("V2 result exact reread rejected")
            verifier.verify("controller_post_result_pre_success")
            success_staging.mkdir(parents=True, exist_ok=False)
            mapping = contract.verify_full_live_closure()
            success_body = {
                "schema": "rq2_public_grid_success_commit_vnext_v2",
                "session_id": session_id,
            ...
            if classification != "committed_success":
                raise contract.ContractRejected("V2 final publication truth rejected")
    ...
    try:
        if registered_test_case == "zero_worker_complete_forgery":
            ...
            for index, block_id in enumerate(contract.BLOCKS, 1):
                ...
                receipt_path = attempt_root / "attempt_receipt.json"
                result = {
                    "schema": "rq2_public_grid_worker_result_vnext_v2",
                    **common,
                    "hello_sha256": contract.sha256_bytes(hello_raw),
                    "envelope_sha256": contract.sha256_bytes(envelope_raw),
                    "pipe_authority_digest": pipe_digest,
                ...
                result_raw = contract.exact_json_bytes(result)
                receipt = {
                    "schema": "rq2_public_grid_worker_attempt_receipt_vnext_v2",
                    **common,
                    "result_path": str(result_path),
                    "result_sha256": contract.sha256_bytes(result_raw),
                    "scientific_payload_sha256": contract.sha256_bytes(scientific_raw),
                ...
                receipt_raw = contract.exact_json_bytes(receipt)
                ack = {
                    "schema": "rq2_public_grid_worker_ack_vnext_v2",
                    **common,
                    "worker_identity": worker_identity,
                    "hello_sha256": contract.sha256_bytes(hello_raw),
                    "envelope_sha256": contract.sha256_bytes(envelope_raw),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if arguments != ["--validate-only"]:
        raise contract.ContractRejected(
            "V2 remains review closed; production/pilot/formal execution is forbidden"
        )
    mapping = contract.verify_full_live_closure()
    print(
        json.dumps(
            {
                ...
                "status": "evidence_publication_successor_v2_review_closed",
