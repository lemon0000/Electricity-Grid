"""Secret-free presence, truth-table, and tree primitives for V2 publication."""
...
def capture_presence(paths: PublicationPaths) -> PresenceSnapshot:
    ...
    result = prior._probe_one_path(paths.result, label="v2 result")
    success = prior._probe_one_path(paths.success, label="v2 success")
    terminal = prior._probe_one_path(paths.terminal, label="v2 terminal")
...
def classify_publication(
    ...
    """Independent implementation of the frozen three-row V2 truth table."""
...
def typed_tree(root: Path) -> dict[str, object]:
    ...
    return {
        "schema": "rq2_public_grid_evidence_publication_typed_tree_v2",
