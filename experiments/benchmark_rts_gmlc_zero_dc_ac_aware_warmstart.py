...
def _read_config(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        != "repository_local_nonformal_warmstart_performance_pilot_after_v1_no_solve_and_v2_byte_drift_invalidation"
        ...
        or benchmark.get("attempt_id") != "warmstart_benchmark_v3_registered_run1"
...
def _verify_provenance(config: Mapping[str, Any]) -> dict[str, object]:
    ...
    superseded_v2_preparation_root = Path(provenance["superseded_v2_preparation_root"])
    _verify_manifest_hash(
        superseded_v2_preparation_root,
        str(provenance["superseded_v2_preparation_manifest_sha256"]),
        "superseded v2 preparation",
    )
    superseded_v2_registration = json.loads(
        (superseded_v2_preparation_root / "registration.json").read_text(
            encoding="utf-8"
        )
    )
    superseded_v2_invalidation_root = Path(
        provenance["superseded_v2_invalidation_root"]
    )
    _verify_manifest_hash(
        superseded_v2_invalidation_root,
        str(provenance["superseded_v2_invalidation_manifest_sha256"]),
        "superseded v2 invalidation",
    )
    superseded_v2_invalidation = json.loads(
        (superseded_v2_invalidation_root / "invalidation.json").read_text(
            encoding="utf-8"
        )
    )
    if (
        superseded_v2_registration.get("benchmark_id")
        != "rts_gmlc_google_day0_zero_dc_ac_aware_warmstart_benchmark_v2"
        or superseded_v2_registration.get("competitive_solve_executed") is not False
        or superseded_v2_registration.get("input_contract_sha256")
        != provenance["superseded_v2_input_contract_sha256"]
        or superseded_v2_invalidation.get("benchmark_id")
        != superseded_v2_registration.get("benchmark_id")
        or superseded_v2_invalidation.get("schema")
        != "rts_gmlc_zero_dc_ac_aware_warmstart_invalidation_v1"
        or superseded_v2_invalidation.get("status")
        != "invalidated_after_two_of_four_runs_due_to_shared_dependency_byte_drift"
        or superseded_v2_invalidation.get("preparation_manifest_sha256")
        != provenance["superseded_v2_preparation_manifest_sha256"]
        or superseded_v2_invalidation.get("input_contract_sha256")
        != provenance["superseded_v2_input_contract_sha256"]
        or superseded_v2_invalidation.get("competitive_solve_count") != 2
        or superseded_v2_invalidation.get("planned_solve_count") != 4
        or superseded_v2_invalidation.get("completed_runs_are_diagnostic_only")
        is not True
        or superseded_v2_invalidation.get("candidate_or_joint_ac_result") is not False
        or provenance.get(
            "superseded_v2_completed_runs_allowed_for_successor_selection"
        )
        is not False
        or superseded_v2_invalidation.get(
            "completed_runs_allowed_for_successor_selection"
        )
        is not False
        or provenance.get(
        ...
        is not True
        or superseded_v2_invalidation.get(
            "successor_must_rerun_complete_registered_matrix"
        )
        is not True
        or (_V2_ROOT / "benchmark").exists()
    ):
        raise RuntimeError("Superseded warm-start v2 evidence drifted")
    ...
    return {
        ...
        "superseded_v2_preparation_manifest_sha256": provenance[
            "superseded_v2_preparation_manifest_sha256"
        ],
        "superseded_v2_input_contract_sha256": provenance[
            "superseded_v2_input_contract_sha256"
        ],
        "superseded_v2_invalidation_manifest_sha256": provenance[
            "superseded_v2_invalidation_manifest_sha256"
        ],
        "superseded_v2_completed_solve_count": 2,
        "superseded_v2_planned_solve_count": 4,
        "superseded_v2_completed_runs_are_diagnostic_only": True,
        "superseded_v2_completed_runs_allowed_for_successor_selection": False,
        "superseded_v2_successor_must_rerun_complete_registered_matrix": True,
...
def prepare(
    ...
    input_contract = {
        "schema": "rts_gmlc_zero_dc_ac_aware_warmstart_inputs_v3",
    ...
    payload = {
        "schema": "rts_gmlc_zero_dc_ac_aware_warmstart_preparation_v3",
...
def run_benchmark(
    ...
    summary = {
        "schema": "rts_gmlc_zero_dc_ac_aware_warmstart_benchmark_result_v3",
        "benchmark_id": config["preregistration"]["id"],
