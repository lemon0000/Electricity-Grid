"""Stdlib-first bootstrap for review-closed Vnext execution successor v3."""
...
class BootstrapRejected(RuntimeError):
    """The pre-project-import V3 bootstrap failed closed."""
...
def _verify_bundle() -> tuple[dict[str, object], str]:
    try:
        ...
        raise BootstrapRejected("V3 bundle JSON malformed") from exc
    ...
    if (
        ...
        raise BootstrapRejected("V3 bundle identity/inventory drifted")
    for relative, expected in inner["files"].items():
        if _sha(_stable(ROOT / relative)) != expected:
            raise BootstrapRejected(f"V3 bundle member hash drifted: {relative}")
...
def _preimport_review(config: dict[str, object], outer_sha256: str) -> None:
    if not REVIEW.exists() or REVIEW.is_symlink():
        raise BootstrapRejected("fixed V3 execution-review PASS receipt is absent")
    try:
        value = json.loads(_stable(REVIEW))
    except json.JSONDecodeError as exc:
        raise BootstrapRejected("V3 execution-review receipt JSON malformed") from exc
    ...
    expected_keys = {
        ...
        "reviewed_outer",
        "bound_v2_outer",
        "bound_v2_pre_run_pass",
        "bound_v2_post_result_rework",
        "bound_public_key",
        "findings",
        "effect",
    }
    if (
        ...
        != {"path": predecessor["pre_run_pass_path"], "sha256": predecessor["pre_run_pass_sha256"]}
        or value.get("bound_v2_post_result_rework")
        != {"path": predecessor["post_result_rework_path"], "sha256": predecessor["post_result_rework_sha256"]}
        ...
        raise BootstrapRejected("V3 execution-review receipt binding/effect drifted")
