"""Fail-closed evidence contract for isolated Vnext successor V6.

V6 changes only resource-audit evidence: deadline classification is bound to
...
from experiments import (
    rq2_public_grid_two_block_pilot_vnext_execution_contract_v5 as predecessor,
)

ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v6.json"
INNER = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v6.SHA256SUMS.json"
OUTER = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v6.OUTER.SHA256SUMS.json"
REVIEW = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_review_pass_v6.json"
PUBLIC_KEY = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v6_ots_public_key.json"
V5_ESCALATE = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_review_escalate_v5.json"
V5_REVIEW_EVIDENCE = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_review_evidence_escalate_v5.json"
V5_KEY_REVOCATION = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v5_ots_key_revocation_v1.json"
...
def load_config() -> dict[str, Any]:
    value = _load_json(CONFIG, label="V6 config")
    if (
        value.get("schema")
        != "rq2_public_grid_two_block_pilot_vnext_execution_successor_v6"
        or value.get("version") != 6
        or value.get("status")
        != "vnext_execution_successor_v6_ready_for_independent_review"
        or tuple(value.get("pilot", {}).get("blocks", ())) != BLOCKS
        or value.get("remediation_scope")
        != "new_isolated_versioned_v6_detection_overrun_and_monitor_owned_persistence_not_v5_resume"
        or any(item is not False for item in value.get("gates", {}).values())
        or value.get("controller_authentication", {}).get("security_certified")
        is not False
    ):
        raise ContractRejected("V6 config identity/gates drifted")
    return value
...
def verify_v5_escalate_and_revocation() -> None:
    config = load_config()
    frozen = config["predecessor_v5"]
    named = {
        V5_ESCALATE: frozen["review_escalate_sha256"],
        V5_REVIEW_EVIDENCE: frozen["review_evidence_sha256"],
        V5_KEY_REVOCATION: frozen["key_revocation_sha256"],
    ...
    if any(_sha_file(path) != digest for path, digest in named.items()):
        raise ContractRejected("sealed V5 ESCALATE/revocation bytes drifted")
    receipt = _load_json(V5_ESCALATE, label="V5 ESCALATE receipt")
    if (
        receipt.get("schema")
        != "rq2_public_grid_two_block_pilot_vnext_execution_successor_review_escalate_v5"
        or receipt.get("version") != 5
        ...
        or receipt.get("reviewed_v5", {}).get("outer_sha256")
        != frozen["outer_sha256"]
        or receipt.get("reviewed_v5", {}).get("inner_sha256")
        != frozen["inner_sha256"]
        or receipt.get("reviewed_v5", {}).get("public_anchor_sha256")
        != frozen["public_anchor_sha256"]
        or receipt.get("reviewed_v5", {}).get("key_id") != frozen["key_id"]
        ...
        or receipt.get("findings", [{}])[0].get(
            "same_deadline_persistence_finding_repeated"
        ) is not True
        or receipt.get("effect", {}).get("v5_execution_authorized") is not False
        ...
        or receipt.get("effect", {}).get("v6_versioned_cycle_only") is not True
    ):
        raise ContractRejected("V5 ESCALATE receipt binding/effect drifted")
    evidence = _load_json(V5_REVIEW_EVIDENCE, label="V5 review evidence")
    revocation = _load_json(V5_KEY_REVOCATION, label="V5 key revocation")
    tombstone = _load_json(
        ROOT / frozen["revoked_tombstone_path"], label="V5 revoked tombstone"
    )
    if (
...
def verify_self_bundle() -> dict[str, str]:
    ...
    for relative in sorted(expected_members):
        ...
        if inner["files"].get(relative) != digest:
            raise ContractRejected(f"V6 bundle member hash drifted: {relative}")
        mapping[relative] = digest
    if len(mapping) != config["bundle"]["self_mapping_exact_count"]:
        raise ContractRejected("V6 self mapping count drifted")
    return dict(sorted(mapping.items()))
...
def verify_live_authorities() -> dict[str, str]:
    config = load_config()
    verify_v5_escalate_and_revocation()
    try:
        predecessor_mapping = predecessor.verify_live_authorities()
    except Exception as exc:
        raise ContractRejected("sealed V4/V6 live authority drifted") from exc
    mapping = _merge_exact(
        predecessor_mapping,
        verify_self_bundle(),
    )
    if len(mapping) != config["bundle"]["live_authority_exact_count"]:
        raise ContractRejected("V6 live authority exact count drifted")
...
def load_public_key() -> dict[str, Any]:
    ...
    if (
        ...
        or value.get("schema")
        != "rq2_public_grid_vnext_execution_successor_v6_lamport_ots_public_key"
...
def lamport_sign_digest(seed: bytes, digest_sha256: str) -> dict[str, Any]:
    ...
    return {
        "schema": "rq2_public_grid_vnext_execution_v6_lamport_signature",
...
def verify_lamport_signature(
    ...
    if (
        ...
        != "rq2_public_grid_vnext_execution_v6_lamport_signature"
...
def validate_resource_journal(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        value.get("schema") != "rq2_public_grid_resource_journal_vnext_execution_v6"
        or value.get("version") != 6
    ...
    if termination is not None:
        ...
        if (
            termination["schema"]
            != "rq2_public_grid_owned_termination_evidence_vnext_execution_v6"
...
def synthetic_resource_journal_for_test(
    *,
    ...
    value = {
        "schema": "rq2_public_grid_resource_journal_vnext_execution_v6",
        "version": 6,
        "owned_identity": identity,
...
def _synthetic_termination_evidence(
    *,
    ...
    return {
        "schema": "rq2_public_grid_owned_termination_evidence_vnext_execution_v6",
        "version": 1,
...
def validate_resource_monitor_outcome(
    ...
    if (
        value.get("schema")
        != "rq2_public_grid_resource_monitor_persisted_outcome_vnext_execution_v6"
        or value.get("version") != 1
...
def collect_solver_runtime_evidence(worker_identity: Mapping[str, int]) -> dict[str, Any]:
    ...
    value = {
        "schema": "rq2_public_grid_solver_runtime_evidence_vnext_execution_v6",
...
def validate_solver_runtime_evidence(
    ...
    if (
        ...
        or value.get("schema")
        != "rq2_public_grid_solver_runtime_evidence_vnext_execution_v6"
...
def build_worker_hello(
    *,
    ...
    v3_mapping = (
        predecessor.predecessor.predecessor.predecessor.v3.verify_full_live_closure()
    )
    return {
        "schema": "rq2_public_grid_worker_hello_vnext_execution_v6",
        "mode": mode,
        "session_id": session_id,
        "execution_index": execution_index,
        "block_id": block_id,
        ...
        "controller_source_sha256": self_mapping[
            "experiments/run_rq2_public_grid_two_block_pilot_vnext_execution_successor_v6.py"
        ],
        "worker_source_sha256": self_mapping[
            "experiments/worker_rq2_public_grid_two_block_pilot_vnext_execution_successor_v6.py"
        ],
        ...
        "v3_closure_mapping_sha256": (
            predecessor.predecessor.predecessor.predecessor.v3.closure_mapping_sha256(
                v3_mapping
            )
        ),
...
def hello_transport_context(hello: Mapping[str, Any]) -> dict[str, Any]:
    if hello.get("schema") != "rq2_public_grid_worker_hello_vnext_execution_v6":
...
def build_worker_envelope(
    *, hello: dict[str, Any], hello_raw: bytes, pipe_authority: dict[str, Any], attempt_root: str
    ...
    return {
        "schema": "rq2_public_grid_worker_envelope_vnext_execution_v6",
...
def build_worker_result(
    *,
    ...
    return {
        "schema": "rq2_public_grid_worker_result_vnext_execution_v6",
        "transport_context": hello_transport_context(hello),
        "hello_sha256": sha256_bytes(hello_raw),
...
def build_worker_exit_notice(
    *, hello: dict[str, Any], envelope: dict[str, Any], result: dict[str, Any], result_raw: bytes, result_path: str
    ...
    return {
        "schema": "rq2_public_grid_worker_exit_notice_vnext_execution_v6",
        "transport_context": hello_transport_context(hello),
...
def build_attempt_receipt(
    *, hello: dict[str, Any], envelope: dict[str, Any], result: dict[str, Any], result_raw: bytes,
    ...
    return {
        "schema": "rq2_public_grid_attempt_receipt_vnext_execution_v6",
        "transport_context": hello_transport_context(hello),
        "hello_sha256": result["hello_sha256"],
        "envelope_sha256": result["envelope_sha256"],
...
def build_controller_ack(
    *, hello: dict[str, Any], envelope: dict[str, Any], result: dict[str, Any], result_raw: bytes,
    ...
    return {
        "schema": "rq2_public_grid_controller_ack_vnext_execution_v6",
        "transport_context": hello_transport_context(hello),
        "hello_sha256": result["hello_sha256"],
        "envelope_sha256": result["envelope_sha256"],
        "exit_notice_sha256": sha256_bytes(exit_notice_raw),
...
def validate_execution_review_object(value: object) -> None:
    config = load_config()
    if not isinstance(value, dict):
        raise ContractRejected("V6 execution-review receipt malformed")
    expected_keys = {
        "schema", "version", "reviewed_on", "reviewer_role", "verdict",
        "reviewed_outer", "bound_v5_outer", "bound_v5_escalate",
        "bound_v5_public_key", "bound_public_key", "findings", "effect"
    }
    if (
        ...
        or value.get("bound_v5_outer")
        != {"path": config["predecessor_v5"]["outer_path"], "sha256": config["predecessor_v5"]["outer_sha256"]}
        or value.get("bound_v5_escalate")
        != {"path": config["predecessor_v5"]["review_escalate_path"], "sha256": config["predecessor_v5"]["review_escalate_sha256"]}
        or value.get("bound_v5_public_key")
        != {"path": config["predecessor_v5"]["public_anchor_path"], "sha256": config["predecessor_v5"]["public_anchor_sha256"], "key_id": config["predecessor_v5"]["key_id"]}
        ...
        raise ContractRejected("V6 execution-review receipt binding/effect drifted")


def require_execution_review() -> dict[str, Any]:
    if not REVIEW.exists() or REVIEW.is_symlink():
        raise ContractRejected("fixed V6 execution-review PASS receipt is absent")
    raw = read_stable(REVIEW)
    try:
        value = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise ContractRejected("V6 execution-review receipt JSON malformed") from exc
    validate_execution_review_object(value)
    if read_stable(REVIEW) != raw:
        raise ContractRejected("V6 execution-review receipt changed during validation")
    return value
...
def run_actual_science(block_id: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    return predecessor.predecessor.predecessor.predecessor.run_actual_science(block_id)
...
def validate_actual_science_payload(
    ...
    require_execution_review()
    verify_live_authorities()
    return predecessor.predecessor.predecessor.predecessor.validate_actual_science_payload(
        payload, block_id
    )
...
def resource_primitives() -> Any:
    verify_live_authorities()
    return predecessor.predecessor.predecessor.predecessor.resource_authority()
...
def build_attestation_payload(
    *, session_id: str, entries: Mapping[str, bytes], resource_journals: Mapping[str, dict[str, Any]],
    ...
    return {
        "schema": "rq2_public_grid_controller_attestation_payload_vnext_execution_v6",
        "session_id": session_id,
...
def build_attestation(payload: dict[str, Any], seed: bytes | bytearray) -> dict[str, Any]:
    ...
    return {
        "schema": "rq2_public_grid_controller_attestation_vnext_execution_v6",
        "public_key": load_public_key(),
        "payload": payload,
...
def verify_attestation(value: Mapping[str, Any]) -> dict[str, Any]:
    if (
        ...
        or value.get("schema")
        != "rq2_public_grid_controller_attestation_vnext_execution_v6"
...
def verify_published_artifacts(
    ...
    """Independently verify V6 publication using only sealed public material."""
    from experiments import (
    ...
    try:
        ...
        raise ContractRejected("published V6 artifact missing or malformed") from exc
    if result_tree != publisher.typed_tree(result) or success_tree != publisher.typed_tree(success):
        raise ContractRejected("published V6 typed tree drifted")
    ...
    if publisher.classify_publication(presence, result_exact=True, success_exact=True) != "committed_success":
        raise ContractRejected("published V6 presence classification drifted")
    for path in result.rglob("*.json"):
        if '"seed_hex"' in read_stable(path).decode("utf-8"):
            raise ContractRejected("private seed leaked into V6 publication")
    if '"seed_hex"' in read_stable(success / "success.json").decode("utf-8"):
        raise ContractRejected("private seed leaked into V6 success readback")
    return {
        "classification": "committed_success",
