"""Review-closed V5 evidence rerun for the fixed nonformal two-block pilot."""
...
def run_review_preloader_e2e() -> dict[str, Any]:
    ...
    try:
        ...
        expected_ack = {
            "schema": "rq2_public_grid_preloader_ack_vnext_execution_v5",
...
def run_review_resource_probe_e2e() -> dict[str, Any]:
    ...
    try:
        ...
        release = {
            "schema": "rq2_public_grid_resource_probe_release_vnext_execution_v5",
            "hello_sha256": contract.sha256_bytes(hello_raw),
        ...
        expected_notice = {
            "schema": "rq2_public_grid_resource_probe_notice_vnext_execution_v5",
            "hello_sha256": contract.sha256_bytes(hello_raw),
...
def run_two_block_nonformal(
    *,
    ...
    """Execute one new evidence run only after the fixed V5 PASS receipt."""
    ...
    if _review_lease_probe:
        ...
        test_lease = {
            "schema": "rq2_public_grid_vnext_execution_successor_v5_lamport_ots_private_lease",
            "version": 1,
            "state": "fresh",
            "key_id": auth["key_id"],
        ...
        if any(path.exists() or path.is_symlink() for path in paths.values()):
            raise contract.ContractRejected(
                "canonical V5 execution roots must be clean absent"
            )
        resources = contract.resource_primitives()
        ...
        if consumed_lease.exists() or consumed_lease.is_symlink():
            raise contract.ContractRejected("one-time V5 key was already consumed")
    ...
    def tombstone_key(state: str = "consumed") -> None:
        ...
        tombstone = {
            "schema": "rq2_public_grid_vnext_execution_successor_v5_lamport_ots_consumed_tombstone",
            "version": 1,
            "state": state,
            "key_id": auth["key_id"],
            "public_key_sha256": auth["public_key_sha256"],
    ...
    def acquire_private_lease() -> bytearray:
        ...
        try:
            ...
            if (
                ...
                or value.get("schema")
                != "rq2_public_grid_vnext_execution_successor_v5_lamport_ots_private_lease"
    ...
    if _review_lease_probe:
        ...
        try:
            ...
            digest = contract.sha256_bytes(b"rq2-v5-review-lease-probe")
    ...
    def dispatch(index: int) -> None:
        ...
        try:
            ...
            persisted_resource_journal = attempt_root / "controller_resource_journal.json"
            resource_journal_raw = contract.exact_json_bytes(resource_journal)
            publisher.atomic_write(persisted_resource_journal, resource_journal_raw)
            if contract.read_stable(persisted_resource_journal) != resource_journal_raw:
                raise contract.ContractRejected(
                    "controller resource journal persistence/readback drifted"
                )
            if (
            ...
            body = {
                "schema": "rq2_public_grid_accepted_evidence_vnext_execution_v5",
                "session_id": session_id,
                "execution_index": index,
                "block_id": block_id,
                "predecessor_digest": predecessor_digest,
