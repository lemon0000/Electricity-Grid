"""Review-closed V3 evidence rerun for the fixed nonformal two-block pilot."""
...
def _terminate_exact(process: subprocess.Popen[Any], **identity: int) -> None:
    resources = contract.resource_primitives()
    resources.terminate_exact_owned_child(process, **identity)


def _start_resource_monitor(
    process: subprocess.Popen[Any], worker_identity: dict[str, int]
) -> tuple[
...
def _wait_first_sample(state: contract.ResourceMonitorState, deadline_ns: int) -> None:
    timeout = max(0.0, (deadline_ns - time.monotonic_ns()) / 1_000_000_000)
    if not state.ready.wait(timeout=timeout) or not state.first_sample_success:
...
def run_review_preloader_e2e() -> dict[str, Any]:
    ...
    try:
        ...
        expected_ack = {
            "schema": "rq2_public_grid_preloader_ack_vnext_execution_v3",
...
def run_review_resource_probe_e2e() -> dict[str, Any]:
    ...
    try:
        ...
        _wait_first_sample(state, deadline)
        release = {
            "schema": "rq2_public_grid_resource_probe_release_vnext_execution_v3",
            "hello_sha256": contract.sha256_bytes(hello_raw),
        ...
        journal = future.result(timeout=30)
        ...
        expected_notice = {
            "schema": "rq2_public_grid_resource_probe_notice_vnext_execution_v3",
            "hello_sha256": contract.sha256_bytes(hello_raw),
...
def run_two_block_nonformal() -> dict[str, Any]:
    """Execute one new evidence run only after the fixed V3 PASS receipt."""
    ...
    if any(path.exists() or path.is_symlink() for path in paths.values()):
        raise contract.ContractRejected("canonical V3 execution roots must be clean absent")
    resources = contract.resource_primitives()
    ...
    if consumed_lease.exists() or consumed_lease.is_symlink():
        raise contract.ContractRejected("one-time V3 key was already consumed")
    ...
    def validate_and_consume_private_lease() -> bytearray:
        """Only this gated controller closure may materialize the production seed."""
        if not fresh_lease.is_file() or fresh_lease.is_symlink() or consumed_lease.exists():
            raise contract.ContractRejected("fresh one-time private lease unavailable")
        ...
        public = contract.load_public_key()
        if (
            set(value) != contract.PRIVATE_LEASE_KEYS
            or value.get("schema")
            != "rq2_public_grid_vnext_execution_successor_v3_lamport_ots_private_lease"
            or value.get("state") != "fresh"
            or value.get("key_id") != public["key_id"]
            or value.get("public_key_sha256") != public["public_key_sha256"]
            or len(seed) != 32
            ...
            raise contract.ContractRejected("private lease/public anchor mismatch")
        os.replace(fresh_lease, consumed_lease)
        if fresh_lease.exists() or contract.read_stable(consumed_lease) != raw:
            raise contract.ContractRejected("private lease atomic consume readback failed")
        return seed

    def tombstone_key() -> None:
        nonlocal key_tombstoned
        if key_tombstoned:
            return
        if not consumed_lease.is_file() or consumed_lease.is_symlink():
            raise contract.ContractRejected("consumed lease unavailable for tombstone")
        tombstone = {
            "schema": "rq2_public_grid_vnext_execution_successor_v3_lamport_ots_consumed_tombstone",
            "version": 1,
            "state": "consumed",
            "key_id": auth["key_id"],
        ...
        if contract.read_stable(consumed_lease) != contract.exact_json_bytes(tombstone):
            raise contract.ContractRejected("consumed lease tombstone readback failed")
        key_tombstoned = True
    ...
    def dispatch(index: int) -> None:
        ...
        try:
            ...
            os.close(controller_write)
            controller_write = -1
            exit_notice_raw, exit_notice = contract.read_frame(
                controller_read, "science exit notice"
            )
            contract.require_eof(controller_read)
            process.wait(timeout=max(1.0, (deadline - time.monotonic_ns()) / 1e9))
            resource_journal = resource_future.result(timeout=30)
            executor.shutdown(wait=True)
            executor = None
            if (
            ...
            body = {
                "schema": "rq2_public_grid_accepted_evidence_vnext_execution_v3",
                "session_id": session_id,
                "execution_index": index,
                "block_id": block_id,
                "predecessor_digest": predecessor_digest,
    ...
    try:
        key_seed = validate_and_consume_private_lease()
        dispatch(1)
        dispatch(2)
