"""V5 lifecycle controller with the full block-zero V4 production path."""
...
from experiments import (
    rq2_public_grid_highs_formal_activation_contract_v5 as authority,
)
from experiments import (
...
REQUEST_SCHEMA = "rq2_grid_need_process_isolated_worker_request_formal_v6"
RELEASE_SCHEMA = "rq2_grid_need_process_isolated_worker_release_formal_v6"
...
def _verify_execution_boundary(label: str) -> dict[str, Any]:
    ...
    preseal = (
        os.environ.get("RQ2_V5_PRESEAL_CLOSURE") is not None
...
def _require_consumed_controller_authority(dynamic_authority: Path) -> dict[str, Any]:
    ...
    expected_schema = (
        ...
        else "rq2_public_grid_highs_formal_activation_v5_consumed_authority"
    )
    if (
...
def validate_only(config_path: Path = CONFIG) -> dict[str, Any]:
    ...
    return {
        "schema": (
            "rq2_grid_need_process_isolated_formal_preflight_v6_sealed"
            if sealed
            else "rq2_grid_need_process_isolated_formal_preflight_v6_draft"
        ),
    ...
    *,
    ...
    try:
        ...
        if preseal_startup_probe:
            ...
            result = {
                "schema": "rq2_v5_preseal_startup_probe_result",
