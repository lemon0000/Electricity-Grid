"""Fail-closed evidence contract for Vnext execution successor v4.

The scientific protocol and resource thresholds are inherited unchanged.  V4
adds a frozen one-second OS-scheduling audit budget around the exact five-second
resource slots, prohibits catch-up sampling, and closes every post-rename lease
failure path with a seed-free tombstone.  It is not a security certification
and does not claim protection from a malicious process running as the same OS
user before the private lease is consumed.
"""
...
from experiments import (
    rq2_public_grid_two_block_pilot_vnext_execution_contract_v3 as predecessor,
)
...
PUBLIC_KEY = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v4_ots_public_key.json"
V3_ESCALATE = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_review_escalate_v3.json"
V3_REVIEW_EVIDENCE = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_review_evidence_escalate_v3.json"
V3_KEY_REVOCATION = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v3_ots_key_revocation_v2.json"
...
def load_config() -> dict[str, Any]:
    value = _load_json(CONFIG, label="V4 config")
    if (
        value.get("schema")
        != "rq2_public_grid_two_block_pilot_vnext_execution_successor_v4"
        or value.get("version") != 4
        or value.get("status")
        != "vnext_execution_successor_v4_ready_for_independent_review"
        or tuple(value.get("pilot", {}).get("blocks", ())) != BLOCKS
        or value.get("remediation_scope")
        != "new_versioned_v4_resource_scheduler_and_lease_cleanup_not_v3_rework_or_resume"
        or any(item is not False for item in value.get("gates", {}).values())
        or value.get("controller_authentication", {}).get("security_certified")
        is not False
    ):
        raise ContractRejected("V4 config identity/gates drifted")
    return value
...
def verify_v3_escalation_and_revocation() -> None:
    config = load_config()
    frozen = config["predecessor_v3"]
    named = {
        V3_ESCALATE: frozen["review_escalate_sha256"],
        V3_REVIEW_EVIDENCE: frozen["review_evidence_sha256"],
        V3_KEY_REVOCATION: frozen["key_revocation_sha256"],
    ...
    if any(_sha_file(path) != digest for path, digest in named.items()):
        raise ContractRejected("sealed V3 escalation/revocation bytes drifted")
    receipt = _load_json(V3_ESCALATE, label="V3 ESCALATE receipt")
    if (
        receipt.get("schema")
        != "rq2_public_grid_two_block_pilot_vnext_execution_successor_review_escalate_v3"
        ...
        or receipt.get("reviewed_v3", {}).get("outer_sha256")
        != frozen["outer_sha256"]
        or receipt.get("reviewed_v3", {}).get("inner_sha256")
        != frozen["inner_sha256"]
        or receipt.get("reviewed_v3", {}).get("public_anchor_sha256")
        != frozen["public_anchor_sha256"]
        or receipt.get("reviewed_v3", {}).get("key_id") != frozen["key_id"]
        or len(receipt.get("findings", ())) != 2
        or receipt.get("findings", [{}])[0].get("finding_id")
        != "RESOURCE_ACTUAL_CADENCE_NOT_BOUNDED"
        or receipt.get("findings", [{}])[0].get(
            "repeated_post_result_resource_finding"
        )
        is not True
        or receipt.get("effect", {}).get("v3_execution_authorized") is not False
        ...
        or receipt.get("effect", {}).get("new_versioned_v4_cycle_authorized")
        is not True
    ):
        raise ContractRejected("V3 ESCALATE receipt binding/effect drifted")
    evidence = _load_json(V3_REVIEW_EVIDENCE, label="V3 review evidence")
    revocation = _load_json(V3_KEY_REVOCATION, label="V3 key revocation")
    tombstone = _load_json(
        ROOT / frozen["revoked_tombstone_path"], label="V3 revoked tombstone"
    )
    if (
        evidence.get("authority") is not False
        or evidence.get("resource_reproducer", {}).get(
            "arbitrary_100s_observation_gap_accepted"
        )
        is not True
        ...
        raise ContractRejected("V3 escalation evidence/key revocation drifted")


def verify_self_bundle() -> dict[str, str]:
    config = load_config()
    outer = _load_json(OUTER, label="V4 outer")
    inner_raw = read_stable(INNER)
    inner = _load_json(INNER, label="V4 inner")
    ...
    if (
        ...
        raise ContractRejected("V4 non-cyclic bundle identity/inventory drifted")
    mapping = {
    ...
    for relative in sorted(expected_members):
        ...
        if inner["files"].get(relative) != digest:
            raise ContractRejected(f"V4 bundle member hash drifted: {relative}")
        mapping[relative] = digest
    if len(mapping) != config["bundle"]["self_mapping_exact_count"]:
        raise ContractRejected("V4 self mapping count drifted")
    return dict(sorted(mapping.items()))
...
def verify_live_authorities() -> dict[str, str]:
    config = load_config()
    verify_v3_escalation_and_revocation()
    try:
        predecessor_mapping = predecessor.verify_live_authorities()
    except Exception as exc:
        raise ContractRejected("sealed V3/V4 live authority drifted") from exc
    mapping = _merge_exact(
        predecessor_mapping,
        verify_self_bundle(),
    )
    if len(mapping) != config["bundle"]["live_authority_exact_count"]:
        raise ContractRejected("V4 live authority exact count drifted")
...
def load_public_key() -> dict[str, Any]:
    ...
    if (
        ...
        or value.get("schema")
        != "rq2_public_grid_vnext_execution_successor_v4_lamport_ots_public_key"
...
def lamport_sign_digest(seed: bytes, digest_sha256: str) -> dict[str, Any]:
    ...
    return {
        "schema": "rq2_public_grid_vnext_execution_v4_lamport_signature",
...
def verify_lamport_signature(
    ...
    if (
        ...
        != "rq2_public_grid_vnext_execution_v4_lamport_signature"
...
def validate_resource_journal(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        value.get("schema") != "rq2_public_grid_resource_journal_vnext_execution_v4"
        or value.get("version") != 4
...
def synthetic_resource_journal_for_test(
    *,
    ...
    value = {
        "schema": "rq2_public_grid_resource_journal_vnext_execution_v4",
        "version": 4,
        "owned_identity": identity,
        ...
        "expected_sample_count_at_exit": (
            samples[-1]["monotonic_ns"] + 1 - start_mono
        )
...
def collect_solver_runtime_evidence(worker_identity: Mapping[str, int]) -> dict[str, Any]:
    ...
    value = {
        "schema": "rq2_public_grid_solver_runtime_evidence_vnext_execution_v4",
...
def validate_solver_runtime_evidence(
    ...
    if (
        ...
        or value.get("schema")
        != "rq2_public_grid_solver_runtime_evidence_vnext_execution_v4"
...
def build_worker_hello(
    *,
    ...
    v3_mapping = predecessor.predecessor.v3.verify_full_live_closure()
    return {
        "schema": "rq2_public_grid_worker_hello_vnext_execution_v4",
        "mode": mode,
        "session_id": session_id,
        "execution_index": execution_index,
        "block_id": block_id,
        ...
        "controller_source_sha256": self_mapping[
            "experiments/run_rq2_public_grid_two_block_pilot_vnext_execution_successor_v4.py"
        ],
        "worker_source_sha256": self_mapping[
            "experiments/worker_rq2_public_grid_two_block_pilot_vnext_execution_successor_v4.py"
        ],
        ...
        "v3_closure_mapping_sha256": predecessor.predecessor.v3.closure_mapping_sha256(
            v3_mapping
        ),
...
def hello_transport_context(hello: Mapping[str, Any]) -> dict[str, Any]:
    if hello.get("schema") != "rq2_public_grid_worker_hello_vnext_execution_v4":
...
def build_worker_envelope(
    *, hello: dict[str, Any], hello_raw: bytes, pipe_authority: dict[str, Any], attempt_root: str
    ...
    return {
        "schema": "rq2_public_grid_worker_envelope_vnext_execution_v4",
...
def build_worker_result(
    *,
    ...
    return {
        "schema": "rq2_public_grid_worker_result_vnext_execution_v4",
        "transport_context": hello_transport_context(hello),
        "hello_sha256": sha256_bytes(hello_raw),
...
def build_worker_exit_notice(
    *, hello: dict[str, Any], envelope: dict[str, Any], result: dict[str, Any], result_raw: bytes, result_path: str
    ...
    return {
        "schema": "rq2_public_grid_worker_exit_notice_vnext_execution_v4",
        "transport_context": hello_transport_context(hello),
...
def build_attempt_receipt(
    *, hello: dict[str, Any], envelope: dict[str, Any], result: dict[str, Any], result_raw: bytes,
    ...
    return {
        "schema": "rq2_public_grid_attempt_receipt_vnext_execution_v4",
        "transport_context": hello_transport_context(hello),
        "hello_sha256": result["hello_sha256"],
        "envelope_sha256": result["envelope_sha256"],
...
def build_controller_ack(
    *, hello: dict[str, Any], envelope: dict[str, Any], result: dict[str, Any], result_raw: bytes,
    ...
    return {
        "schema": "rq2_public_grid_controller_ack_vnext_execution_v4",
        "transport_context": hello_transport_context(hello),
        "hello_sha256": result["hello_sha256"],
        "envelope_sha256": result["envelope_sha256"],
        "exit_notice_sha256": sha256_bytes(exit_notice_raw),
...
def validate_execution_review_object(value: object) -> None:
    config = load_config()
    if not isinstance(value, dict):
        raise ContractRejected("V4 execution-review receipt malformed")
    expected_keys = {
        "schema", "version", "reviewed_on", "reviewer_role", "verdict",
        "reviewed_outer", "bound_v3_outer", "bound_v3_escalate",
        "bound_v3_public_key", "bound_public_key", "findings", "effect"
    }
    if (
        ...
        != {"path": config["predecessor_v3"]["outer_path"], "sha256": config["predecessor_v3"]["outer_sha256"]}
        or value.get("bound_v3_escalate")
        != {"path": config["predecessor_v3"]["review_escalate_path"], "sha256": config["predecessor_v3"]["review_escalate_sha256"]}
        or value.get("bound_v3_public_key")
        != {"path": config["predecessor_v3"]["public_anchor_path"], "sha256": config["predecessor_v3"]["public_anchor_sha256"], "key_id": config["predecessor_v3"]["key_id"]}
        ...
        raise ContractRejected("V4 execution-review receipt binding/effect drifted")


def require_execution_review() -> dict[str, Any]:
    if not REVIEW.exists() or REVIEW.is_symlink():
        raise ContractRejected("fixed V4 execution-review PASS receipt is absent")
    raw = read_stable(REVIEW)
    try:
        value = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise ContractRejected("V4 execution-review receipt JSON malformed") from exc
    validate_execution_review_object(value)
    if read_stable(REVIEW) != raw:
        raise ContractRejected("V4 execution-review receipt changed during validation")
    return value
...
def run_actual_science(block_id: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    return predecessor.predecessor.run_actual_science(block_id)
...
def validate_actual_science_payload(
    ...
    require_execution_review()
    verify_live_authorities()
    return predecessor.predecessor.validate_actual_science_payload(payload, block_id)


def resource_primitives() -> Any:
    verify_live_authorities()
    return predecessor.predecessor.resource_authority()
...
def build_attestation_payload(
    *, session_id: str, entries: Mapping[str, bytes], resource_journals: Mapping[str, dict[str, Any]],
    ...
    return {
        "schema": "rq2_public_grid_controller_attestation_payload_vnext_execution_v4",
        "session_id": session_id,
...
def build_attestation(payload: dict[str, Any], seed: bytes | bytearray) -> dict[str, Any]:
    ...
    return {
        "schema": "rq2_public_grid_controller_attestation_vnext_execution_v4",
        "public_key": load_public_key(),
        "payload": payload,
...
def verify_attestation(value: Mapping[str, Any]) -> dict[str, Any]:
    if (
        ...
        or value.get("schema")
        != "rq2_public_grid_controller_attestation_vnext_execution_v4"
...
def verify_published_artifacts(
    ...
    """Independently verify V4 publication using only sealed public material."""
    from experiments import (
    ...
    try:
        ...
        raise ContractRejected("published V4 artifact missing or malformed") from exc
    if result_tree != publisher.typed_tree(result) or success_tree != publisher.typed_tree(success):
        raise ContractRejected("published V4 typed tree drifted")
    ...
    for execution_index, block in enumerate(BLOCKS, start=1):
        ...
        if (
            evidence.get("schema")
            != "rq2_public_grid_accepted_evidence_vnext_execution_v4"
    ...
    controller_receipt_body = {
        "schema": "rq2_public_grid_controller_receipt_vnext_execution_v4",
    ...
    expected_summary = {
        "schema": "rq2_public_grid_nonformal_two_block_summary_vnext_execution_v4",
        "blocks": list(BLOCKS),
        "record_count": 2,
        "scientific_payload_sha256s": science_hashes,
        "solver_call_accounting_sha256s": accounting_hashes,
    ...
    expected_result_core = {
        "schema": "rq2_public_grid_result_manifest_substantive_core_vnext_execution_v4",
        "session_id": payload["session_id"],
    ...
    expected_success_core = {
        "schema": "rq2_public_grid_success_readback_substantive_core_vnext_execution_v4",
        "session_id": payload["session_id"],
    ...
    expected_result = {
        "schema": "rq2_public_grid_result_manifest_vnext_execution_v4",
    ...
    expected_success = {
        "schema": "rq2_public_grid_success_commit_vnext_execution_v4",
    ...
    if publisher.classify_publication(presence, result_exact=True, success_exact=True) != "committed_success":
        raise ContractRejected("published V4 presence classification drifted")
    for path in result.rglob("*.json"):
        if '"seed_hex"' in read_stable(path).decode("utf-8"):
            raise ContractRejected("private seed leaked into V4 publication")
    if '"seed_hex"' in read_stable(success / "success.json").decode("utf-8"):
        raise ContractRejected("private seed leaked into V4 success readback")
    return {
        "classification": "committed_success",
