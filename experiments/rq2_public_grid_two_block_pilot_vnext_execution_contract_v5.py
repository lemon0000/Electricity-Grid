"""Fail-closed evidence contract for Vnext execution successor v5.

The scientific protocol, resource thresholds, normal journal path, and signing
semantics are inherited unchanged.  V5 adds only a frozen two-second audit cap
for the sealed exact-owned terminate/kill policy and complete termination
evidence for honest incomplete resource outcomes.  It is not a security
certification and does not claim protection from a malicious process running as
the same OS user before the private lease is consumed.
"""
...
from experiments import (
    rq2_public_grid_two_block_pilot_vnext_execution_contract_v4 as predecessor,
)

ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v5.json"
INNER = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v5.SHA256SUMS.json"
OUTER = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v5.OUTER.SHA256SUMS.json"
REVIEW = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_review_pass_v5.json"
PUBLIC_KEY = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v5_ots_public_key.json"
V4_REWORK = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_review_rework_v4.json"
V4_REVIEW_EVIDENCE = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_review_evidence_rework_v4.json"
V4_KEY_REVOCATION = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v4_ots_key_revocation_v1.json"
...
TERMINATION_LAST_SAMPLE_TO_EXIT_MAX_NS = 8_000_000_000
...
def load_config() -> dict[str, Any]:
    value = _load_json(CONFIG, label="V5 config")
    if (
        value.get("schema")
        != "rq2_public_grid_two_block_pilot_vnext_execution_successor_v5"
        or value.get("version") != 5
        or value.get("status")
        != "vnext_execution_successor_v5_ready_for_independent_review"
        or tuple(value.get("pilot", {}).get("blocks", ())) != BLOCKS
        or value.get("remediation_scope")
        != "new_versioned_v5_deadline_termination_persistence_only_not_v4_rework_or_resume"
        or any(item is not False for item in value.get("gates", {}).values())
        or value.get("controller_authentication", {}).get("security_certified")
        is not False
    ):
        raise ContractRejected("V5 config identity/gates drifted")
    return value
...
def verify_v4_rework_and_revocation() -> None:
    config = load_config()
    frozen = config["predecessor_v4"]
    named = {
        V4_REWORK: frozen["review_rework_sha256"],
        V4_REVIEW_EVIDENCE: frozen["review_evidence_sha256"],
        V4_KEY_REVOCATION: frozen["key_revocation_sha256"],
    ...
    if any(_sha_file(path) != digest for path, digest in named.items()):
        raise ContractRejected("sealed V4 REWORK/revocation bytes drifted")
    receipt = _load_json(V4_REWORK, label="V4 REWORK receipt")
    if (
        receipt.get("schema")
        != "rq2_public_grid_two_block_pilot_vnext_execution_successor_review_rework_v4"
        ...
        or receipt.get("reviewed_v4", {}).get("outer_sha256")
        != frozen["outer_sha256"]
        or receipt.get("reviewed_v4", {}).get("inner_sha256")
        != frozen["inner_sha256"]
        or receipt.get("reviewed_v4", {}).get("public_anchor_sha256")
        != frozen["public_anchor_sha256"]
        or receipt.get("reviewed_v4", {}).get("key_id") != frozen["key_id"]
        or len(receipt.get("findings", ())) != 1
        or receipt.get("findings", [{}])[0].get("finding_id")
        != "DEADLINE_MISS_TERMINATION_JOURNAL_NOT_PERSISTABLE"
        or receipt.get("findings", [{}])[0].get(
            "repeated_post_result_resource_finding"
        )
        is not False
        or receipt.get("effect", {}).get("v4_execution_authorized") is not False
        ...
        or receipt.get("effect", {}).get("new_versioned_v5_cycle_authorized")
        is not True
    ):
        raise ContractRejected("V4 REWORK receipt binding/effect drifted")
    evidence = _load_json(V4_REVIEW_EVIDENCE, label="V4 review evidence")
    revocation = _load_json(V4_KEY_REVOCATION, label="V4 key revocation")
    tombstone = _load_json(
        ROOT / frozen["revoked_tombstone_path"], label="V4 revoked tombstone"
    )
    if (
        evidence.get("authority") is not False
        or evidence.get("deadline_miss_reproducer", {}).get(
...
def verify_self_bundle() -> dict[str, str]:
    ...
    for relative in sorted(expected_members):
        ...
        if inner["files"].get(relative) != digest:
            raise ContractRejected(f"V5 bundle member hash drifted: {relative}")
        mapping[relative] = digest
    if len(mapping) != config["bundle"]["self_mapping_exact_count"]:
        raise ContractRejected("V5 self mapping count drifted")
    return dict(sorted(mapping.items()))
...
def verify_live_authorities() -> dict[str, str]:
    config = load_config()
    verify_v4_rework_and_revocation()
    try:
        predecessor_mapping = predecessor.verify_live_authorities()
    except Exception as exc:
        raise ContractRejected("sealed V4/V5 live authority drifted") from exc
    mapping = _merge_exact(
        predecessor_mapping,
        verify_self_bundle(),
    )
    if len(mapping) != config["bundle"]["live_authority_exact_count"]:
        raise ContractRejected("V5 live authority exact count drifted")
...
def load_public_key() -> dict[str, Any]:
    ...
    if (
        ...
        or value.get("schema")
        != "rq2_public_grid_vnext_execution_successor_v5_lamport_ots_public_key"
...
def lamport_sign_digest(seed: bytes, digest_sha256: str) -> dict[str, Any]:
    ...
    return {
        "schema": "rq2_public_grid_vnext_execution_v5_lamport_signature",
...
def verify_lamport_signature(
    ...
    if (
        ...
        != "rq2_public_grid_vnext_execution_v5_lamport_signature"
...
RESOURCE_JOURNAL_KEYS = {
    ...
    "termination_last_sample_to_exit_max_ns",
    "owned_termination_grace_ns",
    "deadline_missed",
    "termination",
    "status",
...
def validate_resource_journal(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        value.get("schema") != "rq2_public_grid_resource_journal_vnext_execution_v5"
        or value.get("version") != 5
        ...
        != NORMAL_LAST_SAMPLE_TO_EXIT_MAX_NS
        or value.get("termination_last_sample_to_exit_max_ns")
        != TERMINATION_LAST_SAMPLE_TO_EXIT_MAX_NS
    ...
    if deadline_missed is not None and (
        ...
        + OBSERVATION_JITTER_BUDGET_NS
        or deadline_missed["observed_monotonic_ns"]
        <= deadline_missed["sample_deadline_monotonic_ns"]
        or type(deadline_missed["observed_wall_time_ns"]) is not int
        or deadline_missed["observed_wall_time_ns"] <= 0
    ...
    if termination is not None:
        ...
        if (
            termination["schema"]
            != "rq2_public_grid_owned_termination_evidence_vnext_execution_v5"
        ...
        if (
            ...
            or (
                ...
                and request_mono < deadline_missed["observed_monotonic_ns"]
            )
        ):
    ...
    if status == "child_exited":
        ...
        if (
            ...
            or len(samples) != expected_sample_count
            or last_to_exit is None
            or not 0 <= last_to_exit <= TERMINATION_LAST_SAMPLE_TO_EXIT_MAX_NS
        ):
        ...
        if (
            ...
            not in {
                ...
                "owned_termination_exit_window_exceeded",
            }
            or value["honest_incomplete"] is not True
            or termination is None
            or (
                termination["termination_completed"]
                and termination["termination_within_grace"]
                and (
                    last_to_exit is None
                    or last_to_exit <= TERMINATION_LAST_SAMPLE_TO_EXIT_MAX_NS
                )
            )
        ):
...
def synthetic_resource_journal_for_test(
    *,
    ...
    value = {
        "schema": "rq2_public_grid_resource_journal_vnext_execution_v5",
        "version": 5,
        "owned_identity": identity,
...
def _synthetic_termination_evidence(
    *,
    ...
    return {
        "schema": "rq2_public_grid_owned_termination_evidence_vnext_execution_v5",
        "version": 1,
    ...
    *, pid: int, create_time_ns: int
    ...
    value = {
        ...
        "deadline_missed": {
            ...
            "observed_wall_time_ns": start_wall
            + OBSERVATION_JITTER_BUDGET_NS
            + 1,
            "phase": "before_sample",
            "owned_identity": identity,
    ...
    try:
        while True:
            ...
            if observed_mono > sample_deadline:
                deadline_missed = {
                    ...
                    "scheduled_monotonic_ns": scheduled,
                    "sample_deadline_monotonic_ns": sample_deadline,
                    "observed_monotonic_ns": observed_mono,
                    "observed_wall_time_ns": observed_wall,
        ...
        if termination is not None:
            ...
            if not termination["termination_completed"]:
                ...
                last_to_exit is not None
                and last_to_exit > TERMINATION_LAST_SAMPLE_TO_EXIT_MAX_NS
            ):
                status = "termination_indeterminate"
                reason = "owned_termination_exit_window_exceeded"
        outcome = {
            "schema": "rq2_public_grid_resource_journal_vnext_execution_v5",
            "version": 5,
            "owned_identity": identity,
...
def collect_solver_runtime_evidence(worker_identity: Mapping[str, int]) -> dict[str, Any]:
    ...
    value = {
        "schema": "rq2_public_grid_solver_runtime_evidence_vnext_execution_v5",
...
def validate_solver_runtime_evidence(
    ...
    if (
        ...
        or value.get("schema")
        != "rq2_public_grid_solver_runtime_evidence_vnext_execution_v5"
...
def build_worker_hello(
    *,
    ...
    v3_mapping = predecessor.predecessor.predecessor.v3.verify_full_live_closure()
    return {
        "schema": "rq2_public_grid_worker_hello_vnext_execution_v5",
        "mode": mode,
        "session_id": session_id,
        "execution_index": execution_index,
        "block_id": block_id,
        ...
        "controller_source_sha256": self_mapping[
            "experiments/run_rq2_public_grid_two_block_pilot_vnext_execution_successor_v5.py"
        ],
        "worker_source_sha256": self_mapping[
            "experiments/worker_rq2_public_grid_two_block_pilot_vnext_execution_successor_v5.py"
        ],
        ...
        "v3_closure_mapping_sha256": (
            predecessor.predecessor.predecessor.v3.closure_mapping_sha256(
                v3_mapping
            )
        ),
...
def hello_transport_context(hello: Mapping[str, Any]) -> dict[str, Any]:
    if hello.get("schema") != "rq2_public_grid_worker_hello_vnext_execution_v5":
...
def build_worker_envelope(
    *, hello: dict[str, Any], hello_raw: bytes, pipe_authority: dict[str, Any], attempt_root: str
    ...
    return {
        "schema": "rq2_public_grid_worker_envelope_vnext_execution_v5",
...
def build_worker_result(
    *,
    ...
    return {
        "schema": "rq2_public_grid_worker_result_vnext_execution_v5",
        "transport_context": hello_transport_context(hello),
        "hello_sha256": sha256_bytes(hello_raw),
...
def build_worker_exit_notice(
    *, hello: dict[str, Any], envelope: dict[str, Any], result: dict[str, Any], result_raw: bytes, result_path: str
    ...
    return {
        "schema": "rq2_public_grid_worker_exit_notice_vnext_execution_v5",
        "transport_context": hello_transport_context(hello),
...
def build_attempt_receipt(
    *, hello: dict[str, Any], envelope: dict[str, Any], result: dict[str, Any], result_raw: bytes,
    ...
    return {
        "schema": "rq2_public_grid_attempt_receipt_vnext_execution_v5",
        "transport_context": hello_transport_context(hello),
        "hello_sha256": result["hello_sha256"],
        "envelope_sha256": result["envelope_sha256"],
...
def build_controller_ack(
    *, hello: dict[str, Any], envelope: dict[str, Any], result: dict[str, Any], result_raw: bytes,
    ...
    return {
        "schema": "rq2_public_grid_controller_ack_vnext_execution_v5",
        "transport_context": hello_transport_context(hello),
        "hello_sha256": result["hello_sha256"],
        "envelope_sha256": result["envelope_sha256"],
        "exit_notice_sha256": sha256_bytes(exit_notice_raw),
...
def validate_execution_review_object(value: object) -> None:
    config = load_config()
    if not isinstance(value, dict):
        raise ContractRejected("V5 execution-review receipt malformed")
    expected_keys = {
        "schema", "version", "reviewed_on", "reviewer_role", "verdict",
        "reviewed_outer", "bound_v4_outer", "bound_v4_rework",
        "bound_v4_public_key", "bound_public_key", "findings", "effect"
    }
    if (
        ...
        or value.get("bound_v4_outer")
        != {"path": config["predecessor_v4"]["outer_path"], "sha256": config["predecessor_v4"]["outer_sha256"]}
        or value.get("bound_v4_rework")
        != {"path": config["predecessor_v4"]["review_rework_path"], "sha256": config["predecessor_v4"]["review_rework_sha256"]}
        or value.get("bound_v4_public_key")
        != {"path": config["predecessor_v4"]["public_anchor_path"], "sha256": config["predecessor_v4"]["public_anchor_sha256"], "key_id": config["predecessor_v4"]["key_id"]}
        ...
        raise ContractRejected("V5 execution-review receipt binding/effect drifted")


def require_execution_review() -> dict[str, Any]:
    if not REVIEW.exists() or REVIEW.is_symlink():
        raise ContractRejected("fixed V5 execution-review PASS receipt is absent")
    raw = read_stable(REVIEW)
    try:
        value = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise ContractRejected("V5 execution-review receipt JSON malformed") from exc
    validate_execution_review_object(value)
    if read_stable(REVIEW) != raw:
        raise ContractRejected("V5 execution-review receipt changed during validation")
    return value
...
def run_actual_science(block_id: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    return predecessor.predecessor.predecessor.run_actual_science(block_id)
...
def validate_actual_science_payload(
    ...
    require_execution_review()
    verify_live_authorities()
    return predecessor.predecessor.predecessor.validate_actual_science_payload(
        payload, block_id
    )
...
def resource_primitives() -> Any:
    verify_live_authorities()
    return predecessor.predecessor.predecessor.resource_authority()
...
def build_attestation_payload(
    *, session_id: str, entries: Mapping[str, bytes], resource_journals: Mapping[str, dict[str, Any]],
    ...
    return {
        "schema": "rq2_public_grid_controller_attestation_payload_vnext_execution_v5",
        "session_id": session_id,
...
def build_attestation(payload: dict[str, Any], seed: bytes | bytearray) -> dict[str, Any]:
    ...
    return {
        "schema": "rq2_public_grid_controller_attestation_vnext_execution_v5",
        "public_key": load_public_key(),
        "payload": payload,
...
def verify_attestation(value: Mapping[str, Any]) -> dict[str, Any]:
    if (
        ...
        or value.get("schema")
        != "rq2_public_grid_controller_attestation_vnext_execution_v5"
...
def verify_published_artifacts(
    ...
    """Independently verify V5 publication using only sealed public material."""
    from experiments import (
    ...
    try:
        ...
        raise ContractRejected("published V5 artifact missing or malformed") from exc
    if result_tree != publisher.typed_tree(result) or success_tree != publisher.typed_tree(success):
        raise ContractRejected("published V5 typed tree drifted")
    ...
    for execution_index, block in enumerate(BLOCKS, start=1):
        ...
        if (
            evidence.get("schema")
            != "rq2_public_grid_accepted_evidence_vnext_execution_v5"
    ...
    controller_receipt_body = {
        "schema": "rq2_public_grid_controller_receipt_vnext_execution_v5",
    ...
    expected_summary = {
        "schema": "rq2_public_grid_nonformal_two_block_summary_vnext_execution_v5",
        "blocks": list(BLOCKS),
        "record_count": 2,
        "scientific_payload_sha256s": science_hashes,
        "solver_call_accounting_sha256s": accounting_hashes,
    ...
    expected_result_core = {
        "schema": "rq2_public_grid_result_manifest_substantive_core_vnext_execution_v5",
        "session_id": payload["session_id"],
    ...
    expected_success_core = {
        "schema": "rq2_public_grid_success_readback_substantive_core_vnext_execution_v5",
        "session_id": payload["session_id"],
    ...
    expected_result = {
        "schema": "rq2_public_grid_result_manifest_vnext_execution_v5",
    ...
    expected_success = {
        "schema": "rq2_public_grid_success_commit_vnext_execution_v5",
    ...
    if publisher.classify_publication(presence, result_exact=True, success_exact=True) != "committed_success":
        raise ContractRejected("published V5 presence classification drifted")
    for path in result.rglob("*.json"):
        if '"seed_hex"' in read_stable(path).decode("utf-8"):
            raise ContractRejected("private seed leaked into V5 publication")
    if '"seed_hex"' in read_stable(success / "success.json").decode("utf-8"):
        raise ContractRejected("private seed leaked into V5 success readback")
    return {
        "classification": "committed_success",
