...
from experiments import (
    rq2_public_grid_highs_formal_activation_contract_v4 as authority,
)
from experiments import (
...
MODULE = (
    "experiments."
    "run_rts_gmlc_public_grid_need_dispatch_v4_process_isolated_formal_v5"
)
REQUEST_SCHEMA = "rq2_grid_need_process_isolated_worker_request_formal_v5"
RELEASE_SCHEMA = "rq2_grid_need_process_isolated_worker_release_formal_v5"
...
def _verify_execution_boundary(label: str) -> dict[str, Any]:
    ...
    if evidence.get("expected_hashes_verified") is not True:
...
def _implementation_bindings() -> dict[str, dict[str, str]]:
    authority.verify_execution_closure()
    closure = authority._load_json(authority.CLOSURE, "execution closure manifest")
...
def _require_consumed_controller_authority(dynamic_authority: Path) -> dict[str, Any]:
    ...
    if (
        ...
        != "rq2_public_grid_highs_formal_activation_successor_v4_consumed_authority"
...
def _startup_bindings(dynamic_authority: Path) -> dict[str, dict[str, str]]:
    ...
    paths = {
        ...
        "execution_closure": authority.CLOSURE,
        "activation_review_pass": authority.ACTIVATION_REVIEW_PASS,
        "user_formal_run_authority": authority.USER_FORMAL_RUN_AUTHORITY,
        "dynamic_authority": dynamic_authority.resolve(),
...
def _complete_startup_handshake(
    ...
    *,
    ...
    observed_paths = {
        ...
        "post_release_unresolved": expected_paths["post_release_unresolved"],
        "controller_terminal_success": expected_paths["controller_terminal_success"],
    }
...
def _dispatch_one(
    ...
    *,
    ...
    controller_receipt = {
        "schema": "rq2_grid_need_process_isolated_attempt_formal_v5",
        "block_id": block_id,
        "request_sha256": authority.sha256_file(request_path),
...
def validate_only(config_path: Path = CONFIG) -> dict[str, Any]:
    closure = _verify_execution_boundary("controller_validate_only")
    ...
    return {
        "schema": "rq2_grid_need_process_isolated_formal_preflight_v4",
...
def run(
    ...
    *,
    ...
    controller_identity = authority._process_identity(
        startup["release_accepted"]["controller_identity"], "controller identity"
    )
    ...
    try:
        ...
        authority.persist_controller_terminal_success(
            ...
            controller_identity=controller_identity,
            release_path=release_path,
            release_acceptance=startup["release_accepted"],
            result=result,
        )
        return result
    except BaseException as exc:
        try:
            authority.persist_post_release_unresolved(
                attempt_root,
