"""Fail-closed evidence contract for isolated Vnext successor V7.

V7 changes only resource-audit evidence: deadline classification is bound to
the observed detection overrun and exact-owned termination duration, and the
monitor persists and reads back its journal before its future resolves.  The
scientific protocol, thresholds, order, solver semantics, and security status
remain unchanged.
"""
...
from experiments import (
    rq2_public_grid_two_block_pilot_vnext_execution_contract_v6 as predecessor,
)

ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v7.json"
INNER = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v7.SHA256SUMS.json"
OUTER = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v7.OUTER.SHA256SUMS.json"
REVIEW = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_review_pass_v7.json"
PUBLIC_KEY = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v7_ots_public_key.json"
V6_REGRESSION_EVIDENCE = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_regression_evidence_v6.json"
V6_KEY_REVOCATION = ROOT / "configs/rq2_public_grid_two_block_pilot_vnext_execution_successor_v6_ots_key_revocation_v1.json"
...
def load_config() -> dict[str, Any]:
    value = _load_json(CONFIG, label="V7 config")
    resource = value.get("resource_contract", {})
    if (
        value.get("schema")
        != "rq2_public_grid_two_block_pilot_vnext_execution_successor_v7"
        or value.get("version") != 7
        or value.get("status")
        != "vnext_execution_successor_v7_ready_for_independent_review"
        or tuple(value.get("pilot", {}).get("blocks", ())) != BLOCKS
        or value.get("remediation_scope")
        != "new_isolated_versioned_v7_resource_sampler_prebinding_not_v6_mutation_or_resume"
        ...
        raise ContractRejected("V7 config identity/gates drifted")
    return value
...
def verify_v6_regression_and_revocation() -> None:
    config = load_config()
    frozen = config["predecessor_v6"]
    named = {
        V6_REGRESSION_EVIDENCE: frozen["regression_evidence_sha256"],
        V6_KEY_REVOCATION: frozen["key_revocation_sha256"],
    ...
    if any(_sha_file(path) != digest for path, digest in named.items()):
        raise ContractRejected("sealed V6 regression/revocation bytes drifted")
    evidence = _load_json(V6_REGRESSION_EVIDENCE, label="V6 regression evidence")
    if (
        evidence.get("schema")
        != "rq2_public_grid_two_block_pilot_vnext_execution_successor_regression_evidence_v6"
        or evidence.get("version") != 1
        or evidence.get("authority") is not False
        or evidence.get("reviewer_verdict") is not None
        or evidence.get("reviewed_v6", {}).get("outer_sha256")
        != frozen["outer_sha256"]
        or evidence.get("reviewed_v6", {}).get("inner_sha256")
        != frozen["inner_sha256"]
        or evidence.get("reviewed_v6", {}).get("public_anchor_sha256")
        != frozen["public_anchor_sha256"]
        or evidence.get("reviewed_v6", {}).get("key_id") != frozen["key_id"]
        or evidence.get("finding", {}).get("finding_id")
        != "RESOURCE_AUTHORITY_HASHING_INSIDE_SAMPLE_DEADLINE"
        or evidence.get("finding", {}).get("captured_status")
        != "resource_sample_deadline_missed"
        or evidence.get("finding", {}).get("captured_phase")
        != "sample_completion"
        or evidence.get("effect", {}).get("v6_execution_authorized") is not False
        or evidence.get("effect", {}).get("v7_versioned_cycle_only") is not True
    ):
        raise ContractRejected("V6 regression evidence binding/effect drifted")
    revocation = _load_json(V6_KEY_REVOCATION, label="V6 key revocation")
    tombstone = _load_json(
        ROOT / frozen["revoked_tombstone_path"], label="V6 revoked tombstone"
    )
    if (
        revocation.get("effect", {}).get("state") != "REVOKED"
...
def verify_self_bundle() -> dict[str, str]:
    ...
    for relative in sorted(expected_members):
        ...
        if inner["files"].get(relative) != digest:
            raise ContractRejected(f"V7 bundle member hash drifted: {relative}")
        mapping[relative] = digest
    if len(mapping) != config["bundle"]["self_mapping_exact_count"]:
        raise ContractRejected("V7 self mapping count drifted")
    return dict(sorted(mapping.items()))
...
def verify_live_authorities() -> dict[str, str]:
    config = load_config()
    verify_v6_regression_and_revocation()
    try:
        predecessor_mapping = predecessor.verify_live_authorities()
    except Exception as exc:
        raise ContractRejected("sealed V4/V7 live authority drifted") from exc
    mapping = _merge_exact(
        predecessor_mapping,
        verify_self_bundle(),
    )
    if len(mapping) != config["bundle"]["live_authority_exact_count"]:
        raise ContractRejected("V7 live authority exact count drifted")
...
def load_public_key() -> dict[str, Any]:
    ...
    if (
        ...
        or value.get("schema")
        != "rq2_public_grid_vnext_execution_successor_v7_lamport_ots_public_key"
...
def lamport_sign_digest(seed: bytes, digest_sha256: str) -> dict[str, Any]:
    ...
    return {
        "schema": "rq2_public_grid_vnext_execution_v7_lamport_signature",
...
def verify_lamport_signature(
    ...
    if (
        ...
        != "rq2_public_grid_vnext_execution_v7_lamport_signature"
...
def _validate_resource_sampler_binding(
    ...
    if (
        value.get("schema")
        != "rq2_public_grid_resource_sampler_authority_binding_vnext_execution_v7"
        or value.get("version") != 1
        or any(
...
def validate_resource_journal(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        value.get("schema") != "rq2_public_grid_resource_journal_vnext_execution_v7"
        or value.get("version") != 7
    ...
    if termination is not None:
        ...
        if (
            termination["schema"]
            != "rq2_public_grid_owned_termination_evidence_vnext_execution_v7"
...
def synthetic_resource_journal_for_test(
    *,
    ...
    value = {
        "schema": "rq2_public_grid_resource_journal_vnext_execution_v7",
        "version": 7,
        "owned_identity": identity,
...
def _synthetic_termination_evidence(
    *,
    ...
    return {
        "schema": "rq2_public_grid_owned_termination_evidence_vnext_execution_v7",
        "version": 1,
...
def validate_resource_monitor_outcome(
    ...
    if (
        value.get("schema")
        != "rq2_public_grid_resource_monitor_persisted_outcome_vnext_execution_v7"
        or value.get("version") != 1
...
def _validate_resource_sampler_prebinding(
    ...
    if (
        value.get("schema")
        != "rq2_public_grid_resource_sampler_prebinding_vnext_execution_v7"
        or value.get("version") != 1
        or any(
            type(item) is not int or item <= 0
            for item in (start_wall, start_mono, completed_wall, completed_mono)
...
def collect_solver_runtime_evidence(worker_identity: Mapping[str, int]) -> dict[str, Any]:
    ...
    value = {
        "schema": "rq2_public_grid_solver_runtime_evidence_vnext_execution_v7",
...
def validate_solver_runtime_evidence(
    ...
    if (
        ...
        or value.get("schema")
        != "rq2_public_grid_solver_runtime_evidence_vnext_execution_v7"
...
def build_worker_hello(
    *,
    ...
    v3_mapping = (
        predecessor.predecessor.predecessor.predecessor.predecessor.v3.verify_full_live_closure()
    )
    return {
        "schema": "rq2_public_grid_worker_hello_vnext_execution_v7",
        "mode": mode,
        "session_id": session_id,
        "execution_index": execution_index,
        "block_id": block_id,
        ...
        "controller_source_sha256": self_mapping[
            "experiments/run_rq2_public_grid_two_block_pilot_vnext_execution_successor_v7.py"
        ],
        "worker_source_sha256": self_mapping[
            "experiments/worker_rq2_public_grid_two_block_pilot_vnext_execution_successor_v7.py"
        ],
        ...
        "v3_closure_mapping_sha256": (
            predecessor.predecessor.predecessor.predecessor.predecessor.v3.closure_mapping_sha256(
                v3_mapping
            )
        ),
...
def hello_transport_context(hello: Mapping[str, Any]) -> dict[str, Any]:
    if hello.get("schema") != "rq2_public_grid_worker_hello_vnext_execution_v7":
...
def build_worker_envelope(
    *, hello: dict[str, Any], hello_raw: bytes, pipe_authority: dict[str, Any], attempt_root: str
    ...
    return {
        "schema": "rq2_public_grid_worker_envelope_vnext_execution_v7",
...
def build_worker_result(
    *,
    ...
    return {
        "schema": "rq2_public_grid_worker_result_vnext_execution_v7",
        "transport_context": hello_transport_context(hello),
        "hello_sha256": sha256_bytes(hello_raw),
...
def build_worker_exit_notice(
    *, hello: dict[str, Any], envelope: dict[str, Any], result: dict[str, Any], result_raw: bytes, result_path: str
    ...
    return {
        "schema": "rq2_public_grid_worker_exit_notice_vnext_execution_v7",
        "transport_context": hello_transport_context(hello),
...
def build_attempt_receipt(
    *, hello: dict[str, Any], envelope: dict[str, Any], result: dict[str, Any], result_raw: bytes,
    ...
    return {
        "schema": "rq2_public_grid_attempt_receipt_vnext_execution_v7",
        "transport_context": hello_transport_context(hello),
        "hello_sha256": result["hello_sha256"],
        "envelope_sha256": result["envelope_sha256"],
...
def build_controller_ack(
    *, hello: dict[str, Any], envelope: dict[str, Any], result: dict[str, Any], result_raw: bytes,
    ...
    return {
        "schema": "rq2_public_grid_controller_ack_vnext_execution_v7",
        "transport_context": hello_transport_context(hello),
        "hello_sha256": result["hello_sha256"],
        "envelope_sha256": result["envelope_sha256"],
        "exit_notice_sha256": sha256_bytes(exit_notice_raw),
...
def validate_execution_review_object(value: object) -> None:
    config = load_config()
    if not isinstance(value, dict):
        raise ContractRejected("V7 execution-review receipt malformed")
    expected_keys = {
        "schema", "version", "reviewed_on", "reviewer_role", "verdict",
        "reviewed_outer", "bound_v6_outer", "bound_v6_regression_evidence",
        "bound_v6_public_key", "bound_public_key", "findings", "effect"
    }
    if (
        ...
        or value.get("bound_v6_outer")
        != {"path": config["predecessor_v6"]["outer_path"], "sha256": config["predecessor_v6"]["outer_sha256"]}
        or value.get("bound_v6_regression_evidence")
        != {"path": config["predecessor_v6"]["regression_evidence_path"], "sha256": config["predecessor_v6"]["regression_evidence_sha256"]}
        or value.get("bound_v6_public_key")
        != {"path": config["predecessor_v6"]["public_anchor_path"], "sha256": config["predecessor_v6"]["public_anchor_sha256"], "key_id": config["predecessor_v6"]["key_id"]}
        ...
        raise ContractRejected("V7 execution-review receipt binding/effect drifted")


def require_execution_review() -> dict[str, Any]:
    if not REVIEW.exists() or REVIEW.is_symlink():
        raise ContractRejected("fixed V7 execution-review PASS receipt is absent")
    raw = read_stable(REVIEW)
    try:
        value = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise ContractRejected("V7 execution-review receipt JSON malformed") from exc
    validate_execution_review_object(value)
    if read_stable(REVIEW) != raw:
        raise ContractRejected("V7 execution-review receipt changed during validation")
    return value
...
def run_actual_science(block_id: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    return predecessor.predecessor.predecessor.predecessor.predecessor.run_actual_science(block_id)
...
def validate_actual_science_payload(
    ...
    require_execution_review()
    verify_live_authorities()
    return predecessor.predecessor.predecessor.predecessor.predecessor.validate_actual_science_payload(
        payload, block_id
    )
...
def resource_primitives() -> Any:
    verify_live_authorities()
    return predecessor.predecessor.predecessor.predecessor.predecessor.resource_authority()
...
def build_attestation_payload(
    *, session_id: str, entries: Mapping[str, bytes], resource_journals: Mapping[str, dict[str, Any]],
    ...
    return {
        "schema": "rq2_public_grid_controller_attestation_payload_vnext_execution_v7",
        "session_id": session_id,
...
def build_attestation(payload: dict[str, Any], seed: bytes | bytearray) -> dict[str, Any]:
    ...
    return {
        "schema": "rq2_public_grid_controller_attestation_vnext_execution_v7",
        "public_key": load_public_key(),
        "payload": payload,
...
def verify_attestation(value: Mapping[str, Any]) -> dict[str, Any]:
    if (
        ...
        or value.get("schema")
        != "rq2_public_grid_controller_attestation_vnext_execution_v7"
...
def verify_published_artifacts(
    ...
    """Independently verify V7 publication using only sealed public material."""
    from experiments import (
    ...
    try:
        ...
        raise ContractRejected("published V7 artifact missing or malformed") from exc
    if result_tree != publisher.typed_tree(result) or success_tree != publisher.typed_tree(success):
        raise ContractRejected("published V7 typed tree drifted")
    ...
    for execution_index, block in enumerate(BLOCKS, start=1):
        ...
        if (
            evidence.get("schema")
            != "rq2_public_grid_accepted_evidence_vnext_execution_v7"
    ...
    controller_receipt_body = {
        "schema": "rq2_public_grid_controller_receipt_vnext_execution_v7",
    ...
    expected_summary = {
        "schema": "rq2_public_grid_nonformal_two_block_summary_vnext_execution_v7",
        "blocks": list(BLOCKS),
        "record_count": 2,
        "scientific_payload_sha256s": science_hashes,
        "solver_call_accounting_sha256s": accounting_hashes,
    ...
    expected_result_core = {
        "schema": "rq2_public_grid_result_manifest_substantive_core_vnext_execution_v7",
        "session_id": payload["session_id"],
    ...
    expected_success_core = {
        "schema": "rq2_public_grid_success_readback_substantive_core_vnext_execution_v7",
        "session_id": payload["session_id"],
    ...
    expected_result = {
        "schema": "rq2_public_grid_result_manifest_vnext_execution_v7",
    ...
    expected_success = {
        "schema": "rq2_public_grid_success_commit_vnext_execution_v7",
    ...
    if publisher.classify_publication(presence, result_exact=True, success_exact=True) != "committed_success":
        raise ContractRejected("published V7 presence classification drifted")
    for path in result.rglob("*.json"):
        if '"seed_hex"' in read_stable(path).decode("utf-8"):
            raise ContractRejected("private seed leaked into V7 publication")
    if '"seed_hex"' in read_stable(success / "success.json").decode("utf-8"):
        raise ContractRejected("private seed leaked into V7 success readback")
    return {
        "classification": "committed_success",
