...
param(
    ...
    [string]$Config = "configs/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4_repair_007.yaml",
...
if (-not $AttemptId) {
    $Prefix = if ($Stage -eq "run-joint-ac") { "joint_repair_007_" } else { "formal_repair_007_" }
...
$LogRoot = Join-Path $ProjectRoot "results/logs/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4_repair_007"
...
$OutputRoot = Join-Path $ProjectRoot "results/tables/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4_repair_007"
...
if (-not $DryRun -and -not (Test-Path -LiteralPath $PreregistrationManifest)) {
    throw "repair-007 preregistration is missing: $PreregistrationManifest"
}
if (-not $DryRun -and $Stage -eq "run-joint-ac" -and -not (Test-Path -LiteralPath $CandidateFrontierManifest)) {
    throw "repair-007 candidate frontier is missing: $CandidateFrontierManifest"
}
...
$Record = [ordered]@{
    schema = "rts_gmlc_v4_repair_007_launcher_v1"
