...
param(
    ...
    [string]$Config = "configs/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4_repair_004.yaml",
...
if (-not $AttemptId) {
    $Prefix = if ($Stage -eq "run-joint-ac") { "joint_repair_004_" } else { "formal_repair_004_" }
...
$LogRoot = Join-Path $ProjectRoot "results/logs/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4_repair_004"
...
$OutputRoot = Join-Path $ProjectRoot "results/tables/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4_repair_004"
...
if (-not $DryRun -and -not (Test-Path -LiteralPath $PreregistrationManifest)) {
    throw "repair-004 preregistration is missing: $PreregistrationManifest"
}
if (-not $DryRun -and $Stage -eq "run-joint-ac" -and -not (Test-Path -LiteralPath $CandidateFrontierManifest)) {
    throw "repair-004 candidate frontier is missing: $CandidateFrontierManifest"
}
...
$Record = [ordered]@{
    schema = "rts_gmlc_v4_repair_004_launcher_v1"
