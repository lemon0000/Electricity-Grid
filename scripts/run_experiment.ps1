...
.DESCRIPTION
    ...
    若要跑正式多阶段实验，请改用 docs/plan/科研实验闭环流程.md 第 3.5 节“两类实验入口”
    所述的专用 runner，不要把它塞进本通用脚本。
...
try {
    ...
    if ($kind -ne "pytest-smoke") {
        throw "本固定入口只支持 kind=pytest-smoke；kind=$kind 属正式实验，需使用专用 runner（见流程文档第 3.5 节）。"
    }
    ...
    $pytestArgs = @("-m", "pytest") + $selection + $extraArgs
    ...
    $proc = Start-Process -FilePath $pythonExe -ArgumentList $pytestArgs `
    ...
    if (-not $proc.WaitForExit($timeoutSeconds * 1000)) {
        ...
        $runStatus = "timeout"
        $exitCode = 124
        $failureMessage = "pytest 冒烟超时（>$timeoutSeconds 秒），已终止。"
    }
    else {
        ...
        if ($procExit -eq 0) {
            ...
            $runStatus = "failed"
            $exitCode = $procExit
            $failureMessage = "pytest 冒烟失败，退出码：$procExit"
        }
    }
}
catch {
    ...
    # 从 pytest 日志抽取通过/失败计数（用 $script: 回写脚本作用域，避免 & 子作用域丢值）。
    Try-Step {
        $logPath = Join-Path $runDir "experiment.log"
    ...
    # metrics.json —— 冒烟只记录测试计数；失败/超时时为 null。
    Try-Step {
        $json = [PSCustomObject]@{
    ...
    # summary.md —— 人类可读运行摘要，明确其非科研结论性质。
    Try-Step {
        Write-Utf8Lines (Join-Path $runDir "summary.md") @(
            "# Run $runId"
            ""
            "- source_tag: $runTag"
            "- source_commit: $runCommit"
            ...
            ""
            "此摘要仅描述跨机闭环管道的冒烟运行状态，不构成模型可行性、安全认证或科研结论。"
        )
    }
