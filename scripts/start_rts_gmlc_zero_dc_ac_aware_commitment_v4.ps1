...
param(
    ...
    [string]$Config = "configs/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4.yaml",
...
if (-not $AttemptId) {
    $Prefix = if ($Stage -eq "run-joint-ac") { "joint_" } else { "formal_" }
...
$LogRoot = Join-Path $ProjectRoot "results/logs/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4"
...
$PreregistrationManifest = Join-Path $ProjectRoot "results/tables/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4/preregistration/SHA256SUMS"
$CandidateFrontierManifest = Join-Path $ProjectRoot "results/tables/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4/candidate_frontier/SHA256SUMS"
...
if (-not $DryRun -and -not (Test-Path -LiteralPath $PreregistrationManifest)) {
    throw "V4 preregistration is missing: $PreregistrationManifest"
}
if (-not $DryRun -and $Stage -eq "run-joint-ac" -and -not (Test-Path -LiteralPath $CandidateFrontierManifest)) {
    throw "V4 candidate frontier is missing: $CandidateFrontierManifest"
}
...
$Record = [ordered]@{
    schema = "rts_gmlc_v4_launcher_v2"
...
$RequestJson = $Record | ConvertTo-Json -Depth 3
[System.IO.File]::WriteAllText($RequestPath, $RequestJson + [Environment]::NewLine, $Utf8NoBom)
...
$StartedJson = $Record | ConvertTo-Json -Depth 3
[System.IO.File]::WriteAllText($StartedPath, $StartedJson + [Environment]::NewLine, $Utf8NoBom)
$StartedJson
