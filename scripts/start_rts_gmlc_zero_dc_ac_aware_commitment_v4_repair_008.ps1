...
param(
    ...
    [string]$Config = "configs/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4_repair_008.yaml",
...
if (-not $AttemptId) {
    $Prefix = if ($Stage -eq "run-joint-ac") { "joint_repair_008_" } else { "formal_repair_008_" }
...
$LogRoot = Join-Path $ProjectRoot "results/logs/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4_repair_008"
...
$OutputRoot = Join-Path $ProjectRoot "results/tables/rts_gmlc_google_day0_zero_dc_ac_aware_commitment_v4_repair_008"
...
if (-not $DryRun -and -not (Test-Path -LiteralPath $PreregistrationManifest)) {
    throw "repair-008 preregistration is missing: $PreregistrationManifest"
}
if (-not $DryRun -and $Stage -eq "run-joint-ac" -and -not (Test-Path -LiteralPath $CandidateFrontierManifest)) {
    throw "repair-008 candidate frontier is missing: $CandidateFrontierManifest"
}
...
$Record = [ordered]@{
    schema = "rts_gmlc_v4_repair_008_launcher_v1"
