@echo off
REM Repair-009 ifocus4 run-joint-ac (ASCII only).
REM Requires published candidate_frontier under ifocus4 output root.
...
set LOG=%LOG_DIR%\joint_ac_%STAMP%.log

echo Starting run-joint-ac
echo CONFIG=%CONFIG%
echo OUTPUT_ROOT=%OUTPUT_ROOT%
echo LOG=%LOG%

"%PYTHON%" -u -B "%RUNNER%" --config "%CONFIG%" --stage run-joint-ac 1>"%LOG%" 2>&1
