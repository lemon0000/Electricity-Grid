...
STAGES = frozenset({"proxy_maximization", "cost_normalization"})
...
def _stable_scalar(item: object) -> object:
    if isinstance(item, dict):
        return {str(key): _stable_scalar(value_) for key, value_ in sorted(item.items())}
...
def apply_shared_snapshot(model: Any, snapshot: SharedSnapshot) -> None:
    ...
    source = {(component, index): number for component, index, number in snapshot.values}
    ...
    *,
    ...
    return {
        ...
        "relative_gap_to_incumbent": (
            interval.absolute_gap / max(abs(incumbent), 1.0e-12)
            if interval
            else None
        ),
...
def promotions(
    ...
    by_state = {str(record["state_id"]): str(record["status"]) for record in screen_records}
    promoted = []
    for state_id in all_state_ids:
        status = by_state.get(state_id)
        if status == "certified_infeasible":
            promoted.append(
                {"state_id": state_id, "reason": "certified_infeasible"}
            )
