"""Gurobi successor for repair-007: replaces HiGHS with Gurobi 4 threads (frozen by Gurobi academic pilot benchmark_v1). Inherits repair-007 checkpoints. repair-008 was deliberately interrupted to switch solvers and produced no new checkpoints."""
...
from experiments import (
    record_rts_gmlc_zero_dc_ac_aware_commitment_v4_repair_007_failure as failure_record,
)
from experiments import (
    run_rts_gmlc_zero_dc_ac_aware_commitment_v4_repair_007_formal as repair007,
)
from experiments import (
...
# repair-009: patch the frozen pilot module's _solve_handle to support Gurobi.
# This is applied at module-load time, BEFORE any hash verification runs, so the
...
CHECKPOINT_SCHEMA = "rts_gmlc_v4_repair_009_cost_bisection_candidate_checkpoint_v1"
PREFIX_EVIDENCE_SCHEMA = "rts_gmlc_v4_repair_009_prefix_import_evidence_v1"
HYBRID_EVIDENCE_SCHEMA = "rts_gmlc_v4_repair_009_hybrid_cost_evidence_v1"
COMBINED_COST_SCHEMA = "rts_gmlc_v4_repair_009_combined_cost_certificate_v1"
FRONTIER_SCHEMA = "rts_gmlc_v4_repair_009_candidate_frontier_v1"
FLOAT_SERIALIZATION = "python_json_roundtrip_full_precision_v1"
_SUCCESSOR_MODULE = (
    "experiments.run_rts_gmlc_zero_dc_ac_aware_commitment_v4_repair_009_formal"
)
_CONFIG_KEYS = {
    "schema",
    "preregistration",
...
def _read_config(config_path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        != list(failure_record.EXPECTED_CHECKPOINT_MANIFEST_SHA256S)
        or predecessor.get("candidate_json_sha256s")
        != list(failure_record.EXPECTED_CANDIDATE_JSON_SHA256S)
    ...
    for ordinal, (control, delta) in enumerate(zip(controls, expected_deltas), 1):
        ...
        if (
            ...
            raise ValueError("repair-009 candidate control order drifted")
    return config


def _verify_parent_failure(
    config: Mapping[str, Any], failure_root: Path | None
) -> None:
    """Validate repair-006 predecessor state: preregistration + checkpoints intact,
    no frontier or joint-AC published.  repair-006 terminated due to cost bisection
    unresolved (not infeasibility), so no formal failure-record file is required."""
    predecessor = config["predecessor_repair_007"]
    ...
    if _sha256(preregistration / "SHA256SUMS") != predecessor["preregistration_manifest_sha256"]:
        raise RuntimeError("repair-009 parent preregistration manifest drifted")
    ...
    if (
        ...
        raise RuntimeError("repair-009 parent preregistration content drifted")
    if (root / "candidate_frontier").exists() or (root / "joint_ac").exists():
        raise RuntimeError("repair-009 parent gained forbidden downstream evidence")
    checkpoints = root / "candidate_checkpoints"
    names = predecessor["candidate_checkpoint_names"]
    for ordinal, name in enumerate(names, 1):
        cp = checkpoints / name
        if (
            _sha256(cp / "SHA256SUMS")
            ...
            raise RuntimeError(f"repair-009 parent checkpoint {name} hash drifted")


def _source_context(config: Mapping[str, Any]) -> v4._FrontierContext:
    return repair007._build_context(
        Path(config["predecessor_repair_007"]["config_path"])
    )


_VERIFIED_SOURCE_PREFIX_CACHE: dict[str, tuple[tuple[v4._Candidate, SharedSnapshot], ...]] = {}


def _verified_source_prefix(
    config: Mapping[str, Any],
) -> tuple[tuple[v4._Candidate, SharedSnapshot], ...]:
    # Cache by input_contract_sha256 so the costly 5-level chain rebuild
    # only happens once per process, not once per call site (preregistration,
    # _verify_frozen_inputs, _validate_candidate_physics, main loop).
    cache_key = str(config.get("predecessor_repair_007", {}).get("input_contract_sha256", ""))
    if cache_key and cache_key in _VERIFIED_SOURCE_PREFIX_CACHE:
        return _VERIFIED_SOURCE_PREFIX_CACHE[cache_key]
    predecessor = config["predecessor_repair_007"]
    root = Path(predecessor["root"])
    source_context = _source_context(config)
    loaded: list[tuple[v4._Candidate, SharedSnapshot]] = []
    for ordinal in range(1, 5):
        item = repair007._load_candidate_checkpoint(source_context, root, ordinal)
        if item is None:
            raise RuntimeError("repair-009 source prefix checkpoint is missing")
        ...
        if (
            ...
            raise RuntimeError("repair-009 source prefix hash drifted")
        loaded.append((candidate, snapshot))
    result = tuple(loaded)
    if cache_key:
        _VERIFIED_SOURCE_PREFIX_CACHE[cache_key] = result
    return result


def _verify_frozen_inputs(
    config: Mapping[str, Any], *, predecessor_failure_root: Path | None = None
) -> None:
    base = config["base_v4"]
    ...
    # Verify that the Gurobi pilot module (monkey-patched at load time) matches the contract
    gurobi_pilot_path = config["implementation"].get("pilot_gurobi_module_path")
    gurobi_pilot_sha = config["implementation"].get("pilot_gurobi_module_sha256")
    if gurobi_pilot_path and gurobi_pilot_sha:
        _verify_file(Path(gurobi_pilot_path), str(gurobi_pilot_sha), "Gurobi pilot module")
...
def _build_context(
    config_path: Path, *, predecessor_failure_root: Path | None = None
) -> v4._FrontierContext:
    successor_config = _read_config(config_path)
    _verify_frozen_inputs(
        successor_config,
        predecessor_failure_root=predecessor_failure_root,
    )
    ...
    contract = {
        ...
        "predecessor_repair_007": successor_config["predecessor_repair_007"],
...
def prepare_preregistration(
    ...
    *,
    ...
    predecessor_failure_root: Path | None = None,
) -> dict[str, Any]:
    ...
    if target.exists():
        ...
        if (
            ...
            raise RuntimeError("repair-009 published preregistration drifted")
        return observed
    if output_root.exists() and any(output_root.iterdir()):
        raise RuntimeError("Cannot prepare repair-009 beside existing artifacts")
...
def _prefix_candidate(
    ...
    predecessor = context.input_contract["predecessor_repair_007"]
    evidence: dict[str, object] = {
        ...
        "source_checkpoint_validated_by_repair_008_runner": True,
    ...
    imported = replace(
        candidate,
        source="repair_009_verified_repair_008_prefix",
...
def _hybrid_candidate(
    ...
    *,
    ...
    predecessor_evidence_sha = common_input_signature_sha256(
        {
            ...
            "repair_008_proxy_successor_contract_sha256": (
                context.input_contract["predecessor_repair_007"][
        ...
        return run_exact_cg_stage(
            ...
            time_limits=repair004.ExactCgTimeLimits(
                master_seconds=float(
                    context.input_contract["predecessor_repair_007"].get(
    ...
    parent_successor = repair007._read_config(
        Path(context.input_contract["predecessor_repair_007"]["config_path"])
    )["formal_successor"]
    ...
    proxy_evidence: dict[str, object] = {
        "schema": repair007.HYBRID_EVIDENCE_SCHEMA,
...
def _validate_candidate_physics(
    ...
    if ordinal <= 4:
        # SHA256 of candidate.json was already verified at import time (source_checkpoint_imported).
        # Rebuilding the full 5-level chain here to re-derive the same bytes is O(n²) and
        # adds ~3.7 hours per candidate.  The identity check above is sufficient.
        return
    source_context = _source_context(context.input_contract)
    repair007._validate_candidate_physics(candidate, source_context, ordinal)
...
def _validate_persisted_cost_audit(
    ...
    *,
    ...
    ``proxy_tolerance`` gates identity between commitment-capability proxy,
    snapshot proxy, and the candidate's stored proxy. ``actual_proxy_tolerance``
    gates the continuous re-solve's ``actual_proxy_fraction`` and must match the
    feasibility tolerance used by ``FormalCgModelAdapter.audit_full_state`` when
    it accepted the audit; using the tighter floor tolerance here rejects
    audits the solver path already passed (repair-009 ifocus2 candidate 5).
    """
...
def _validate_proxy_checkpoint_evidence(
    ...
    *,
    ...
    if (
        proxy.get("schema") != repair007.HYBRID_EVIDENCE_SCHEMA
...
def _validate_checkpoint_document(
    ...
    if ordinal <= 4:
        ...
        if (
            ...
            != predecessor["candidate_json_sha256s"][ordinal - 1]
            or evidence.get("source_checkpoint_validated_by_repair_008_runner")
            is not True
...
def _run_joint_ac_attempt(
    ...
    original = repair007._joint_worker_command
    repair007._joint_worker_command = _joint_worker_command
    try:
        return repair007._run_joint_ac_attempt(
            context,
            output_root,
            registration,
            candidates,
        ...
        repair007._joint_worker_command = original
