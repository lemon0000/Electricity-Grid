...
def _read_config(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or cg.get("initial_active_state_source")
        != "parent_zero_final_active_state_ids"
        ...
        or cg.get("promote_statuses")
        != ["certified_infeasible", "unresolved"]
...
def _verify_provenance(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for raw_path, expected in path_hashes.items():
        ...
        if not path.is_file() or _sha256(path) != expected:
            raise RuntimeError(f"Frozen formulation-pilot dependency drifted: {raw_path}")
    ...
    termination = _load_json(
        v2_root / "operational_termination", "termination.json"
    )
    if (
        registration.get("input_contract_sha256")
        != frozen["v2_input_contract_sha256"]
...
def prepare(
    ...
    def writer(staging: Path) -> None:
        ...
        artifact_paths = (
            ...
            Path(frozen["solver_benchmark_root"])
            / "preparation"
            / "SHA256SUMS",
            Path(frozen["solver_benchmark_root"])
            / "benchmark"
...
def _build_problem(config: Mapping[str, Any]) -> _Problem:
    ...
    budget = horizon_cost * (
        1.0 + float(config["pilot"]["relative_cost_budget_delta"])
    )
    return _Problem(
...
def _build_model_handle(
    ...
    *,
    ...
    model = _build_model(scuc_context, fixed_initial=problem.parent_context.initial_state)
    ...
    model.cost_cap = Constraint(
        ...
        <= problem.cost_budget_usd
        + float(config["budget"]["cost_cap_absolute_tolerance_usd"])
    )
    ...
    if stage == "proxy_maximization":
        ...
        if proxy_floor is None or not math.isfinite(float(proxy_floor)):
            raise ValueError("Cost stage requires a finite frozen proxy floor")
...
def _screen_state(
    ...
    *,
    ...
    if feasible:
        status = "feasible"
    elif solved["termination_condition"] == str(TerminationCondition.infeasible):
...
def _commitment_proxy_from_snapshot(
    ...
    values = {(component, index): number for component, index, number in snapshot.values}
    thermal_uids = tuple(
        generator.uid
...
def _run_monolith(
    ...
    *,
    ...
    eligible = bool(full_audit and solved["bound_valid"] and certificate["target_gap_attained"])
...
def _select_formulation(
    ...
    for formulation in formulations:
        runs = [
            ...
            if str(record["formulation"]) == formulation
        ]
    ...
    selected = (
        str(eligible_summaries[0]["formulation"]) if eligible_summaries else None
    )
    return {
        "status": (
...
def run_pilot(
    ...
    def writer(staging: Path) -> None:
        ...
        summary = {
            ...
            "relative_cost_budget_delta": config["pilot"][
            ...
            "target_mip_relative_gap": config["solver"][
