...
def _read_config(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        != "rts_gmlc_google_day0_zero_dc_ac_aware_warmstart_benchmark_v1"
        or preregistration.get("formal_candidate_result") is not False
        or preregistration.get("joint_ac_solver_call_count") != 0
        or preregistration.get("v3_runtime_outcome_used_to_motivate_pilot")
        is not True
...
def _state_angles(
    ...
    if set(angles) != {int(bus) for bus in handle.model.BUS}:
        raise RuntimeError(f"Angle reconstruction failed for {state.state_id} {time_index}")
    return angles
...
def _assign_parent_start(
    ...
    timestamps = tuple(timestamp.isoformat() for timestamp in problem.request.timestamps)
    ...
    for time_index, timestamp in enumerate(timestamps):
        ...
        for state_id in handle.state_ids:
            if state_id == "normal":
                continue
            for uid in model.GEN:
                row = maps["security_generator"][
                    (timestamp, state_id, str(uid))
                ]
        ...
        for time_index, timestamp in enumerate(timestamps):
            ...
            if state.response_mode == "fixed" or state_id == "normal":
                ...
                ac_export = sum(
                    ...
                    flows[str(uid)] for uid in context.incoming_branches[from_bus]
                )
                dc_value = (
                    bus_generation
...
def prepare(
    ...
    payload = {
        "schema": "rts_gmlc_zero_dc_ac_aware_warmstart_preparation_v1",
    ...
    if target.exists():
        ...
        observed = json.loads((target / "registration.json").read_text(encoding="utf-8"))
...
def _install_audited_appsi_warm_start(solver: Any) -> None:
    def warm_start(instance: Any) -> None:
        ...
        for var_id, column in instance._pyomo_var_to_solver_var_map.items():
            ...
            candidate = value(variable, exception=False)
            if candidate is None or not math.isfinite(float(candidate)):
                raise RuntimeError(f"Missing appsi warm-start value for {variable.name}")
...
def _parse_native_log(
    ...
    accepted_lines = [line.strip() for line in text.splitlines() if acceptance_text in line]
    rejected_lines = [line.strip() for line in text.splitlines() if rejection_text in line]
    ...
    for line in text.splitlines():
        tokens = line.split()
        source_offset = 1 if tokens and len(tokens[0]) == 1 and tokens[0].isalpha() else 0
        if (
...
def _solve_run(
    ...
    incumbent_usable = formulation._incumbent_is_usable(
        ...
        variables_finite=(formulation._all_variables_finite(handle.model) if loaded else False),
        ...
        summaries.append(
            {
                ...
                "eligible_repetitions": sum(bool(record["eligible"]) for record in records),
                "eligible": eligible,
                ...
                "maximum_solver_wall_seconds": max(solve_seconds) if solve_seconds else None,
            }
        )
...
def run_benchmark(
    ...
    progress = JsonlProgressWriter(progress_path)
    runs = []
    ...
    summary = {
        "schema": "rts_gmlc_zero_dc_ac_aware_warmstart_benchmark_result_v1",
        "benchmark_id": config["preregistration"]["id"],
...
def main() -> None:
    ...
    result = (
        ...
        else run_benchmark(args.config, output_directory=args.output_directory)
    )
