...
_EXECUTION_CONFIG_RELATIVE = (
    "configs/rq2_joint_deliverability_execution_successor_v3.yaml"
)
...
def _load_reviewed_execution_config(
    ...
    effect = receipt.get("effect")
    if (
        receipt.get("schema") != "rq2_joint_deliverability_execution_review_pass_v3"
        or receipt.get("review_scope")
        != "rq2_joint_deliverability_execution_successor_v3_exact_outer"
        ...
        or not isinstance(effect, Mapping)
        or effect.get("independent_v3_R3_review_passed") is not True
...
def stream_holdout_stage(
    *,
    ...
    raise ExecutionBlocked(
        "execution successor v3 public stage is closed pending fresh-process activation"
    )
