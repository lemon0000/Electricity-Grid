...
_LEVEL_SET_STAGES = frozenset(
    {"level_set_cost_minimization", "level_set_budget_feasibility"}
)
...
class MasterSolveResult:
    ...
    globally_infeasible: bool = False
...
class ExactCgStageResult:
    ...
    audited_snapshot: SharedSnapshot | None = None
...
def run_exact_cg_stage(
    *,
    ...
    No active-set state is retained between invocations.  A level-set
    cost-minimization stage may return a valid dual-only master record without
    an incumbent.  The budget-feasibility stage instead has an explicit,
    fail-closed early-separation outcome when its active decision MIP is
    globally infeasible.
    """
    ...
    bound_only_early_separation: dict[str, object] | None = None
    ...
    for iteration in range(1, maximum_iterations + 1):
        ...
        master_record = {
            **_call_payload(master_call),
            ...
            "globally_infeasible": bool(master.globally_infeasible),
            "decision_budget_cap_usd": _finite_number(master.decision_budget_cap_usd),
            "shared_snapshot_sha256": (
        ...
        dual_bound = _finite_number(master.dual_bound)
        if master.bound_valid and dual_bound is not None:
            dual_bounds.append(dual_bound)
        decision_cap = _finite_number(master.decision_budget_cap_usd)
        if stage == "level_set_budget_feasibility" and master.globally_infeasible:
            if master.incumbent_usable or master.snapshot is not None:
                failure_reason = "decision_mip_returned_inconsistent_infeasibility"
                break
            if decision_cap is None:
                failure_reason = "decision_mip_infeasibility_lacked_budget_cap"
                break
            bound_only_early_separation = {
                ...
                "inactive_state_screen_executed": False,
                "final_full_state_audit_executed": False,
                "mathematical_infeasibility_claim": False,
                "claim_scope": "no_budget_feasible_solution_at_or_above_this_proxy_floor",
            }
            separation_event = dict(bound_only_early_separation)
            separation_event["separation_schema"] = separation_event.pop("schema")
            callbacks.emit(
                "exact_cg_bound_only_early_separation",
                separation_event,
            )
            break
        if stage == "level_set_cost_minimization" and not master.incumbent_usable:
            failure_reason = "level_set_master_bound_only_without_audited_incumbent"
            break
        if stage == "level_set_budget_feasibility" and not master.incumbent_usable:
            failure_reason = "decision_mip_unresolved_without_feasible_incumbent"
            break
        if stage not in _LEVEL_SET_STAGES and (
            not master.incumbent_usable or not master.bound_valid or dual_bound is None
        ):
    ...
    if (
        ...
        and bound_only_early_separation is None
    ...
    if (
        failure_reason is None
        and stage not in _LEVEL_SET_STAGES
    ...
    audited_oracle_witness = bool(
        stage in _LEVEL_SET_STAGES
        and failure_reason is None
        and final_snapshot is not None
        and full_feasible_objective is not None
    )
    relative_acceptance_passed = bool(
        audited_oracle_witness
        or (
            certificate["valid"]
            and relative_to_feasible is not None
    ...
    if (
        failure_reason is None
        and stage not in _LEVEL_SET_STAGES
        and not maximum_acceptance_passed
    ):
        failure_reason = "final_bound_certificate_exceeds_maximum_acceptance"

    eligible = failure_reason is None and bound_only_early_separation is None
    level_set_oracle_outcome = (
        "bound_only_early_separation"
        if bound_only_early_separation is not None
        else (
            "audited_feasible"
            if eligible and stage in _LEVEL_SET_STAGES
            else "unresolved" if stage in _LEVEL_SET_STAGES else None
        )
    )
    eligibility_status = (
        "bound_only_early_separation"
        if bound_only_early_separation is not None
            ...
            if eligible and stage in _LEVEL_SET_STAGES
                ...
                else "eligible_within_maximum" if eligible else "ineligible"
            )
        )
    )
    acceptance = {
    ...
    stage_record: dict[str, object] = {
        ...
        "level_set_oracle_outcome": level_set_oracle_outcome,
        "bound_only_early_separation": bound_only_early_separation,
    ...
    return ExactCgStageResult(
        ...
        audited_snapshot=(
            final_snapshot if full_feasible_objective is not None else None
        ),
    )
...
def _validate_inputs(
    *,
    ...
    if stage in {
        ...
        "level_set_cost_minimization",
        "level_set_budget_feasibility",
    } and (_finite_number(proxy_floor) is None):
        raise ValueError("Cost minimization requires a finite proxy floor")
    if (
