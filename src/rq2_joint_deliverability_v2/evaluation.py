...
from .model import FOUR_ARM_IDS
from .solver_adapter import Rq2SolverSpec, solver_options
...
def _validate_capacity_cells(
    ...
    for raw_cell in cells:
        ...
        for arm_id in FOUR_ARM_IDS:
            ...
            fallback_failure_labels: set[str] = set()
            fallback_unresolved = False
            representative_incumbent: float | None = None
            if fallback:
                if (
                    not isinstance(certificate, Mapping)
                    or certificate.get("status") != "candidate_resolved"
                ):
                    raise ValueError(
                        "fallback evidence lacks a resolved representative candidate"
                    )
                representative_incumbent = _finite(
                    certificate.get("incumbent_capacity"),
                    "representative incumbent capacity",
                    minimum=0.0,
                )
            for certificate in fallback:
                ...
                finalized_fallback = finalize_capacity_certificate(
                    arm_id=arm_id,
                    candidate=certificate,
                )
                if (
                    finalized_fallback.status == RESOLVED
                    and finalized_fallback.reported_point is not None
                ):
                    assert representative_incumbent is not None
                    if (
                        finalized_fallback.reported_point
                        > representative_incumbent
                        + expected_solver_specification.feasibility_tolerance
                    ):
                        fallback_failure_labels.add(
                            "full_support_required_capacity_exceeds_"
                            "representative_incumbent"
                        )
                elif finalized_fallback.status == PROVEN_INFEASIBLE:
                    fallback_failure_labels.add(
                        "full_support_proven_infeasible_at_registered_cap"
                    )
                else:
                    fallback_unresolved = True
            registered_fallback_failure_labels = {
                "full_support_required_capacity_exceeds_representative_incumbent",
                "full_support_proven_infeasible_at_registered_cap",
            }
            if (
                set(failures) & registered_fallback_failure_labels
            ) != fallback_failure_labels:
                raise ValueError("fallback certificates and failure inventory disagree")
            if audit["status"] == "passed" and (
                fallback_failure_labels or fallback_unresolved
            ):
                raise ValueError(
                    "passed full-support audit has unresolved or failed fallback"
                )
            if audit["status"] == "unresolved" and not fallback_unresolved:
                raise ValueError(
                    "unresolved full-support audit lacks unresolved fallback"
                )
...
def validate_output_payloads(
    ...
    *,
    ...
    network_evidence_by_hash: dict[str, Mapping[str, object]] = {}
    for cell in capacity["cells"]:
        arm = cell["arms"]["network_only_shared"]
        if arm["solver_certificate"] is None:
            continue
        digest = str(arm["planning_input_sha256"])
        previous = network_evidence_by_hash.setdefault(digest, arm)
        if not _canonical_equal(arm, previous):
            raise ValueError(
                "network-only alpha reuse changed capacity or support evidence"
            )
...
def publish_output_bundle(
    ...
    *,
    ...
    result_parent_fsynced = False
    success_appeared = False
    success_parent_fsynced = False
    try:
        staging.mkdir()
        ...
        success_parent_fsynced = True
        observed = _validate_committed_output(target, success)
    except BaseException as publication_error:
        if (result_appeared and not result_parent_fsynced) or (
            success_appeared and not success_parent_fsynced
        ):
            raise RuntimeError(
