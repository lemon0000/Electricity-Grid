...
from experiments import (
    bootstrap_rq2_public_grid_highs_formal_activation_successor_v6 as bootstrap,
)
from experiments import rq2_public_grid_highs_formal_activation_contract_v6 as contract
from experiments import (
    run_rts_gmlc_public_grid_need_dispatch_v4_process_isolated_formal_v7 as controller,
)
...
def _terminal_contender(
    ...
    from experiments import (
        rq2_public_grid_highs_formal_activation_contract_v6 as child_contract,
    )
...
def _persist_valid_spawn_receipt(
    ...
    receipt = {
        "schema": "rq2_public_grid_highs_formal_activation_spawn_receipt_v6_preseal",
        "version": 6,
...
def test_v4_science_and_sealed_v5_successor_bindings_remain_exact() -> None:
...
def test_v6_materializes_full_production_wiring_behind_draft_gate() -> None:
...
def test_v6_contract_exposes_complete_production_api() -> None:
    for name in (
...
def _run_production_spawn_sandbox(
    ...
    *,
    ...
    frozen_closure = {
        "schema": "rq2_v6_test_frozen_execution_closure",
...
def test_tmp_v6_closure_matches_independent_modulefinder_exact_set(
    tmp_path: Path,
) -> None:
    root = contract.ROOT
...
def test_selected_current_closure_has_matching_authority_state(
    ...
    monkeypatch.delenv("RQ2_V6_PRESEAL_CLOSURE", raising=False)
...
def test_review_gate_precedes_run_authority_and_closure_drift_fails(
    ...
    monkeypatch.setenv("RQ2_V6_PRESEAL_CLOSURE", str(closure_path))
...
def test_production_artifact_paths_expands_one_shot_mapping(
    ...
    production = {
        "inner_manifest": "configs/v6.inner.json",
        "outer_manifest": "configs/v6.outer.json",
        "execution_closure": "configs/v6.closure.json",
        "one_shot_lease": {
            "fresh_path": "results/leases/v6.fresh.json",
            "fresh_sha256": "a" * 64,
            "consumed_path": "results/leases/v6.consumed.json",
            "authority_id": "b" * 64,
            "one_shot": True,
        },
        "activation_review_receipt": "configs/v6.review.json",
        "user_formal_run_authority": "configs/v6.user.json",
    }
    monkeypatch.setattr(
    ...
    assert len(paths) == 7
    assert contract.ROOT / "results/leases/v6.fresh.json" in paths
    assert contract.ROOT / "results/leases/v6.consumed.json" in paths
...
def test_fresh_lease_and_review_user_receipts_require_exact_keys(
    ...
    review = {
        "schema": "rq2_public_grid_highs_formal_activation_successor_review_pass_v6",
        "version": 6,
    ...
    user = {
        "schema": "rq2_public_grid_highs_formal_run_authority_v6",
        "version": 6,
...
def test_production_dynamic_full_closure_is_validated_once_under_timeout(
    ...
    closure.update(
        {
            "schema": "rq2_public_grid_highs_formal_activation_v6_execution_closure",
