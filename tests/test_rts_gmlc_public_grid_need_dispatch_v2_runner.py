...
import experiments.run_rts_gmlc_public_grid_need_dispatch_v2 as runner
...
def _config(tmp_path: Path, *, require_all: bool = True) -> tuple[Path, dict]:
    config = {
        ...
        "output": {
            "schema": "test_grid_need_dispatch_v2",
...
def _contract_identity(module_sha: str = "b" * 64) -> dict[str, object]:
    return {
        "contract_path": "configs/test_contract.yaml",
...
def test_production_preflight_keeps_formal_execution_closed():
    successor = runner.run(
        Path("configs/rts_gmlc_public_grid_need_dispatch_v2.yaml"),
        validate_only=True,
    )
    ...
    assert not successor["user_formal_run_authorized"]
