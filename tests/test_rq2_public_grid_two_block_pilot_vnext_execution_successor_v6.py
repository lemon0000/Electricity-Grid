...
def _deadline_outcome(
    *, detection_overrun_ns: int, termination_duration_ns: int
    ...
    def persist(journal: dict[str, object]) -> dict[str, object]:
        return {
            "schema": "rq2_public_grid_resource_monitor_persisted_outcome_vnext_execution_v6",
            "version": 1,
...
def test_v6_test_module_uses_detection_and_duration_not_fixed_exit_cap() -> None:
...
def test_normal_review_sign_destroys_test_seed_and_reuse_is_rejected() -> None:
    ...
    assert "one-time V6 key was already consumed" in source
...
def test_lamport_signature_valid_and_tamper_wrong_key_fail() -> None:
    ...
    digest = hashlib.sha256(b"v6 synthetic payload").hexdigest()
...
def test_v6_public_anchor_has_no_private_material() -> None:
    ...
    assert value["key_id"] == (
        "259f485f0ee48c7637a71dae0757151333eb82ffc89a030f5f10c43ad2686a91"
    )
    assert value["public_key_sha256"] == (
        "8919be7319e4648fa8122440f9018126b9c5b1d48a921e5ee42fe1703771f7da"
    )
...
def test_resource_journal_and_success_readback_are_signed_substance() -> None:
    ...
    journals = {
        ...
        contract.BLOCKS[1]: contract.synthetic_deadline_missed_journal_for_test(
...
def test_execution_review_gate_precedes_v6_production_lease_touch() -> None:
    config = contract.load_config()
...
def test_v5_escalate_and_seed_free_revocation_are_exact() -> None:
    contract.verify_v5_escalate_and_revocation()
    frozen = contract.load_config()["predecessor_v5"]
...
def test_canonical_validate_only_and_execute_remain_review_gated() -> None:
    ...
    assert "fixed V6 execution-review PASS receipt is absent" in execute.stderr
