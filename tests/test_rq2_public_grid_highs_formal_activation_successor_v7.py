...
from experiments import (
    bootstrap_rq2_public_grid_highs_formal_activation_successor_v7 as bootstrap,
)
from experiments import rq2_public_grid_highs_formal_activation_contract_v7 as contract
from experiments import (
    run_rts_gmlc_public_grid_need_dispatch_v4_process_isolated_formal_v8 as controller,
)
...
def _terminal_contender(
    ...
    from experiments import (
        rq2_public_grid_highs_formal_activation_contract_v7 as child_contract,
    )
...
def _persist_valid_spawn_receipt(
    ...
    receipt = {
        "schema": "rq2_public_grid_highs_formal_activation_spawn_receipt_v7_preseal",
        "version": 7,
...
def test_v4_science_and_sealed_v6_successor_bindings_remain_exact() -> None:
    ...
    assert report["sealed_predecessor_v6_outer_sha256"] == (
        "12a00b1c4a75943c81e712bc02f53ed38d67a6749e440c1103294a3c568937aa"
    )
    assert report["sealed_predecessor_v6_closure_sha256"] == (
        "535bb0eba4c36b58efd45711f1e9fffbcaf65f703754517896fd67c7b0c3c0e4"
    )
    assert report["sealed_predecessor_v6_escalate_sha256"] == (
        "ee41fe26122862650fd492fe54aa950936f8cb38da21991b063130a771f6a2aa"
    )
    sealed_predecessor = contract.load_config()["sealed_predecessor_v6"]
    assert sealed_predecessor["changes_scientific_protocol"] is False
    assert sealed_predecessor["supersession_reason"] == (
        "official_rework_successor_lifecycle_escalation"
    )
    receipt = json.loads(
        contract.SEALED_PREDECESSOR_ESCALATE.read_text(encoding="utf-8")
    )
    assert receipt["verdict"] == "ESCALATE"
    assert receipt["reviewed_outer"]["sha256"] == (
        "12a00b1c4a75943c81e712bc02f53ed38d67a6749e440c1103294a3c568937aa"
    )
    assert receipt["finding_counts"] == {"blocker": 1, "major": 0, "minor": 0}
    assert receipt["findings"][0]["original_documentation_finding_corrected"] is True
    assert receipt["findings"][0]["execution_authority_effect"] == "none"
    assert receipt["cryptographic_reviewer_signature_present"] is False
    ...
    _assert_v7_draft_keeps_every_authority_and_claim_gate_closed()
    _assert_v5_v6_protected_hashes_remain_unchanged()


def _assert_v7_draft_keeps_every_authority_and_claim_gate_closed() -> None:
    config = contract.load_config()
    assert config["status"] == "DRAFT_NONAUTHORITATIVE"
    ...
    assert all(value is None for value in config["production_artifacts"].values())


def _assert_v5_v6_protected_hashes_remain_unchanged() -> None:
    expected = {
        "configs/rq2_public_grid_highs_formal_activation_successor_v5.OUTER.SHA256SUMS.json": "7d6a217ab1536e52b37180d5a1fa5b715eb03980ae3cbb3e4de2e9fd315e64ea",
        "configs/rq2_public_grid_highs_formal_activation_successor_v5.SHA256SUMS.json": "5cca0675bfd9a0b624287cf2705d089a4044493eacfa54c6ac82e074227c1a03",
        "configs/rq2_public_grid_highs_formal_activation_successor_v5.EXECUTION_CLOSURE.SHA256SUMS.json": "f844b0ef12cf8111e533d13104d02e41e70eafbc6d1b56972dfd267fbdf5b08e",
        "results/leases/rq2_public_grid_highs_formal_activation_successor_v5.lease.json": "4fc3da709214be3d8ea358b73a12e963ab18911db2fa414560a4c4636e45118c",
        "configs/rq2_public_grid_highs_formal_activation_successor_v6.OUTER.SHA256SUMS.json": "12a00b1c4a75943c81e712bc02f53ed38d67a6749e440c1103294a3c568937aa",
        "configs/rq2_public_grid_highs_formal_activation_successor_v6.SHA256SUMS.json": "6e4470ea6903c34aac046ca22f8aff043a1ae3a22a474d8ab886c84070718ee3",
        "configs/rq2_public_grid_highs_formal_activation_successor_v6.EXECUTION_CLOSURE.SHA256SUMS.json": "535bb0eba4c36b58efd45711f1e9fffbcaf65f703754517896fd67c7b0c3c0e4",
        "configs/rq2_public_grid_highs_formal_activation_successor_v6.PRE_SEAL_AUDIT.json": "16d865a2fa13d1ba2d98019bdc09645195244f2a54a084c8db7db02f400f6d18",
        "configs/rq2_public_grid_highs_formal_activation_successor_v6.json": "025d244ecd03848adc657feb74e71e0fd1c7522040f40023fe19f84953741e44",
        "configs/rts_gmlc_public_grid_need_dispatch_v4_highs_process_isolated_formal_v7.yaml": "2128a11ebca54396dd7927afcd9496de10013fbb00142a12fa5df26d4543a05c",
        "results/leases/rq2_public_grid_highs_formal_activation_successor_v6.lease.json": "da599ee30522cd6597636bf8317ac8a90f518650a757cebc52ef41f57d325612",
    }
    assert {
        path: contract.sha256_file(contract.ROOT / path) for path in expected
    } == expected
...
def test_v7_materializes_full_production_wiring_behind_draft_gate() -> None:
...
def test_v7_contract_exposes_complete_production_api() -> None:
    for name in (
...
def _run_production_spawn_sandbox(
    ...
    *,
    ...
    frozen_closure = {
        "schema": "rq2_v7_test_frozen_execution_closure",
...
def test_tmp_v7_closure_matches_independent_modulefinder_exact_set(
    tmp_path: Path,
) -> None:
    root = contract.ROOT
    ...
    production_closure = (
        root
        / "configs/rq2_public_grid_highs_formal_activation_successor_v7.EXECUTION_CLOSURE.SHA256SUMS.json"
    )
...
def test_selected_current_closure_has_matching_authority_state(
    ...
    monkeypatch.delenv("RQ2_V7_PRESEAL_CLOSURE", raising=False)
...
def test_bootstrap_acceptance_to_spawn_try_line_gap_is_recovered(
    ...
    real_popen = bootstrap.subprocess.Popen
    real_wait_for_release_acceptance = bootstrap._wait_for_release_acceptance
    ...
    def inject_at_gap(frame: Any, event: str, _arg: Any) -> Any:
        nonlocal injected
        if frame.f_code is not bootstrap._spawn_controller.__code__:
            return None
    ...
    def arm_trace_after_acceptance(*args: Any, **kwargs: Any) -> dict[str, Any]:
        acceptance = real_wait_for_release_acceptance(*args, **kwargs)
        caller = sys._getframe(1)
        assert caller.f_code is bootstrap._spawn_controller.__code__
        caller.f_trace = inject_at_gap
        sys.settrace(inject_at_gap)
        return acceptance

    monkeypatch.setattr(bootstrap.subprocess, "Popen", capture_controller)
    monkeypatch.setattr(
        bootstrap, "_wait_for_release_acceptance", arm_trace_after_acceptance
    )
...
def test_review_gate_precedes_run_authority_and_closure_drift_fails(
    ...
    monkeypatch.setenv("RQ2_V7_PRESEAL_CLOSURE", str(closure_path))
...
def test_production_artifact_paths_expands_one_shot_mapping(
    ...
    production = {
        "inner_manifest": "configs/v7.inner.json",
        "outer_manifest": "configs/v7.outer.json",
        "execution_closure": "configs/v7.closure.json",
        "one_shot_lease": {
            "fresh_path": "results/leases/v7.fresh.json",
            "fresh_sha256": "a" * 64,
            "consumed_path": "results/leases/v7.consumed.json",
            "authority_id": "b" * 64,
            "one_shot": True,
        },
        "activation_review_receipt": "configs/v7.review.json",
        "user_formal_run_authority": "configs/v7.user.json",
    }
    monkeypatch.setattr(
    ...
    assert len(paths) == 7
    assert contract.ROOT / "results/leases/v7.fresh.json" in paths
    assert contract.ROOT / "results/leases/v7.consumed.json" in paths


def test_sealed_gate_rejects_missing_or_drifted_predecessor_escalate_before_effects(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    receipt_member = contract.SEALED_PREDECESSOR_ESCALATE.relative_to(
        contract.ROOT
    ).as_posix()
    lease = {
        "fresh_path": "results/leases/v7-test.fresh.json",
        "fresh_sha256": "0" * 64,
    ...
    closure_member = (
        "configs/rq2_public_grid_highs_formal_activation_successor_v7."
        "EXECUTION_CLOSURE.SHA256SUMS.json"
    )
    with monkeypatch.context() as member_patch:
        member_patch.setattr(
            contract,
            "_production_artifact_path",
            lambda name: contract.ROOT / closure_member,
        )
        expected_members = contract._expected_inner_member_paths(lease)
    assert receipt_member in expected_members

    for failure in ("missing", "content-drift"):
        case_root = tmp_path / failure
        members_root = case_root / "members"
        members_root.mkdir(parents=True)
        artifact_paths = {
            name: case_root / f"{name}.json"
            for name in ("inner_manifest", "outer_manifest", "execution_closure")
        }
        artifact_paths["execution_closure"] = case_root / closure_member
        artifact_paths["execution_closure"].parent.mkdir(parents=True)
        member_paths = {
            raw_path: members_root / f"{index:02d}.bin"
            for index, raw_path in enumerate(sorted(expected_members))
        }
        for raw_path, path in member_paths.items():
            path.write_bytes(f"fixture:{raw_path}\n".encode())
        receipt_path = member_paths[receipt_member]
        receipt_path.write_bytes(contract.SEALED_PREDECESSOR_ESCALATE.read_bytes())
        fresh_path = member_paths[str(lease["fresh_path"])]
        lease["fresh_sha256"] = contract.sha256_file(fresh_path)
        closure = {
        ...
        contract.persist_json_stable(artifact_paths["execution_closure"], closure)
        member_paths[closure_member] = artifact_paths["execution_closure"]
        members = {
            raw_path: contract.sha256_file(member_paths[raw_path])
            for raw_path in sorted(expected_members)
        }
        inner = {
        ...
        contract.persist_json_stable(artifact_paths["inner_manifest"], inner)
        outer = {
            "schema": "rq2_public_grid_highs_formal_activation_successor_v7_outer",
            "version": 7,
            "inner": {
                "path": artifact_paths["inner_manifest"]
                .relative_to(case_root)
                .as_posix(),
                "sha256": contract.sha256_file(artifact_paths["inner_manifest"]),
            },
        }
        contract.persist_json_stable(artifact_paths["outer_manifest"], outer)
        if failure == "missing":
            receipt_path.unlink()
        else:
        ...
        calls = {
            name: 0 for name in ("review", "user", "preflight", "consume", "spawn")
        }
        consumed_path = case_root / "consumed.json"
        with monkeypatch.context() as case_patch:
            case_patch.setattr(contract, "ROOT", case_root)
            case_patch.setattr(
                contract,
                "load_config",
                lambda: {
            ...
            case_patch.setattr(
                contract,
                "_expected_inner_member_paths",
                lambda bound_lease: expected_members,
            )
            case_patch.setattr(
                contract,
                "_repo_path",
            ...
            with pytest.raises(
                contract.FormalActivationRejected,
                match="successor_review_escalate_v6",
            ):
                bootstrap.execute()
        assert calls == {
            "review": 0,
