...
from experiments import (
    bootstrap_rq2_joint_deliverability_activation_v1 as bootstrap,
)
from experiments import run_rq2_joint_deliverability_activation_v1 as controller
from experiments import validate_rq2_joint_deliverability_activation_v1 as validator
...
def test_python_closure_matches_independent_ast_discovery() -> None:
    ...
    expected = {
        ...
        "experiments/bootstrap_rq2_joint_deliverability_activation_v1.py",
        "experiments/run_rq2_joint_deliverability_activation_v1.py",
...
def test_stable_read_rejects_path_swap_after_descriptor_open(
    ...
    def open_then_swap(path: str | bytes, flags: int) -> int:
        nonlocal swapped
        descriptor = real_open(path, flags)
        if not swapped and Path(path) == artifact.absolute():
...
def test_presence_uses_lstat_reparse_evidence(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    candidate = tmp_path / "junction"
    real_lstat = bootstrap.os.lstat

    def injected_lstat(path: str | bytes | os.PathLike[str]) -> object:
        if Path(path) == candidate:
            return SimpleNamespace(
                st_mode=bootstrap.stat.S_IFDIR,
                st_file_attributes=0x400,
            )
        return real_lstat(path)

    monkeypatch.setattr(bootstrap.os, "lstat", injected_lstat)
