...
OUTER_MANIFEST = Path(
    "configs/rq2_public_data_robust_identification_preregistration_v4."
    "SHA256SUMS.json"
)
...
def test_preregistration_v4_frozen_hashes_are_live():
...
def test_preregistration_v4_keeps_every_formal_gate_closed():
    prereg = yaml.safe_load(PREREGISTRATION.read_text(encoding="utf-8"))
...
def test_preregistration_v4_outer_manifest_is_live():
    prereg = yaml.safe_load(PREREGISTRATION.read_text(encoding="utf-8"))
    manifest = json.loads(OUTER_MANIFEST.read_text(encoding="utf-8"))
    assert manifest["schema"] == "rq2_public_data_preregistration_manifest_v4"
