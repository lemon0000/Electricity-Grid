...
import experiments.run_rq2_public_pairwise_replay as runner
...
def _context(config: dict) -> tuple[dict, dict[str, object]]:
    return config, {
...
def test_production_preflight_keeps_replay_closed():
    report = runner.run(
        Path("configs/rq2_public_pairwise_replay_v1.yaml"),
        validate_only=True,
    )

    assert report["parameter_cells"] == 15
    assert report["workload_training_blocks"] == 34
    assert report["workload_holdout_blocks"] == 34
    assert not report["grid_need_dispatch_ready"]
    assert not report["formal_execution_ready"]
    assert not report["independent_R4_review_passed"]
