...
def test_successor_registers_every_algorithm_source_explicitly() -> None:
    ...
    assert {
        ...
        "parent_formal_runner_path",
        "invalidation_publisher_path",
        "runner_path",
        "monitor_path",
        "launcher_path",
    } <= set(implementation)
...
def test_successor_preserves_parent_algorithm_and_decision_pilot_identity() -> None:
    config = formal._read_config(formal.DEFAULT_CONFIG_PATH)

    assert config["formal_successor"]["algorithm_id"] == (
        "direct_exact_cg_then_bracketed_level_set_repair_003"
    )
    assert "repair_003_decision_pilot" in config["control_plane_selection"]["root"]
    assert config["control_plane_selection"]["required_schema"] == (
        "rts_gmlc_v4_repair_003_level_set_decision_pilot_v2"
    )
    assert config["implementation"]["pilot_module_path"].endswith(
        "run_rts_gmlc_zero_dc_ac_aware_commitment_v4_repair_003.py"
    )


def test_context_and_preregistration_require_verified_parent_invalidation(
    tmp_path: Path,
) -> None:
    parent_output = tmp_path / "parent"
    attempt = tmp_path / "attempt"
    shutil.copytree(invalidation.OUTPUT_ROOT, parent_output)
    shutil.copytree(invalidation.ATTEMPT_LOG_ROOT, attempt)
    invalidation.record_invalidation(
        output_root=parent_output,
        attempt_log_root=attempt,
    )
    invalidation_root = parent_output / "invalidation"
    successor_output = tmp_path / "successor"

    context = formal._build_context(
        formal.DEFAULT_CONFIG_PATH,
        predecessor_invalidation_root=invalidation_root,
    )
    first = formal.prepare_preregistration(
        formal.DEFAULT_CONFIG_PATH,
        output_directory=successor_output,
        predecessor_invalidation_root=invalidation_root,
    )
    second = formal.prepare_preregistration(
        formal.DEFAULT_CONFIG_PATH,
        output_directory=successor_output,
        predecessor_invalidation_root=invalidation_root,
    )
    ...
    assert first == second
    assert first["input_contract_sha256"] == context.input_contract_sha256
    assert (
        first["input_contract"]["predecessor_repair_003"][
            "invalidation_manifest_sha256"
        ]
        == "dd35214345d5c0a516f12bbfb6c898df12ca6da6f42f6fb0deb039a82a176053"
    )
    formal._verify_manifest(successor_output / "preregistration")
...
def test_prefix_checkpoint_rejects_copied_predecessor_stage_claim(
    ...
    context = type(
        ...
        {
            "config": {
                "preregistration": {"id": "repair-004-test"},
                "candidate_frontier": {
