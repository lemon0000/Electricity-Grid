...
from experiments import run_rq2_joint_deliverability_execution_v1 as runner
from experiments import (
    run_rq2_joint_deliverability_implementation_v2 as implementation_runner,
)
from experiments import (
    validate_rq2_joint_deliverability_execution_v1 as validator,
)
from src.rq2_joint_deliverability_execution_v1 import core
...
CONFIG = ROOT / "configs/rq2_joint_deliverability_execution_successor_v1.yaml"
...
def test_static_candidate_and_validate_only_are_closed() -> None:
    config = _config()
    static = validator.validate(config, require_sealed=False)
...
def test_execution_review_receipt_is_the_fixed_outer_authority(
    ...
    core_relative = "src/rq2_joint_deliverability_execution_v1/core.py"
    ...
    inner_path = tmp_path / (
        "configs/rq2_joint_deliverability_execution_successor_v1.SHA256SUMS.json"
    )
    inner_payload = {
        "schema": "rq2_joint_deliverability_execution_inner_v1",
        "version": 1,
        "files": {
    ...
    outer_payload = {
        "schema": "rq2_joint_deliverability_execution_outer_v1",
        "version": 1,
        "inner": {
...
def test_validate_only_runner_supports_direct_script_invocation() -> None:
    completed = subprocess.run(
        [
            sys.executable,
            str(ROOT / "experiments/run_rq2_joint_deliverability_execution_v1.py"),
            "--validate-only",
        ],
        cwd=ROOT,
        check=True,
...
import experiments.run_rq2_joint_deliverability_execution_v1  # noqa: F401
...
"""
    ...
    expected = {
        ...
        "experiments/run_rq2_joint_deliverability_execution_v1.py",
        "experiments/validate_rq2_joint_deliverability_execution_v1.py",
        "src/__init__.py",
        "src/rq2_joint_deliverability_execution_v1/__init__.py",
        "src/rq2_joint_deliverability_execution_v1/core.py",
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("gates", "formal_execution_ready", True),
        ("gates", "user_formal_run_authorized", True),
        ("gates", "security_certified", True),
        ("execution_contract", "formal_activation_wrapper", "implemented"),
    ),
)
def test_validator_rejects_opened_gates(
    ...
    with pytest.raises(ValueError):
        validator.validate(config, require_sealed=False)
