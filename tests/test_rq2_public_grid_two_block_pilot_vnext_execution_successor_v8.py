...
from experiments import (
    rq2_public_grid_two_block_pilot_vnext_execution_contract_v8 as contract,
)
from experiments import (
    run_rq2_public_grid_two_block_pilot_vnext_execution_successor_v8 as controller,
)
...
class _PriorityClock(_Clock):
    def __init__(self) -> None:
        super().__init__()
        self._pending_advance_ns: int | None = None
        self._monotonic_calls_before_advance = 0

    def arm_after_observation(self, nanoseconds: int) -> None:
        assert self._pending_advance_ns is None
        self._pending_advance_ns = nanoseconds
        self._monotonic_calls_before_advance = 1

    def monotonic_ns(self) -> int:
        if self._pending_advance_ns is not None:
            if self._monotonic_calls_before_advance == 0:
                self.advance(self._pending_advance_ns)
                self._pending_advance_ns = None
            else:
                self._monotonic_calls_before_advance -= 1
        return self.now_ns


def _poll_priority_outcome(
    *, next_loop_delta_ns: int
) -> tuple[dict[str, object], contract.ResourceMonitorState, int, int]:
    clock = _PriorityClock()
    process = _OwnedProcess()
    state = contract.ResourceMonitorState()
    sample_calls = 0
    persist_calls = 0
    path = "memory://v8-poll-priority.json"
    ...
    def sample(pid: int, create_time_ns: int) -> tuple[int, int]:
        ...
        sample_calls += 1
        if sample_calls == 1:
            clock.arm_after_observation(next_loop_delta_ns)
            process.exited = True
            return 1024, 12 * 1024**3
        raise OSError("exited child cannot provide an exact same-pair sample")

    def persist(journal: dict[str, object]) -> dict[str, object]:
        nonlocal persist_calls
        persist_calls += 1
        return {
            "schema": "rq2_public_grid_resource_monitor_persisted_outcome_vnext_execution_v8",
            "version": 1,
    ...
    return outcome, state, sample_calls, persist_calls


def test_v8_poll_exit_before_next_slot_is_clean_and_persisted() -> None:
    outcome, state, sample_calls, persist_calls = _poll_priority_outcome(
        next_loop_delta_ns=contract.SAMPLE_INTERVAL_NS - 1
    )
    ...
    assert outcome["readback_verified"] is True
    assert state.outcome == outcome


def test_v8_due_slot_exit_cannot_bypass_exact_sample_failure() -> None:
    outcome, state, sample_calls, persist_calls = _poll_priority_outcome(
        next_loop_delta_ns=contract.SAMPLE_INTERVAL_NS
    )
    journal = outcome["resource_journal"]
    assert journal["status"] == "sampling_error"
    assert journal["reason"] == "resource_sample_failed"
    assert journal["honest_incomplete"] is True
    assert journal["mathematical_infeasibility_inferred"] is False
    assert journal["termination"]["termination_action"] == (
...
def test_v8_overdue_slot_exit_persists_unique_deadline_miss() -> None:
    outcome, state, sample_calls, persist_calls = _poll_priority_outcome(
        next_loop_delta_ns=(
    ...
    assert journal["deadline_missed"]["sequence_index"] == 1
    assert journal["expected_sample_count_at_exit"] == 2
    assert journal["sample_count"] == 1
    assert journal["termination"]["unobserved_due_slot_count"] == 1
    assert journal["termination"]["termination_action"] == (
        "already_exited_before_signal"
    )
    assert sample_calls == 1
...
def test_v8_exact_deadline_is_inclusive_sample_window() -> None:
    outcome, state, sample_calls, persist_calls = _poll_priority_outcome(
        next_loop_delta_ns=(
            contract.SAMPLE_INTERVAL_NS
            + contract.OBSERVATION_JITTER_BUDGET_NS
    ...
    assert outcome["resource_journal"]["status"] == "sampling_error"
    assert sample_calls == 2
    assert persist_calls == 1
    assert state.outcome == outcome


def test_v8_watchdog_cannot_be_bypassed_by_exited_poll() -> None:
    clock = _PriorityClock()
    process = _OwnedProcess()
    state = contract.ResourceMonitorState()
    sample_calls = 0
    persist_calls = 0
    path = "memory://v8-watchdog-priority.json"

    def sample(_pid: int, _create_time_ns: int) -> tuple[int, int]:
        nonlocal sample_calls
        sample_calls += 1
        clock.arm_after_observation(contract.SAMPLE_INTERVAL_NS)
        if sample_calls == contract.WATCHDOG_NS // contract.SAMPLE_INTERVAL_NS:
            process.exited = True
        return 1024, 12 * 1024**3
    ...
    assert journal["reason"] == "external_watchdog_reached"
    assert journal["expected_sample_count_at_exit"] == sample_calls + 1
    assert journal["termination"]["unobserved_due_slot_count"] == 1
    assert persist_calls == 1
    assert state.outcome == outcome
...
def test_v8_test_module_uses_detection_and_duration_not_fixed_exit_cap() -> None:
...
def test_normal_review_sign_destroys_test_seed_and_reuse_is_rejected() -> None:
    ...
    assert "one-time V8 key was already consumed" in source
...
def test_frozen_audit_values_and_science_thresholds_are_exact() -> None:
    ...
    assert resource["global_watchdog_checked_before_child_exit_poll"] is True
    assert resource["child_exit_poll_allowed_only_before_scheduled_slot"] is True
    assert resource[
...
def test_lamport_signature_valid_and_tamper_wrong_key_fail() -> None:
    ...
    digest = hashlib.sha256(b"v8 synthetic payload").hexdigest()
...
def test_v8_public_anchor_has_no_private_material() -> None:
...
def test_execution_review_gate_precedes_v8_production_lease_touch() -> None:
    config = contract.load_config()
...
def test_v7_escalate_and_seed_free_revocation_are_exact() -> None:
    contract.verify_v7_escalate_and_revocation()
    frozen = contract.load_config()["predecessor_v7"]
...
def test_bundle_live_closure_new_roots_and_review_absence() -> None:
    ...
    assert len(self_mapping) == config["bundle"]["self_mapping_exact_count"] == 14
...
def test_canonical_validate_only_and_execute_remain_review_gated() -> None:
    ...
    assert "fixed V8 execution-review PASS receipt is absent" in execute.stderr
