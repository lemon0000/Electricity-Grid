...
from experiments import (
    validate_rq2_joint_deliverability_preregistration_successor_v4 as validator,
)

ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs/rq2_joint_deliverability_preregistration_successor_v4.yaml"
SPEC = ROOT / "docs/model_spec/rq2_joint_deliverability_estimands_v3.md"
PLAN = ROOT / "docs/plan/RQ2_联合服务可交付前沿确认性方案_v4.md"
INNER = ROOT / (
...
def test_predecessor_is_provenance_only_and_hash_bound() -> None:
    ...
    assert (
        validator._sha256(validator.V3_OUTER)
        == (authority["reviewed_v3_outer"]["sha256"])
    )
    assert (
        validator._sha256(validator.V3_ESCALATE)
        == (authority["v3_escalate_receipt"]["sha256"])
    )
...
def test_E0_transport_and_common_coupling_are_exact() -> None:
    ...
    assert (
        transport["multimetric_sign_statement_requires_one_common_pi_witness"] is True
    )
    common_pi = transport["common_pi_problem"]
    assert common_pi["predicate_forms"] == {
        "robust_positive": (
            "negative_metric_dot_pi_less_than_or_equal_to_negative_1e-6"
        ),
        "robust_negative": ("metric_dot_pi_less_than_or_equal_to_negative_1e-6"),
        "nonpositive": "metric_dot_pi_less_than_or_equal_to_1e-6",
    }
    assert common_pi["phase_one_constraint"] == (
        "each_predicate_left_side_minus_z_less_than_or_equal_to_rhs"
    )
    assert common_pi["certified_incompatible"] == (
        "valid dual feasible lower bound on phase_one_optimum greater than 1e-8"
    )
    assert "dual_lower_bound_serialized" in common_pi["certificate_requirements"]
    statements = transport["common_pi_registered_statements"]
    assert set(statements) == {
        "b6_operational_penalty",
        "b6_operational_relief",
    }
    assert [
        branch["predicates"][0]["metric"]
        for branch in statements["b6_operational_penalty"]["branches"]
    ] == [
...
def test_manifest_unknown_keys_duplicate_json_and_symlink_fail_closed(
    ...
    digest = "0" * 64
    inner = {
        "schema": ("rq2_joint_deliverability_preregistration_successor_manifest_v4"),
        "files": {relative: digest for relative in validator.EXPECTED_INNER_MEMBERS},
    }
    outer = {
        "schema": ("rq2_joint_deliverability_preregistration_successor_outer_v4"),
        "version": 4,
        "inner": {
