...
from experiments.run_rq2_public_identification_grid_v2 import (
...
def _package(tmp_path: Path, *, complete: bool = True) -> Path:
    ...
    (package / "summary.json").write_text(
        json.dumps(
            {
                "schema": "test_pairwise_replay_v2",
...
def _config(tmp_path: Path, package: Path) -> Path:
    config = {
        "input": {
            ...
            "expected_pairwise_schema": "test_pairwise_replay_v2",
        ...
        "output": {
            "schema": "test_identification_grid_v2",
