...
本文件是v4路线的checkpoint-inventory completeness successor。科学设计
保持不变；本版只修复R4复审发现的inventory required-key-set缺口。
...
- 每个v3 stage从同一冻结provenance contract加载runner、直接模型模块和
  solver package版本；任一live hash或版本漂移都在计算前拒绝。
- grid与pairwise checkpoint绑定canonical stage-base provenance hash；
  发布包携带逐checkpoint SHA-256 inventory和`provenance.json`。
- 下游不仅验证重新计算后的package manifest，还验证上游config SHA、
  contract、implementation、solver、source/input manifests和checkpoint
  inventory，禁止旧checkpoint被当前代码重新署名。
- grid inventory键集合必须精确等于全部注册block IDs；pairwise inventory
  键集合必须精确等于全部cell policy checkpoints与所有eligible cell的完整
  row-by-column Cartesian pair checkpoints。
- inventory digest只接受小写十六进制64字符SHA-256。删除、增加或改名任一
  inventory条目，即使同步重算provenance、summary和package manifest，也
  必须在下游消费前拒绝。
- 安全相关JSON使用duplicate-rejecting loader；不得利用同名键的last-value
  覆盖语义隐藏非法digest或字段。
...
1. 全量1071-block grid-need dispatch尚未发布；
2. pairwise replay与identification依赖该上游包；
3. v5 inventory-completeness successor尚需独立R4审查；
4. 用户已授权所需权限，但正式运行只能在R4 `PASS`后按阶段门启动。
