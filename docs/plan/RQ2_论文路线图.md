...
| **Temporal H2 固定策略 holdout** | [temporal_economic_holdout.py](file:///Users/bytedance/Workspace/Electricty-Grid/Electricity-Grid/src/evaluation/temporal_economic_holdout.py)；入口 [run_rq2_h2_temporal_holdout.py](file:///Users/bytedance/Workspace/Electricty-Grid/Electricity-Grid/experiments/run_rq2_h2_temporal_holdout.py) | [test_temporal_economic_holdout.py](file:///Users/bytedance/Workspace/Electricty-Grid/Electricity-Grid/tests/test_temporal_economic_holdout.py)、[test_rq2_h2_temporal_holdout_runner.py](file:///Users/bytedance/Workspace/Electricty-Grid/Electricity-Grid/tests/test_rq2_h2_temporal_holdout_runner.py) | manual chronology mechanism-only；固定`D_flex`、真实共享recourse、failure-channel拆分、right-censoring诚实报告 |
...
- 旧generated/reduced H2样本只保存窗口均值，缺失小时顺序，不能直接用于temporal H2；新runner对此fail-closed，下一增量需建立连续窗口生成与时序场景缩减。
- R3独立审查：最终 **PASS**；覆盖双证书hard failure、hard/soft互斥、任意正概率unresolved关闭H2、终端状态持久化、training-before-holdout调用顺序及immutable manifest发布。
