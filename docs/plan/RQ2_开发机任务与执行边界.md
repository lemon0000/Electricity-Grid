...
5. 完善四臂增强基线的测试、schema和报告模板，但不得把缺失的正式grid package
   替换为合成或历史checkpoint。
6. 基于现有阴性结果撰写条件识别、共同不足和失败区域叙事，同时保留
   `formal_result=false`与`security_certified=false`。

## 3. 当前顺序

1. 同步当前状态导航、blocker总表和README。
2. 由用户决定是否授权创建formal activation V5 draft。该授权只覆盖设计与短验证；
   seal、独立review和formal run仍分别受门禁约束。
3. 若V5最终通过独立review并另获formal-run authority，从block zero运行冻结的
   1071-block process-isolated HiGHS grid流程。
4. grid package完整发布并验签后，依次开放四臂planning/pairwise和identification。
5. 根据四臂结果选择论文叙事：
   - 出现all-coupling稳健的B6-specific差异：报告重复信用的条件风险；
   - 共同不足占主导：转向hourly-CFE共同不足边界；
   - 符号依赖coupling：报告partial identification。
