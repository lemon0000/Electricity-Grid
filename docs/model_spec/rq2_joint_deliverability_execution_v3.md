# RQ2 联合服务可交付前沿 execution successor v3
...
sealed execution v2 在 PASS receipt 物化后，其 focused suite 再次因 live
authority 状态变化而 raw-red，现行 verdict 为 official `ESCALATE`。用户已明确
重新授权新的 v3 successor。v3 绑定 v2 outer、ESCALATE receipt 与 superseded
PASS bytes，只修复 review-authority 状态测试：public-stage 测试不读取 live
receipt，receipt absent/present 转移在临时根中完整重放，review 后的下一门
`dispatched-grid manifest not bound`另行验证。v1/v2 sealed bytes、科学协议、
solver、execution evidence schema 和正式执行权限均不改变。
...
successor lineage 先绑定并递归验证 sealed execution v2 outer、official
`ESCALATE` receipt及逐字节保留的 superseded PASS receipt，并要求旧 fixed PASS
authority 路径保持 absent。运行静态 authority 再由代码内部从 live bytes派生：
...
调用者不能注入静态 trust root。v3 的四个公开 stage 入口保持硬关闭，统一抛出
...
本 v3 保留的私有 execution helpers 不接受 repository root、execution outer digest
...
`configs/rq2_joint_deliverability_execution_review_pass_v3.yaml`，要求独立
...
测试合同不得把 live fixed-path receipt 的当前存在性当作永久前提。公开入口关闭
测试只验证不可注入参数与无条件 `ExecutionBlocked`；review receipt 的缺失、合法
出现及篡改在同一临时根中依次验证；合法 review 后的 grid-manifest 阻塞通过独立
fixture 验证。由此 focused/broad 命令必须在 receipt 物化前后原样通过，不允许
deselect 本 successor 的状态断言。
...
authority内部计算；调用者提供的 store identity 必须精确相等。v3将后两项登记为
...
test helper。v3公开planning入口保持硬关闭，后续fresh-process activation
