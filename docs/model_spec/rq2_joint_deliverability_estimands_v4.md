...
v5机器协议是完整、自包含的候选权威。达到seal commitment point后，本版本才成为
...
holdout和scalar transport endpoint证书完整。某一arm为unresolved或training-support
...
两个operational labels只读取上述三个B6-minus-correct指标的scalar transport
endpoints。量词顺序固定为“先存在一个注册指标，再对所有允许coupling成立”：

\[
\text{penalty}\iff\exists m:\ L_m=\min_{\pi\in\Pi}\Delta_m(\pi)>10^{-6},
\]

\[
\text{relief}\iff\exists m:\ U_m=\max_{\pi\in\Pi}\Delta_m(\pi)<-10^{-6}.
\]

因此，某个coupling上出现正值或负值的existential witness不足以支持任一robust
operational label，也不执行common-\(\pi\) phase-I。两个label可以因不同指标分别
获得robust证书并同时成立。证书float权威表示为IEEE-754 `float.hex()`，JSON采用
sort-keys、compact separators、UTF-8和末尾换行。
