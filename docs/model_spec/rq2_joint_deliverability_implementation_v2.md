# RQ2 联合服务可交付前沿 reference implementation v2

## 1. 文档地位

本文件说明
`configs/rq2_joint_deliverability_preregistration_successor_v5.yaml`
的 versioned reference implementation。科学定义仍以 sealed V5 outer、inner、配置、
...
它是 v1 official R3 `REWORK` 后唯一一次聚焦 successor；v1 outer 与 8 个
sealed members 保持不可变。

## 2. 模块边界

### 2.1 场景层

`src/rq2_joint_deliverability_v2/scenarios.py`负责：
...
`src/rq2_joint_deliverability_v2/model.py`负责：
...
`src/rq2_joint_deliverability_v2/evaluation.py`负责：
...
`experiments/run_rq2_joint_deliverability_implementation_v2.py`编排内存阶段。
canonical 只读入口为：

```bash
OMP_NUM_THREADS=1 python -m \
  experiments.run_rq2_joint_deliverability_implementation_v2 --validate-only
```

`experiments/validate_rq2_joint_deliverability_implementation_v2.py`只做静态、
...
v2 只修复 official R3 review 登记的四项 Major：

1. full-support fallback certificate 必须与 representative incumbent、audit
   status 和 failure inventory 一致；
2. 相同 `planning_input_sha256` 的 solver-backed network-only 输出必须携带
   相同完整 arm evidence，不能只满足 reuse 计数；
3. model、scenario、evaluation 与 solver adapter 已隔离到空 initializer 的
   `src/rq2_joint_deliverability_v2/`；fresh-process import closure 中的本地模块
   均由 successor manifest 显式绑定；
4. result/success rename 只有在对应 parent-directory fsync 成功后才可进入
   readback reconciliation；否则固定为 `commit_indeterminate`。

对应反例覆盖 fallback 容量超限、fallback infeasible 伪装为 passed、
network-only 复用证书漂移和 success parent fsync 失败。上述修复不改变 V5
科学协议、solver 参数、46-cell 设计、output schema 或正式执行权限。
