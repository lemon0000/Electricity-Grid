# RQ2 joint-deliverability fresh-process activation successor v1

## 1. Status and authority

This R3 successor establishes the process-isolation boundary required by sealed
execution v3. It does not authorize or expose a write-capable experiment stage.
...
The activation review receipt remains outside the reviewed outer to avoid a
...
`bootstrap_rq2_joint_deliverability_activation_v1.py` imports only Python
standard-library modules at module load. Validation proceeds in this order:

1. stable-read and verify the activation config;
2. when sealed, verify activation outer, inner and every exact member;
3. recursively verify execution v3 outer, inner and all 22 members;
4. verify the fixed execution v3 PASS receipt digest;
5. stable-read and hash every member of the registered local Python closure;
6. bind the verified config bytes, activation/execution identities and all 13
   verified local source files into one canonical parent envelope;
7. create a private empty temporary bytecode-cache directory and start a
   ...
   bootstrap;
8. before project imports, reject site initialization,
   any nonempty or replaced bytecode-cache directory, and any preloaded local
   module other than the bootstrap script itself;
9. install a private in-memory loader for the envelope's verified local source
   ...
   v3;
10. revalidate execution v3 authority and confirm registered inputs remain
   non-ready;
11. require the isolated bytecode-cache directory to remain empty, map every
...
12. have the parent require exact `13/11/11` verified/observed/executed
...
symlink/reparse aliases, unstable reads, digest drift, a preloaded local module,
...
Presence checks walk path components with `lstat`; only `FileNotFoundError`
before any alias is accepted as absence. A dangling Windows junction therefore
cannot satisfy an authority-absence or draft-manifest-absence gate.
