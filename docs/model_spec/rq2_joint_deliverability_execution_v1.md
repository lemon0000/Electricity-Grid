# RQ2 联合服务可交付前沿 execution successor v1
...
静态 authority 由代码内部从 live bytes 派生：
...
调用者不能注入静态 trust root。v1 的四个公开 stage 入口保持硬关闭，统一抛出
...
本 v1 保留的私有 execution helpers 不接受 repository root、execution outer digest
...
`configs/rq2_joint_deliverability_execution_review_pass_v1.yaml`，要求独立
...
authority内部计算；调用者提供的 store identity 必须精确相等。v1将后两项登记为
...
test helper。v1公开planning入口保持硬关闭，后续fresh-process activation
